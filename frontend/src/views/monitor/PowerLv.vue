<template>
  <div>
    <div class="view-head">
      ...
      <span class="sub">{{ tl('0.4KV 低压配电') }} {{ tl('·') }} {{ tl('实时电力参数与运行状态') }}</span>
    </div>
    ...
    <div class="grid cols-4" v-if="s">
      ...
      <MetricCard metric-name="lv-load" :label="tl('平均负载率')" :value="s.avgLoadPercent" unit="%" quality="good" :online="true" />
      <MetricCard metric-name="lv-voltage" :label="tl('平均电压')" :value="s.avgVoltage" unit="V" quality="good" :online="true" />
    </div>
...
import { getPowerLv, type PowerSystemSummary } from '@/api/power'
const { t: tl } = useI18n()
