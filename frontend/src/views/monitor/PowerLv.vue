<template>
  <div class="net-lv">
    ...
    <div class="view-head">
      <h1>{{ tl('0.4KV 低压配电') }}</h1>
      <span class="sub">{{
        tl('变压器 → 低压柜/抽屉 → 母排 → 列头柜/PDU · 备自投 · 柴发进线 · 全电参量')
      }}</span>
    ...
    <template v-else-if="s">
      ...
      <Panel>
        ...
        <template #extra>
          <div class="legend">
            ...
            <button class="lv-edit" @click="editOpen = true">
        ...
        <div class="schematic-wrap">
          ...
          >
            ...
            <text :x="BUS_I.x + BUS_I.w / 2" :y="BUS_I.y - 8" class="bus-label">
              {{ tl('低压 Ⅰ段母排') }}
            </text>
            <!-- II 段母排 -->
            <rect
              ...
              class="bus"
            />
            <text :x="BUS_II.x + BUS_II.w / 2" :y="BUS_II.y - 8" class="bus-label">
              {{ tl('低压 Ⅱ段母排') }}
            </text>

            <!-- 变压器进线 T1 / T2 -->
            <g v-for="(t, ti) in txNodesView" :key="'t' + ti" class="feeder-line">
            ...
            >
              ...
              <text
                ...
                class="bus-tie-label"
              >
                {{ busTieNodeView.autoSwitch }}
              </text>
            </g>

            <!-- 柴发进线开关 QDG (mock) -->
            <g v-if="!dgNodeView.hidden" class="breaker-node" @click="selectNode(dgNodeView)">
              <rect
                :x="dgNodeView.x - BR_W / 2"
                :y="dgNodeView.y - BR_H / 2"
                :width="BR_W"
                :height="BR_H"
                rx="5"
                :class="['breaker-rect', breakerCls(dgNodeView.breaker)]"
              />
              <text :x="dgNodeView.x" :y="dgNodeView.y + 4" class="breaker-text">
                {{ dgNodeView.code }}
              </text>
            </g>
      ...
      <div class="grid cols-3">
        <Panel v-for="a in atsPanels" :key="a.id">
          ...
          <div class="ats-body">
            <div class="ats-row">
              <span class="k">{{ tl('模式') }}</span
              ><span class="v">{{ a.mode }}</span>
            </div>
            <div class="ats-row">
              <span class="k">{{ tl('常用侧电压') }}</span
              ><span class="v mono">{{ fmt(a.uIn) }} V</span>
            </div>
            <div class="ats-row">
              <span class="k">{{ tl('负载侧电压') }}</span
              ><span class="v mono">{{ fmt(a.uOut) }} V</span>
            </div>
            <div class="ats-row">
              <span class="k">{{ tl('负载功率') }}</span
              ><span class="v mono">{{ fmt(a.p, 0) }} kW</span>
            </div>
            <div class="ats-row">
              <span class="k">{{ tl('末次切换') }}</span
              ><span class="v mono muted">{{ a.lastSw }}</span>
            </div>
          </div>
          <!-- 电源流向条 -->
          <div class="flow-bar">
        ...
        <Panel>
          ...
          <template #extra>
            <span class="pill" :class="busTieNode.breaker.includes('合') ? 'g' : 'a'">{{
              busTieNode.breaker
            }}</span>
          </template>
          <div class="ats-body">
            <div class="ats-row">
              <span class="k">{{ tl('母联开关') }}</span
              ><span class="v mono">{{ busTieNode.code }} · {{ busTieNode.breaker }}</span>
            </div>
            <div class="ats-row">
              <span class="k">{{ tl('备自投逻辑') }}</span
              ><span class="v mono">{{ busTieNode.autoSwitch }}</span>
            </div>
            <div class="ats-row">
              <span class="k">{{ tl('柴发进线') }}</span
              ><span class="v mono">{{ dgNode.code }} · {{ dgNode.breaker }}</span>
            </div>
            <div class="ats-row">
              <span class="k">{{ tl('柴发状态') }}</span
              ><span class="v" :class="dgStateCls">{{ dgStateText }}</span>
            </div>
          </div>
          <div class="flow-bar">
            <span class="flow-src" :class="dgNode.breaker.includes('合') ? 'on' : ''">{{
              tl('柴发')
            }}</span>
            <span class="flow-arrow">→</span>
      ...
      <Panel class="scroll-x moni-card">
        ...
        <template #extra>
          <span class="pill" :class="branchAllClosed ? 'g' : 'a'"
            >{{ s.branches.length }} {{ tl('路') }} · {{ tl('合闸') }} {{ branchClosedCount }}/{{
              s.branches.length
            }}</span
          >
        </template>
        <table>
          ...
          <tbody>
            <tr v-for="d in s.branches" :key="d.id">
              ...
              <td class="mono">{{ fmt(d.ua, 0) }}</td>
              <td class="mono">{{ fmt(d.ub, 0) }}</td>
              <td class="mono">{{ fmt(d.uc, 0) }}</td>
              <td class="mono">{{ fmt(d.ia, 0) }}</td>
              <td class="mono">{{ fmt(d.ib, 0) }}</td>
              <td class="mono">{{ fmt(d.ic, 0) }}</td>
              <td class="mono">{{ fmt(d.freq) }}</td>
              <td class="mono">{{ fmt(d.p, 0) }}</td>
              <td class="mono">{{ fmt(d.q, 0) }}</td>
      ...
      <div class="grid cols-2">
        ...
        <Panel>
          ...
          <template #extra>
            <span class="pill" :class="imbalance > 2 ? 'a' : 'g'"
              >{{ tl('最大不平衡') }} {{ imbalance }}%</span
            >
          </template>
          <TrendChart :title="''" :x-axis-data="loadLabels" :series="loadSeries" :height="220" />
        </Panel>
      </div>
      ...
      <div class="grid cols-3" v-if="s.upsGroups?.length || s.hvdc?.length || s.busbars?.length">
        <Panel class="scroll-x moni-card" v-if="s.upsGroups?.length">
          ...
          <template #extra>
            <span class="pill" :class="upsAllNormal ? 'g' : 'a'"
              >{{ s.upsGroups.length }} {{ tl('组') }} · {{ upsNormalCount }} {{ tl('正常') }}</span
            >
          </template>
          <table class="mini-tbl">
            ...
            <tbody>
              <tr v-for="u in s.upsGroups" :key="u.id">
                ...
                <td>
                  <span class="tag" :class="u.bypass === '正常' ? 'g' : 'a'">{{ u.bypass }}</span>
                </td>
                <td>
                  <span class="tag" :class="u.state === '正常' ? 'g' : 'a'">{{ u.state }}</span>
                </td>
          ...
          <table class="mini-tbl">
            ...
            <tbody>
              <tr v-for="h in s.hvdc" :key="h.id">
                ...
                <td>
                  <span class="tag" :class="h.state === '正常' ? 'g' : 'a'">{{ h.state }}</span>
                </td>
              </tr>
            </tbody>
          </table>
          ...
          <table class="mini-tbl">
            ...
            <tbody>
              <tr v-for="b in s.busbars" :key="b.id">
                ...
                <td>
                  <span class="tag" :class="b.state === '正常' ? 'g' : 'a'">{{ b.state }}</span>
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <Panel class="scroll-x moni-card" v-if="s.spds?.length">
        ...
        <template #extra>
          <span class="pill" :class="spdAlarmCount === 0 ? 'g' : 'a'"
            >{{ s.spds.length }} {{ tl('路') }} · {{ tl('正常') }} {{ spdNormalCount }}/{{
              s.spds.length
            }}</span
          >
        </template>
        <table class="mini-tbl">
          ...
          <tbody>
            <tr v-for="sp in s.spds" :key="sp.id">
              ...
              <td>
                <span class="tag" :class="sigLevelTagCls(sp.level)">{{ sp.state }}</span>
              </td>
              <td class="mono" :class="sp.leakI > 0.5 ? 'r-text' : 'g-text'">
                {{ fmt(sp.leakI, 2) }}
              </td>
              <td class="mono" :class="sp.count > 5 ? 'a-text' : ''">{{ sp.count }}</td>
              <td>
                <span class="tag" :class="sp.status === '正常' ? 'g' : 'r'">{{ sp.status }}</span>
              </td>
            </tr>
          </tbody>
        </table>
...
const graphicEditor = useGraphicEditor('power-lv-schematic', {
  title: '0.4KV 低压一次系统图',
})
...
/** 变压器进线 (无 kvs, 只暴露名称与坐标) */
const txAdapter: NodeAdapter<{ x: number; y: number; busY: number; label: string }> = {
  toNode: (t) => ({ id: 'TX-' + t.label, label: t.label, type: tl('变压器'), x: t.x, y: t.y }),
  fromNode: (g, base) => ({
    x: g.x ?? base?.x ?? 0,
    y: g.y ?? base?.y ?? 0,
    busY: base?.busY ?? BUS_I.y,
    label: g.label || g.id,
  }),
}

const txNodesView = computed(() => graphicEditor.apply(txNodes.value, txAdapter))
const txBreakersView = computed(() => graphicEditor.apply(txBreakers.value, breakerAdapter))
const feederNodesView = computed(() => graphicEditor.apply(feederNodes.value, breakerAdapter))
/** 单节点: 被用户删除时带 hidden 标记, 模板据此隐藏 (结构件不能真的消失) */
...
const dgNodeView = computed(() => {
  const arr = graphicEditor.apply([dgNode.value], breakerAdapter)
  return { ...(arr[0] ?? dgNode.value), hidden: arr.length === 0 }
})
const graphicDefaults = (): GraphicNode[] => [
  ...txNodes.value.map(txAdapter.toNode),
  ...[...txBreakers.value, ...feederNodes.value, busTieNode.value, dgNode.value].map(
    breakerAdapter.toNode,
  ),
]
...
const branchAllClosed = computed(
  () => branches.value.length > 0 && branchClosedCount.value === branches.value.length,
)

const spdAlarmCount = computed(
  () => (s.value?.spds ?? []).filter((sp) => sp.status !== '正常').length,
)
const spdNormalCount = computed(
  () => (s.value?.spds ?? []).filter((sp) => sp.status === '正常').length,
)
const upsNormalCount = computed(
  () => (s.value?.upsGroups ?? []).filter((u) => u.state === '正常').length,
)
...
const energyReport = computed(() => {
  ...
  const mk = (seg: string, arr: LvBranchView[]) => ({
    seg,
    kwh: sumKwh(arr),
    pct: Number(((sumKwh(arr) / total) * 100).toFixed(1)),
...
const loadSeries = computed(() => [
  {
    name: tl('负载率 (%)'),
    type: 'bar' as const,
    data: branches.value.slice(0, 12).map((d) => d.loadPct),
    color: '#3b82f6',
    barWidth: '55%',
  },
])
  ...
  ;(s.value?.spds ?? []).forEach((sp) => {
    if (sp.status !== '正常')
      out.push({
        ...
        time: ts(3),
        source: sp.id,
        message: tl('防雷器 SPD 报警'),
        value: sp.status,
      })
    else if (sp.leakI > 0.5)
      out.push({
        level: 'warning',
        time: ts(5),
        source: sp.id,
        message: tl('SPD 泄漏电流增大'),
        value: sp.leakI.toFixed(2) + 'mA',
      })
  })
...
function mockSummary(): LvSummary {
  ...
  const branches: LvBranchView[] = branchDefs.map(([id, name, rated], i) => {
    ...
    return {
      ...
      p,
      q: Math.round(p * 0.28),
      pf: pClosed ? 0.94 + (i % 3) * 0.01 : 1,
      energy: Math.round(100000 + i * 450000),
      thdu: 2 + (i % 3),
      thdi: 3 + (i % 4),
      loadPct,
    }
...
async function loadData() {
  ...
  try {
    ...
    if (data && (data.branches?.length || data.transformers?.length)) {
      s.value = data
      markPartial('24h 相电流趋势、柴发/母联节点为本地演示，其余读数来自实时接口')
    } else {
      s.value = mockSummary()
      markMock()
    }
