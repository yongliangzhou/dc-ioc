<template>
  <div>
    <div class="view-head">
      ...
      <span class="sub">{{ tl('空调末端') }} {{ tl('·') }} {{ tl('实时送回风与湿度') }}</span>
    </div>
    ...
    <div class="grid cols-4" v-if="s">
      ...
      <MetricCard metric-name="crac-online" :label="tl('在线率')" :value="onlinePercent" unit="%" quality="good" :online="true" />
      <MetricCard metric-name="crac-supply" :label="tl('送风温度均值')" :value="s.avgTemperatureOut" unit="°C" quality="good" :online="true" />
      <MetricCard metric-name="crac-fan" :label="tl('风机转速均值')" :value="s.avgFanSpeed" unit="%" quality="good" :online="true" />
    </div>
...
import { getCrac, type CracSummary } from '@/api/hvac'
const { t: tl } = useI18n()
