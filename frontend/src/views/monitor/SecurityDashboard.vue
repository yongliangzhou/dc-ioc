<template>
  <div>
    <div class="view-head">
      ...
      <span class="sub">{{ tl('nav.securityCctv') }} / {{ tl('nav.securityAcs') }} / {{ tl('nav.securityIds') }} / {{ tl('nav.securityFire') }} {{ tl('·') }} {{ tl('总览 · 点击进入子系统') }}</span>
    </div>
    ...
    <div class="grid cols-2" v-if="overview">
      <router-link to="/monitor/security/cctv" class="entry-card">
        <div class="card-head"><span class="ct">{{ tl('nav.securityCctv') }}</span><span class="pill" :class="pillCls(overview.cctv)">{{ overview.cctv.online }}/{{ overview.cctv.total }} {{ tl('在线') }}</span></div>
        <div class="sub-label">{{ tl('视频监控 · 实时录像与回放') }}</div>
        <div class="kvs"><span class="k">{{ tl('今日事件') }}</span><span class="v">{{ overview.cctv.eventsToday ?? 0 }}</span><span class="k">{{ tl('今日告警') }}</span><span class="v">{{ overview.cctv.alertsToday ?? 0 }}</span></div>
      </router-link>
      <router-link to="/monitor/security/acs" class="entry-card">
        <div class="card-head"><span class="ct">{{ tl('nav.securityAcs') }}</span><span class="pill" :class="pillCls(overview.acs)">{{ overview.acs.online }}/{{ overview.acs.total }} {{ tl('在线') }}</span></div>
        <div class="sub-label">{{ tl('门禁管理 · 出入控制与授权') }}</div>
        <div class="kvs"><span class="k">{{ tl('今日事件') }}</span><span class="v">{{ overview.acs.eventsToday ?? 0 }}</span><span class="k">{{ tl('今日告警') }}</span><span class="v">{{ overview.acs.alertsToday ?? 0 }}</span></div>
      </router-link>
    </div>
    <div class="grid cols-2" v-if="overview" style="margin-top:12px">
      <router-link to="/monitor/security/ids" class="entry-card">
        <div class="card-head"><span class="ct">{{ tl('nav.securityIds') }}</span><span class="pill" :class="pillCls(overview.ids)">{{ overview.ids.online }}/{{ overview.ids.total }} {{ tl('在线') }}</span></div>
        <div class="sub-label">{{ tl('防入侵系统 · 周界与内部探测') }}</div>
        <div class="kvs"><span class="k">{{ tl('今日事件') }}</span><span class="v">{{ overview.ids.eventsToday ?? 0 }}</span><span class="k">{{ tl('今日告警') }}</span><span class="v">{{ overview.ids.alertsToday ?? 0 }}</span></div>
      </router-link>
      <router-link to="/monitor/security/fire" class="entry-card">
        <div class="card-head"><span class="ct">{{ tl('nav.securityFire') }}</span><span class="pill" :class="pillCls(overview.fire)">{{ overview.fire.online }}/{{ overview.fire.total }} {{ tl('在线') }}</span></div>
        <div class="sub-label">{{ tl('消防报警 · 烟感/温感/灭火') }}</div>
        <div class="kvs"><span class="k">{{ tl('今日事件') }}</span><span class="v">{{ overview.fire.eventsToday ?? 0 }}</span><span class="k">{{ tl('今日告警') }}</span><span class="v">{{ overview.fire.alertsToday ?? 0 }}</span></div>
      </router-link>
    </div>
...
import { getSecurityOverview, type SecurityOverview, type SecuritySystemSummary } from '@/api/security'
const { t: tl } = useI18n()
