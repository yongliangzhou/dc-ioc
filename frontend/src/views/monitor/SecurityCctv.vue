<template>
  <div>
    <div class="view-head">
      ...
      <span class="sub">{{ tl('视频监控') }} {{ tl('·') }} {{ tl('实时录像与回放') }}</span>
    </div>

    <div class="grid cols-4" v-if="s">
      <MetricCard metric-name="cctv-total" :label="tl('设备总数')" :value="s.total" unit="台" quality="good" :online="true" />
      <MetricCard metric-name="cctv-online" :label="tl('在线率')" :value="onlinePercent" unit="%" quality="good" :online="true" />
      <MetricCard metric-name="cctv-events" :label="tl('今日事件')" :value="s.eventsToday" unit="起" quality="good" :online="true" />
      <MetricCard metric-name="cctv-alerts" :label="tl('今日告警')" :value="s.alertsToday" unit="起" :quality="s.alertsToday ? 'uncertain' : 'good'" :online="true" :severity="s.alertsToday ? 'warn' : 'normal'" />
    </div>
    ...
    <div class="card" v-if="s">
      ...
      <div class="device-list" v-if="s.devices.length">
        <div class="device-row" v-for="d in s.devices" :key="d.id">
          ...
          <div class="d-metrics">
            <span v-if="d.lastEvent">{{ tl('最近') }}: {{ d.lastEvent }}</span>
            <span v-if="d.lastEventTime"> | {{ d.lastEventTime }}</span>
          </div>
        </div>
      </div>
...
import { getSecurityCctv, type SecuritySystemSummary } from '@/api/security'
const { t: tl } = useI18n()

const s = ref<SecuritySystemSummary | null>(null)
const error = ref('')
