  ...
  <div class="chiller-page">
    ...
    <div class="page-topbar">
      <h2 class="page-title">
        <span class="title-icon">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--cyan)" stroke-width="2"><path d="M12 2L4 6v6.09c0 5.05 3.41 9.76 8 10.91 4.59-1.15 8-5.86 8-10.91V6l-8-4z"/></svg>
        </span>
        冷源系统
      ...
      <div class="topbar-actions">
        ...
        <button class="btn-refresh" @click="loadData" :disabled="loading">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" :class="{ spinning: loading }"><path d="M21 12a9 9 0 1 1-9-9c2.52 0 4.93 1 6.74 2.74L21 8"/><path d="M21 3v5h-5"/></svg>
          {{ loading ? '刷新中...' : '刷新' }}
        </button>
      </div>
    </div>
    ...
    <div class="kpi-row">
      ...
      <KpiCard
        ...
        unit=""
        :subtitle="chiller?.total ? `${((chiller.online / chiller.total) * 100).toFixed(0)}% 在线率` : ''"
        :trend="0"
      />
    </div>
    ...
    <div class="section">
      ...
      <div class="flow-svg-wrap">
        <svg viewBox="0 0 1000 340" class="flow-svg" @click="onFlowClick">
          <!-- 冷冻回水总管 -->
          <rect x="20" y="130" width="60" height="100" rx="4" fill="rgba(6,182,212,0.08)" stroke="var(--cyan)" stroke-width="1"/>
          <text x="50" y="175" text-anchor="middle" fill="var(--cyan)" font-size="11">冷冻回水</text>
          <text x="50" y="192" text-anchor="middle" fill="var(--txt3)" font-size="9">总管</text>
          <text x="50" y="218" text-anchor="middle" fill="var(--txt)" font-size="11" font-weight="700">{{ chiller?.returnTemp ?? '-' }}℃</text>

          <!-- 冷水机组 1 -->
          <rect x="120" y="80" width="120" height="200" rx="8" class="flow-chiller" :class="chillerState(0)" data-idx="0"/>
          <text x="180" y="140" text-anchor="middle" fill="var(--txt)" font-size="13" font-weight="700">{{ groupName(0) }}</text>
          <StatusBadge :status="groupStatus(0)" size="tiny" class="flow-badge"/>
          <text x="180" y="200" text-anchor="middle" :fill="groupStatus(0) === 'online' ? '#22c55e' : 'var(--txt3)'" font-size="18" font-weight="800">{{ groupLoad(0) }}%</text>
          <text x="180" y="218" text-anchor="middle" fill="var(--cyan)" font-size="10">COP {{ groupCop(0) }}</text>
          <text x="180" y="235" text-anchor="middle" fill="var(--txt)" font-size="10">{{ groupEvapT(0) }} / {{ groupCondT(0) }}℃</text>
          <text x="180" y="260" text-anchor="middle" fill="var(--txt3)" font-size="9" class="click-hint">点击查看详情</text>

          <!-- 冷水机组 2 -->
          <rect x="280" y="80" width="120" height="200" rx="8" class="flow-chiller" :class="chillerState(1)" data-idx="1"/>
          <text x="340" y="140" text-anchor="middle" fill="var(--txt)" font-size="13" font-weight="700">{{ groupName(1) }}</text>
          <StatusBadge :status="groupStatus(1)" size="tiny" class="flow-badge"/>
          <text x="340" y="200" text-anchor="middle" :fill="groupStatus(1) === 'online' ? '#22c55e' : 'var(--txt3)'" font-size="18" font-weight="800">{{ groupLoad(1) }}%</text>
          <text x="340" y="218" text-anchor="middle" fill="var(--cyan)" font-size="10">COP {{ groupCop(1) }}</text>
          <text x="340" y="235" text-anchor="middle" fill="var(--txt)" font-size="10">{{ groupEvapT(1) }} / {{ groupCondT(1) }}℃</text>
          <text x="340" y="260" text-anchor="middle" fill="var(--txt3)" font-size="9" class="click-hint">点击查看详情</text>

          <!-- 冷水机组 3 -->
          <rect x="440" y="80" width="120" height="200" rx="8" class="flow-chiller" :class="chillerState(2)" data-idx="2"/>
          <text x="500" y="140" text-anchor="middle" fill="var(--txt)" font-size="13" font-weight="700">{{ groupName(2) }}</text>
          <StatusBadge :status="groupStatus(2)" size="tiny" class="flow-badge"/>
          <text x="500" y="200" text-anchor="middle" :fill="groupStatus(2) === 'online' ? '#22c55e' : 'var(--txt3)'" font-size="18" font-weight="800">{{ groupLoad(2) }}%</text>
          <text x="500" y="218" text-anchor="middle" fill="var(--cyan)" font-size="10">COP {{ groupCop(2) }}</text>
          <text x="500" y="235" text-anchor="middle" fill="var(--txt)" font-size="10">{{ groupEvapT(2) }} / {{ groupCondT(2) }}℃</text>
          <text x="500" y="260" text-anchor="middle" fill="var(--txt3)" font-size="9" class="click-hint">点击查看详情</text>

          <!-- 冷冻供水总管 -->
          <rect x="600" y="130" width="60" height="100" rx="4" fill="rgba(34,197,94,0.08)" stroke="var(--green)" stroke-width="1"/>
          <text x="630" y="175" text-anchor="middle" fill="var(--green)" font-size="11">冷冻供水</text>
          <text x="630" y="192" text-anchor="middle" fill="var(--txt3)" font-size="9">总管</text>
          <text x="630" y="218" text-anchor="middle" fill="var(--txt)" font-size="11" font-weight="700">{{ chiller?.supplyTemp ?? '-' }}℃</text>

          <!-- 冷却塔 -->
          <rect x="700" y="60" width="90" height="100" rx="6" fill="rgba(249,115,22,0.06)" stroke="var(--amber)" stroke-width="1" stroke-dasharray="4"/>
          <text x="745" y="100" text-anchor="middle" fill="var(--amber)" font-size="11">冷却塔</text>
          <text x="745" y="118" text-anchor="middle" fill="var(--txt3)" font-size="9">×{{ chiller?.towers?.length || 0 }}台</text>
          <text x="745" y="140" text-anchor="middle" fill="var(--txt)" font-size="10">{{ towerSummary }}</text>
          <text x="745" y="158" text-anchor="middle" fill="var(--txt3)" font-size="9" class="click-hint">点击展开</text>

          <!-- 冷冻泵 CHWP -->
          <rect x="120" y="295" width="120" height="40" rx="4" fill="rgba(6,182,212,0.06)" stroke="var(--line)" stroke-width="1"/>
          <text x="180" y="318" text-anchor="middle" fill="var(--cyan)" font-size="11">❰ 冷冻一次泵 CHWP</text>

          <!-- 冷却泵 CWP -->
          <rect x="280" y="295" width="120" height="40" rx="4" fill="rgba(249,115,22,0.06)" stroke="var(--line)" stroke-width="1"/>
          <text x="340" y="318" text-anchor="middle" fill="var(--amber)" font-size="11">❰ 冷却水泵 CWP</text>

          <!-- 板换 -->
          <rect x="440" y="295" width="120" height="40" rx="4" fill="rgba(139,92,246,0.06)" stroke="var(--line)" stroke-width="1"/>
          <text x="500" y="318" text-anchor="middle" fill="var(--purple2, #8b5cf6)" font-size="11">❰ 板式换热器</text>

          <!-- 连接线（简化：水平线 + 箭头） -->
          <line x1="80" y1="180" x2="115" y2="180" stroke="var(--cyan)" stroke-width="2" marker-end="url(#arrowCyan)"/>
          ...
          <line x1="560" y1="180" x2="595" y2="180" stroke="var(--green)" stroke-width="2" marker-end="url(#arrowGreen)"/>
          <line x1="660" y1="180" x2="695" y2="120" stroke="var(--amber)" stroke-width="1.5" stroke-dasharray="5 3"/>
          <line x1="745" y1="160" x2="745" y2="295" stroke="var(--amber)" stroke-width="1.5" stroke-dasharray="5 3" marker-end="url(#arrowAmber)"/>
          ...
          <defs>
            ...
            <marker id="arrowAmber" viewBox="0 0 10 10" refX="5" refY="10" markerWidth="6" markerHeight="6" orient="auto">
    ...
    <!-- ========== 制冷机组分组详情 ========== -->
    <div class="section">
      <h3 class="section-title"><span class="dot c0"></span> 制冷机组分组 (CH ↔ 冷冻泵 ↔ 冷却泵 ↔ 蓄冷罐)</h3>
      <div class="chiller-groups">
        <GroupCard
          v-for="(g, i) in chillerGroups"
          :key="'g'+i"
          :title="g.chiller?.id || `机组 ${i + 1}`"
          :status="g.chiller?.state === '运行' ? 'online' : g.chiller?.state === '检修' ? 'fault' : 'standby'"
          :defaultOpen="i === 0"
        >
          <!-- 冷机核心指标 -->
          <div class="group-grid">
            <KpiCard
              ...
              :progressColor="(g.chiller?.load ?? 0) > 80 ? 'var(--red)' : (g.chiller?.load ?? 0) > 60 ? 'var(--amber)' : 'var(--cyan)'"
              size="sm"
            />
            <KpiCard
              title="COP 能效比"
            ...
            <KpiCard
              title="蒸发温度"
              :value="g.chiller?.evapT ?? '-'"
              unit="℃"
              size="sm"
            />
            <KpiCard
              title="冷凝温度"
              :value="g.chiller?.condT ?? '-'"
              unit="℃"
              size="sm"
            />
            <KpiCard
    ...
    <!-- ========== 趋势图 2: COP vs %RLA 散点 ========== -->
    <div class="section">
      <h3 class="section-title"><span class="dot c2"></span> 机组能效比 COP 与负载率 %RLA 散点趋势</h3>
    ...
    <!-- ========== 趋势图 6: 制冷负载 + 自然冷源利用率 ========== -->
    <div class="section">
      <h3 class="section-title"><span class="dot c6"></span> 总制冷负载 RT 与自然冷源利用率 月度柱状叠加</h3>
      <TrendChart
        :height="320"
    ...
    <!-- ========== 趋势图 7: ΔT vs 旁通阀开度 ========== -->
    <div class="section">
      <h3 class="section-title"><span class="dot c7"></span> 冷冻水供回水温差 ΔT 与旁通阀开度 — 过去1小时</h3>
      <TrendChart
        :height="280"
    ...
    <div class="section" v-if="chiller">
      ...
      <div class="sub-row" v-if="chiller.towers?.length">
        <h4 class="sub-head">冷却塔 <span class="sub-count">{{ chiller.towers.length }}台</span></h4>
        <DeviceTable
          :columns="towerColumns"
          :rows="towerRows"
          :count="chiller.towers.length"
        />
      </div>

      <!-- 板式换热器 -->
      <div class="sub-row" v-if="chiller.hexs?.length">
        <h4 class="sub-head">板式换热器 <span class="sub-count">{{ chiller.hexs.length }}台</span></h4>
        <DeviceTable
          :columns="hexColumns"
          :rows="hexRows"
          :count="chiller.hexs.length"
        />
      </div>
      ...
      <div class="sub-row" v-if="chiller.storageTank">
        ...
        <div class="tank-info-cards">
          <KpiCard title="液位" :value="chiller.storageTank.level" unit="%" :progress="chiller.storageTank.level" :progressColor="'var(--green)'" size="sm" />
          <KpiCard title="运行模式" :value="chiller.storageTank.mode || '-'" unit="" size="sm" />
          <KpiCard title="放冷最低液位" :value="chiller.storageTank.dischargeMin" unit="%" size="sm" />
          <KpiCard title="罐体容量" :value="chiller.storageTank.capacity" unit="m³" :decimals="0" size="sm" />
          <KpiCard title="顶部温度" :value="chiller.storageTank.topTemp" unit="℃" :decimals="1" size="sm" />
          <KpiCard title="底部温度" :value="chiller.storageTank.botTemp" unit="℃" :decimals="1" size="sm" />
          <KpiCard title="当前流量" :value="chiller.storageTank.flow" unit="m³/h" :decimals="0" size="sm" />
          <KpiCard title="功率" :value="chiller.storageTank.power" unit="kW" :decimals="1" size="sm" />
        </div>
      </div>
      ...
      <div class="sub-row" v-if="chiller.pipePressure">
        ...
        <div class="tank-info-cards">
          <KpiCard title="冷冻供水母管" :value="chiller.pipePressure.supplyHeader" unit="bar" :decimals="2" size="sm" />
          <KpiCard title="冷冻回水母管" :value="chiller.pipePressure.returnHeader" unit="bar" :decimals="2" size="sm" />
          <KpiCard title="二次供水母管" :value="chiller.pipePressure.secSupplyHeader" unit="bar" :decimals="2" size="sm" />
          <KpiCard title="二次回水母管" :value="chiller.pipePressure.secReturnHeader" unit="bar" :decimals="2" size="sm" />
          <KpiCard title="冷却供水母管" :value="chiller.pipePressure.condenserSupply" unit="bar" :decimals="2" size="sm" />
          <KpiCard title="冷却回水母管" :value="chiller.pipePressure.condenserReturn" unit="bar" :decimals="2" size="sm" />
        </div>
      </div>
      ...
      <div class="sub-row">
        ...
        <div class="aux-grid">
          <div class="aux-card" v-if="chiller.makeupDevice?.id">
            <div class="aux-title">定压补水装置 <StatusBadge :status="chiller.makeupDevice.state === '运行' ? 'online' : 'standby'" size="tiny" /></div>
            <div class="aux-metrics">
              <span>供水压力 {{ chiller.makeupDevice.supplyPressure }} / {{ chiller.makeupDevice.setpointPressure }} bar</span>
          ...
          <div class="aux-card" v-if="chiller.bypassFilter?.id">
            <div class="aux-title">旁滤装置 <StatusBadge :status="chiller.bypassFilter.state === '运行' ? 'online' : 'standby'" size="tiny" /></div>
            <div class="aux-metrics">
...
async function loadData() {
  loading.value = true
  try {
    const [plantRes, trendsRes] = await Promise.all([
      getChillerPlant(),
      getChillerTrends(),
    ])
  ...
  try {
    ...
    activeAlarms.value = items
      .filter((a: any) =>
        ...
        a.level && a.level !== 'info' && a.status !== 'acknowledged'
      )
      .slice(0, 10)
      .map((a: any) => ({
...
const freezeSeries = computed(() => {
  ...
  if (!d) return []
  return [
    { name: '冷冻水出水温度', type: 'line' as const, data: d.supplyTemp, color: C.cyan, smooth: true },
    { name: '室外湿球温度', type: 'line' as const, data: d.wetBulb, color: C.green, smooth: true, lineStyle: { type: 'dashed' } },
    { name: '负载率', type: 'line' as const, data: d.loadPct, color: C.orange, smooth: true, yAxisIndex: 1, areaStyle: true },
  ]
})
...
const condSeries = computed(() => {
  ...
  return [
    ...
    { name: '温差', type: 'line' as const, data: d.diff, color: C.purple, smooth: true, lineStyle: { type: 'dashed' } },
  ]
})
...
const monthlySeries = computed(() => {
  ...
  return [
    ...
    { name: '自然冷源利用率(%)', type: 'line' as const, data: d.freeCoolingPct, color: C.green, smooth: true, yAxisIndex: 1, areaStyle: true },
  ]
})
...
const deltaSeries = computed(() => {
  ...
  if (!d) return []
  return [
    { name: 'ΔT供回水温差', type: 'line' as const, data: d.deltaT, color: C.cyan, smooth: true, areaStyle: true },
    { name: '旁通阀开度', type: 'line' as const, data: d.bypassValve, color: C.orange, smooth: true, yAxisIndex: 1 },
  ]
})
...
function renderCopRla() {
  ...
  copChart.setOption({
    tooltip: { trigger: 'item', formatter: (p: any) => `${p.value[2]}<br/>RLA: ${p.value[0]}% | COP: ${p.value[1]}` },
    legend: { data: chIds, bottom: 0, textStyle: { color: '#94a3b8', fontSize: 11 } },
    grid: { left: 55, right: 20, top: 20, bottom: 35 },
    xAxis: { type: 'value', name: 'RLA (%)', nameTextStyle: { color: '#94a3b8' }, axisLabel: { color: '#64748b' }, splitLine: { lineStyle: { color: 'var(--line)' } } },
    yAxis: { type: 'value', name: 'COP', nameTextStyle: { color: '#94a3b8' }, axisLabel: { color: '#64748b' }, splitLine: { lineStyle: { color: 'var(--line)' } } },
...
function renderPumpFreqFlow() {
  ...
  function tl(pts: { hz: number; flow: number }[]) {
    ...
    return [[mn, slope * mn + inter], [mx, slope * mx + inter]]
  }
  ...
  pumpChart.setOption({
    ...
    legend: { data: ['冷冻泵', '冷却泵'], bottom: 0, textStyle: { color: '#94a3b8', fontSize: 11 } },
    grid: { left: 55, right: 20, top: 20, bottom: 35 },
    xAxis: { type: 'value', name: '频率 (Hz)', nameTextStyle: { color: '#94a3b8' }, axisLabel: { color: '#64748b' }, splitLine: { lineStyle: { color: 'var(--line)' } } },
    yAxis: { type: 'value', name: '流量 (m³/h)', nameTextStyle: { color: '#94a3b8' }, axisLabel: { color: '#64748b' }, splitLine: { lineStyle: { color: 'var(--line)' } } },
    series: [
      { name: '冷冻泵', type: 'scatter', symbolSize: 7, data: data.chwPump.map((p: any) => [p.hz, p.flow]), itemStyle: { color: C.cyan, opacity: 0.7 } },
      { name: '冷冻泵趋势线', type: 'line', data: chwTL, lineStyle: { color: C.cyan, width: 2, type: 'dashed' }, symbol: 'none', silent: true },
      { name: '冷却泵', type: 'scatter', symbolSize: 7, data: data.cwPump.map((p: any) => [p.hz, p.flow]), itemStyle: { color: C.orange, opacity: 0.7 } },
      { name: '冷却泵趋势线', type: 'line', data: cwTL, lineStyle: { color: C.orange, width: 2, type: 'dashed' }, symbol: 'none', silent: true },
    ],
  }, true)
}
...
const towerRows = computed(() => (chiller.value?.towers ?? []).map(t => ({
  ...t,
...
const hexRows = computed(() => (chiller.value?.hexs ?? []).map(h => ({
  ...h,
...
const allPumpRows = computed(() => {
  const rows: any[] = []
  ;(chiller.value?.pumpsChw ?? []).forEach(p => rows.push({ ...p, type: '一次冷冻泵', status: p.state === '运行' ? 'online' : 'standby', inP: p.inPressure, outP: p.outPressure }))
  ;(chiller.value?.pumpsSec ?? []).forEach(p => rows.push({ ...p, type: '二次冷冻泵', status: p.state === '运行' ? 'online' : 'standby', inP: p.inPressure, outP: p.outPressure }))
  ;(chiller.value?.pumpsCw ?? []).forEach(p => rows.push({ ...p, type: '冷却泵', status: p.state === '运行' ? 'online' : 'standby', inP: p.inPressure, outP: p.outPressure }))
  return rows
})
...
.page-subtitle {
  font-size: 11px; font-weight: 400; color: var(--txt3);
  margin-left: 4px;
}
.topbar-actions {
  display: flex; align-items: center; gap: 10px;
...
.flow-svg {
  width: 100%; min-width: 840px; display: block;
}
.flow-chiller {
  cursor: pointer; transition: opacity 0.2s;
}
.flow-chiller:hover { opacity: 0.85; }
.flow-chiller.fault { opacity: 0.5; }
.flow-chiller.standby { opacity: 0.6; }
.flow-badge {
  display: flex; justify-content: center; margin-top: -2px;
}
.click-hint { opacity: 0; transition: opacity 0.15s; }
.flow-chiller:hover .click-hint { opacity: 1; }

/* Group Grid */
.chiller-groups { display: flex; flex-direction: column; gap: 12px; }
.group-grid {
  display: grid;
...
.pump-label.chw { background: rgba(6,182,212,0.15); color: #06b6d4; }
.pump-label.cw { background: rgba(249,115,22,0.15); color: #f97316; }
.pump-id { font-weight: 600; color: var(--txt); min-width: 60px; }
.pump-val { color: var(--txt2); font-variant-numeric: tabular-nums; }
...
.tank-status.on { color: #22c55e; }

/* Subsystem */
.sub-row { margin-bottom: 16px; }
...
.sub-count { font-size: 11px; color: var(--txt3); font-weight: 400; }
...
.alarm-row.emergency { border-left: 3px solid var(--red); background: rgba(239,68,68,0.05); }
.alarm-row.critical { border-left: 3px solid var(--amber, #f97316); background: rgba(249,115,22,0.04); }
.alarm-row.warning { border-left: 3px solid #eab308; }
.alarm-ts { color: var(--txt3); font-size: 11px; white-space: nowrap; }
