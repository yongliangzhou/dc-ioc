<template>
  <div>
    <div class="view-head">
      ...
      <span class="sub">{{ tl('10KV 中压配电') }} {{ tl('·') }} {{ tl('实时电力参数与运行状态') }}</span>
    </div>
    ...
    <div class="grid cols-4" v-if="s">
      ...
      <MetricCard metric-name="hv-load" :label="tl('平均负载率')" :value="s.avgLoadPercent" unit="%" quality="good" :online="true" />
      <MetricCard metric-name="hv-voltage" :label="tl('平均电压')" :value="s.avgVoltage" unit="V" quality="good" :online="true" />
    </div>
...
import { getPowerHv, type PowerSystemSummary } from '@/api/power'
const { t: tl } = useI18n()
