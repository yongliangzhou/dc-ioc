  ...
  <div class="net-hv">
    ...
    <template v-else-if="s">
      ...
      <Panel title="电气一次系统图">
        ...
        <div class="schematic-wrap">
          ...
          >
            ...
            <text :x="BUS_A.x + BUS_A.w / 2" :y="BUS_A.y - 8" class="bus-label">
              {{ tl('10KV Ⅰ段母线') }}
            </text>
            <rect :x="BUS_B.x" :y="BUS_B.y" :width="BUS_B.w" :height="BUS_H" rx="4" class="bus" />
            <text :x="BUS_B.x + BUS_B.w / 2" :y="BUS_B.y - 8" class="bus-label">
              {{ tl('10KV Ⅱ段母线') }}
            </text>
        ...
        <transition name="fade">
          <div v-if="selectedNode" class="node-detail">
            <div class="nd-head">
              ...
              <span class="tag" :class="breakerCls(selectedNode.breaker)">{{
                selectedNode.breaker
              }}</span>
      ...
      <div class="grid cols-6" v-if="primaryIncomer">
        ...
        <KpiCard
          :title="tl('电流')"
          :value="primaryIncomer.i"
          unit="A"
          :decimals="0"
          :bar-value="Math.min(100, (primaryIncomer.i / ratedI) * 100)"
          bar-color="var(--blue)"
          size="sm"
          :status="
            primaryIncomer.i > ratedI
              ? 'danger'
              : primaryIncomer.i > ratedI * 0.85
                ? 'warning'
                : 'normal'
          "
        />
        <KpiCard
          :title="tl('有功功率')"
          :value="primaryIncomer.p"
          unit="MW"
          :decimals="2"
          :bar-value="Math.min(100, (primaryIncomer.p / ratedP) * 100)"
        ...
        <KpiCard
          :title="tl('功率因数')"
          :value="primaryIncomer.pf"
          :decimals="3"
          :status="primaryIncomer.pf < 0.9 ? 'warning' : 'normal'"
          size="sm"
          :value-class="pfCls(primaryIncomer.pf)"
        />
        <KpiCard
          :title="tl('频率')"
          :value="primaryIncomer.freq"
          unit="Hz"
          :decimals="2"
          :status="freqStatus(primaryIncomer.freq)"
          size="sm"
        />
        <KpiCard
          :title="tl('电度')"
          :value="primaryIncomer.energy"
          unit="kWh"
          :decimals="0"
          size="sm"
          :prefix="''"
      ...
      <div class="grid cols-2">
        <Panel class="scroll-x" title="10KV 进线监测 (三相电参量)">
          <template #extra>
            <span class="pill" :class="incomerAllClosed ? 'g' : 'a'"
              >{{ s.incomers.length }} {{ tl('路') }} · {{ tl('合闸') }} {{ incomerClosedCount }}/{{
                s.incomers.length
              }}</span
            >
          </template>
          <table>
            ...
            <tbody>
              <tr v-for="d in s.incomers" :key="d.id">
                ...
                <td>
                  <span class="tag" :class="breakerCls(d.breaker)">{{ d.breaker }}</span>
                </td>
                <td class="mono">{{ fmt(d.ua) }}</td>
                <td class="mono">{{ fmt(d.ub) }}</td>
                <td class="mono">{{ fmt(d.uc) }}</td>
                <td class="mono">{{ fmt(d.ia, 0) }}</td>
                <td class="mono">{{ fmt(d.ib, 0) }}</td>
                <td class="mono">{{ fmt(d.ic, 0) }}</td>
                <td class="mono">{{ fmt(d.p) }}</td>
                <td class="mono">{{ fmt(d.q) }}</td>
        ...
        <Panel class="scroll-x" title="10KV 出线/馈线监测">
          <template #extra>
            <span class="pill" :class="feederAllClosed ? 'g' : 'a'"
              >{{ s.feeders.length }} {{ tl('路') }} · {{ tl('合闸') }} {{ feederClosedCount }}/{{
                s.feeders.length
              }}</span
            >
          </template>
      ...
      <div class="grid cols-2">
        <Panel title="谐波电压含有率 (THD / 各次)">
          <template #extra>
            <span class="pill" :class="(s.quality?.thdU ?? 0) > 3 ? 'a' : 'g'"
              >THD-U {{ fmt(s.quality?.thdU) }}%</span
            >
          </template>
          <TrendChart
            :title="''"
            ...
            <tbody>
              <tr v-for="(e, ei) in events" :key="ei">
                ...
                <td>
                  <span
                    class="tag"
                    :class="e.type === 'trip' ? 'r' : e.type === 'action' ? 'a' : 'b'"
                    >{{ e.typeText }}</span
                  >
                </td>
                ...
                <td>
                  <span class="tag" :class="e.acted ? 'r' : 'g'">{{
                    e.acted ? tl('动作') : tl('告警')
                  }}</span>
                </td>
              </tr>
      ...
      <div class="grid cols-3" v-if="s.transformers?.length">
        <Panel class="col-span-2" title="10KV/0.4KV 配电变压器">
          <template #extra>
            <span class="pill" :class="txAllRunning ? 'g' : 'a'"
              >{{ s.transformers.length }} {{ tl('台') }} · {{ txRunningCount }}
              {{ tl('运行') }}</span
            >
          </template>
          <div class="tx-grid">
            <div class="tx-block" v-for="t in s.transformers" :key="t.id">
              ...
              <div class="tx-mini-grid">
                <div class="tx-mini">
                  <span class="k">{{ tl('负载率') }}</span
                  ><span class="v mono" :class="loadCls(t.load)">{{ t.load }}%</span>
                </div>
                <div class="tx-mini">
                  <span class="k">{{ tl('绕组温度') }}</span
                  ><span class="v mono" :class="tempCls(t.windingT, 85, 95)"
                    >{{ t.windingT }}°C</span
                  >
                </div>
                <div class="tx-mini">
                  <span class="k">{{ tl('油温') }}</span
                  ><span class="v mono" :class="tempCls(t.oilT, 75, 85)">{{ t.oilT }}°C</span>
                </div>
                <div class="tx-mini">
                  <span class="k">{{ tl('环境温度') }}</span
                  ><span class="v mono">{{ t.ambT }}°C</span>
                </div>
                <div class="tx-mini">
                  <span class="k">{{ tl('湿度') }}</span
                  ><span class="v mono" :class="humCls(t.humidity)">{{ t.humidity }}%RH</span>
                </div>
                <div class="tx-mini">
                  <span class="k">{{ tl('高压侧') }}</span
                  ><span class="v mono">{{ fmt(t.uHigh) }}kV / {{ fmt(t.iHigh, 0) }}A</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <Panel title="母线段电压">
          ...
          <div class="bus-grid">
            <div class="bus-item" v-for="b in s.busSections" :key="b.id">
              ...
              <span class="bus-val" :class="busStateCls(b.state)"
                >{{ fmt(b.u) }} <small>kV</small></span
              >
          ...
          <div class="mt10" v-if="s.dcPanel">
            <div class="kv">
              <span class="k">{{ tl('直流屏') }} {{ s.dcPanel.id }}</span
              ><span class="v">{{ s.dcPanel.state }}</span>
            </div>
            <div class="kv">
              <span class="k">DC {{ tl('母线') }}</span
              ><span class="v mono"
                >{{ s.dcPanel.dcBus }}V / {{ tl('目标') }} {{ s.dcPanel.dcBusTarget }}V</span
              >
            </div>
          </div>
        </Panel>
      ...
      <!-- ======== 微机保护装置 ======== -->
      <Panel class="scroll-x" v-if="s.protectionRelays?.length" title="微机保护装置 (REF615/611)">
        <template #extra>
          <span class="pill" :class="relayAllComm ? 'g' : 'a'"
            >{{ s.protectionRelays.length }} {{ tl('套') }} · {{ tl('通讯正常') }}
            {{ relayCommCount }}/{{ s.protectionRelays.length }}</span
          >
        </template>
        <table>
          <thead>
          ...
          <tbody>
            <tr v-for="r in s.protectionRelays" :key="r.id">
              ...
              <td>
                <span class="tag" :class="r.state === '运行' ? 'g' : 'b'">{{ r.state }}</span>
              </td>
              <td>
                <span class="tag" :class="relayFuncCls(r.overcurrent)">{{ r.overcurrent }}</span>
              </td>
              <td>
                <span class="tag" :class="relayFuncCls(r.earthFault)">{{ r.earthFault }}</span>
              </td>
              <td>
                <span class="tag" :class="relayFuncCls(r.diff)">{{ r.diff }}</span>
              </td>
              <td>
                <span class="tag" :class="relayFuncCls(r.underVoltage)">{{ r.underVoltage }}</span>
              </td>
              <td>
                <span class="tag" :class="relayFuncCls(r.overVoltage)">{{ r.overVoltage }}</span>
              </td>
              <td>
                <span class="tag" :class="relayFuncCls(r.freq)">{{ r.freq }}</span>
              </td>
              <td>
                <span class="tag" :class="r.comm === '正常' ? 'g' : 'r'">{{ r.comm }}</span>
              </td>
              <td class="muted mono" :class="r.tripCount > 0 ? 'a' : ''" style="font-size: 11px">
                {{ r.lastTrip }}
              </td>
...
function genPf(n: number, base: number, noise: number): number[] {
  return Array.from({ length: n }, (_, i) =>
    Number(
      (base + Math.sin((i / n) * Math.PI * 2) * 0.02 + (Math.random() - 0.5) * noise).toFixed(3),
    ),
  )
}
const pfTrend = reactive({
  labels: genHours(24),
  series: [
    { name: tl('功率因数'), type: 'line' as const, data: genPf(24, 0.97, 0.02), color: '#22c55e' },
  ],
})
...
const events = computed(() => {
  ...
  if (s.value) {
    out.push({
      time: '08-02 03:14:22',
      device: s.value.incomers[0]?.id ?? 'IN1',
      type: 'action',
      typeText: tl('备自投'),
      element: s.value.busTie?.id ?? 'QB',
      desc: tl('Ⅰ段失电，母联自投成功，Ⅱ段带全站负荷'),
      acted: false,
    })
    out.push({
      time: '07-31 22:48:05',
      device: s.value.feeders[0]?.id ?? 'F1',
      type: 'trip',
      typeText: tl('跳闸'),
      element: '50/51',
      desc: tl('馈线速断保护动作，排查电缆接地'),
      acted: true,
    })
  }
...
// 统计
// ──────────────────────────────────────────
const incomerClosedCount = computed(
  () => (s.value?.incomers ?? []).filter((d) => isClosed(d.breaker)).length,
)
const incomerAllClosed = computed(() => {
  const l = s.value?.incomers ?? []
  return l.length > 0 && incomerClosedCount.value === l.length
})
const feederClosedCount = computed(
  () => (s.value?.feeders ?? []).filter((d) => isClosed(d.breaker)).length,
)
const feederAllClosed = computed(() => {
  const l = s.value?.feeders ?? []
  return l.length > 0 && feederClosedCount.value === l.length
})
const txRunningCount = computed(
  () => (s.value?.transformers ?? []).filter((t) => t.state === '运行').length,
)
const txAllRunning = computed(() => {
  const l = s.value?.transformers ?? []
  return l.length > 0 && txRunningCount.value === l.length
})
const relayCommCount = computed(
  () => (s.value?.protectionRelays ?? []).filter((r) => r.comm === '正常').length,
)
...
function mockSummary(): HvSummary {
  ...
  return {
    ...
    ats: {
      logic: tl('Ⅰ段失电经延时跳 Q1、合 QB 由Ⅱ段带全站负荷'),
      lastTest: '2026-07-20',
      switchTime: '<2s',
    },
    feeders,
    ...
    arcSuppression: {
      ...
      residualCurrent: 4,
      state: tl('运行'),
      groundingTx: { id: 'ZT1', state: '运行', t: 52, i: 12 },
    },
    metering: {
      incomer1: {
        energyTotal: 18452300,
        energyPeak: 7200000,
        energyValley: 3100000,
        energyFlat: 8152300,
        demand: 12.4,
        demandMax: 14.8,
      },
      incomer2: {
        energyTotal: 15098200,
        energyPeak: 6000000,
        energyValley: 2500000,
        energyFlat: 6598200,
        demand: 10.1,
        demandMax: 12.0,
      },
    ...
    quality: {
      ...
      thdI: 4.1,
      unbalance: 1.3,
      incomer1: { thdU: 2.3, thdI: 4.0, unbalance: 1.2, flicker: 0.7 },
      incomer2: { thdU: 2.5, thdI: 4.2, unbalance: 1.4, flicker: 0.8 },
    },
