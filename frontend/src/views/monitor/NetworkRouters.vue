<template>
  <div>
    <div class="view-head">
      <h1>{{ tl('设施监控') }} {{ tl('·') }} {{ tl('nav.networkMonitor') }} {{ tl('·') }} {{ tl('nav.networkRrouter') }}</h1>
      <span class="sub">{{ tl('路由转发') }} · {{ tl('协议状态') }}</span>
    </div>

    <div class="grid cols-4" v-if="s">
      <MetricCard metric-name="rt-total" :label="tl('设备总数')" :value="s.total" unit="台" quality="good" :online="true" />
      <MetricCard metric-name="rt-online" :label="tl('在线率')" :value="onlinePercent" unit="%" quality="good" :online="true" />
      <MetricCard metric-name="rt-ping" :label="tl('平均延迟')" :value="s.avgPingMs" unit="ms" quality="good" :online="true" />
      <MetricCard metric-name="rt-bw" :label="tl('平均带宽利用率')" :value="s.avgBwUtilization" unit="%" :quality="s.avgBwUtilization && s.avgBwUtilization > 70 ? 'uncertain' : 'good'" :online="true" :severity="s.avgBwUtilization && s.avgBwUtilization > 85 ? 'warn' : 'normal'" />
    </div>

    <div class="card" v-if="s">
      <div class="card-head">
        <span class="ct">{{ tl('nav.networkRrouter') }}</span>
      ...
      <div class="device-list" v-if="s.devices.length">
        <div class="device-row" v-for="d in s.devices" :key="d.id">
          ...
          <div class="d-metrics">
            <span v-if="d.pingMs != null">{{ d.pingMs.toFixed(1) }}ms</span>
            <span> | {{ d.bwUtilization != null ? d.bwUtilization.toFixed(0) + '%' : '-' }}</span>
            <span> | {{ d.portCount }}P</span>
            <span v-if="d.firmwareVersion"> | {{ d.firmwareVersion }}</span>
          </div>
        </div>
      </div>
...
import { getNetworkOverview, type NetworkSystemSummary } from '@/api/monitor'
const { t: tl } = useI18n()

const s = ref<NetworkSystemSummary | null>(null)
const error = ref('')
...
async function load() {
  error.value = ''
  try {
    const o = await getNetworkOverview()
    s.value = o.routers
  } catch (e: any) {
