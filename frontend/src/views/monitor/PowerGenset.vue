<template>
  <div>
    <div class="view-head">
      ...
      <span class="sub">{{ tl('柴发并机系统') }} {{ tl('·') }} {{ tl('实时电力参数与运行状态') }}</span>
    </div>

    <div class="grid cols-4" v-if="s">
      <MetricCard metric-name="genset-total" :label="tl('设备总数')" :value="s.total" unit="台" quality="good" :online="true" />
      <MetricCard metric-name="genset-online" :label="tl('在线率')" :value="onlinePercent" unit="%" quality="good" :online="true" />
      <MetricCard metric-name="genset-load" :label="tl('平均负载率')" :value="s.avgLoadPercent" unit="%" quality="good" :online="true" />
      <MetricCard metric-name="genset-voltage" :label="tl('平均电压')" :value="s.avgVoltage" unit="V" quality="good" :online="true" />
    </div>
      ...
      <div class="device-list" v-if="s.devices.length">
        <div class="device-row" v-for="d in s.devices" :key="d.id">
          ...
          <div class="d-metrics">
            ...
            <span v-if="d.voltage != null"> | {{ d.voltage.toFixed(0) }}V</span>
          </div>
        </div>
      </div>
...
import { getPowerGenset, type PowerSystemSummary } from '@/api/power'
const { t: tl } = useI18n()
