<template>
  <div>
    <div class="view-head">
      ...
      <span class="sub">{{ tl('核心层') }} · {{ tl('汇聚层') }} · {{ tl('接入层') }} {{ tl('·') }} {{ tl('交换机端口状态与流量') }}</span>
    </div>

    <div class="grid cols-4" v-if="s">
      <MetricCard metric-name="sw-total" :label="tl('设备总数')" :value="s.total" unit="台" quality="good" :online="true" />
      <MetricCard metric-name="sw-online" :label="tl('在线率')" :value="onlinePercent" unit="%" quality="good" :online="true" />
      <MetricCard metric-name="sw-ping" :label="tl('平均延迟')" :value="s.avgPingMs" unit="ms" quality="good" :online="true" />
      <MetricCard metric-name="sw-bw" :label="tl('平均带宽利用率')" :value="s.avgBwUtilization" unit="%" :quality="s.avgBwUtilization && s.avgBwUtilization > 70 ? 'uncertain' : 'good'" :online="true" :severity="s.avgBwUtilization && s.avgBwUtilization > 85 ? 'warn' : 'normal'" />
    </div>
