<template>
  <div>
    <div class="view-head">
      ...
      <span class="sub">{{ tl('消防报警') }} {{ tl('·') }} {{ tl('烟感/温感/灭火') }}</span>
    </div>

    <div class="grid cols-4" v-if="s">
      <MetricCard metric-name="fire-total" :label="tl('设备总数')" :value="s.total" unit="台" quality="good" :online="true" />
      <MetricCard metric-name="fire-online" :label="tl('在线率')" :value="onlinePercent" unit="%" quality="good" :online="true" />
      <MetricCard metric-name="fire-events" :label="tl('今日事件')" :value="s.eventsToday" unit="起" quality="good" :online="true" />
      <MetricCard metric-name="fire-alerts" :label="tl('今日告警')" :value="s.alertsToday" unit="起" :quality="s.alertsToday ? 'bad' : 'good'" :online="true" :severity="s.alertsToday ? 'crit' : 'normal'" />
    </div>
...
import { getSecurityFire, type SecuritySystemSummary } from '@/api/security'
const { t: tl } = useI18n()
