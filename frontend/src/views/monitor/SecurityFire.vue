    ...
    <div class="grid-main">
        ...
        <div class="plan-wrap">
          <svg viewBox="0 0 620 400" class="plan-svg" @click="selected = null">
            ...
            <g v-if="alarmPoint" class="pop">
              ...
              <text x="442" y="208" class="pop-d">状态：{{ alarmPoint.state === 'alarm' ? '报警' : '故障' }}</text>
              <text x="442" y="226" class="pop-d" v-if="alarmPoint.desc">{{ alarmPoint.desc }}</text>
            </g>

            <!-- 消防点位 -->
            <g v-for="p in points" :key="p.id" @click.stop="selectPoint(p)">
              <circle :cx="p.x" :cy="p.y" r="5.5" :class="'fp ' + p.kind + ' ' + p.state" />
              <circle v-if="p.state === 'alarm'" :cx="p.x" :cy="p.y" r="10" class="fp-pulse" />
              <text :x="p.x + 8" :y="p.y + 3" class="fp-name" v-if="p.state !== 'normal' || selected">{{ p.short }}</text>
            </g>
          </svg>
          <div v-if="selected" class="z-info">
            <div class="z-info-h"><span class="fp" :class="selected.kind + ' ' + selected.state" style="width:10px;height:10px;border-radius:50%;display:inline-block"></span>{{ selected.label }}</div>
            <div class="z-info-b">
      ...
      <Panel class="host-card">
        ...
        <div class="host-grid">
          ...
          <div class="host-kv"><span class="k">回路数</span><span class="v">{{ s.loops }}</span></div>
          <div class="host-kv"><span class="k">探测点位</span><span class="v">{{ s.points }}</span></div>
          <div class="host-kv"><span class="k">故障点位</span><span class="v" :class="s.faultPoints ? 'a' : 'g'">{{ s.faultPoints }}</span></div>
          <div class="host-kv"><span class="k">气体灭火</span><span class="v" :class="s.gas.ready === s.gas.zones ? 'g' : 'a'">{{ s.gas.ready }}/{{ s.gas.zones }} 就绪</span></div>
          <div class="host-kv"><span class="k">已释放</span><span class="v" :class="s.gas.released ? 'a' : 'g'">{{ s.gas.released }} 区</span></div>
        </div>
        <div class="host-vesda">
          <div class="hv-head">极早期 VESDA（吸气式）</div>
          <div class="hv-list">
            <div v-for="v in s.vesda" :key="v.id" class="hv-row" :class="v.level === '轻微' ? 'warn' : 'ok'">
    ...
    <div class="grid-sub">
      ...
      <Panel>
        ...
        <div class="det-tabs">
          ...
          <button class="det-tab" :class="{ active: detFilter === 'smoke' }" @click="detFilter = 'smoke'">烟感</button>
          <button class="det-tab" :class="{ active: detFilter === 'heat' }" @click="detFilter = 'heat'">温感</button>
          <button class="det-tab" :class="{ active: detFilter === 'manual' }" @click="detFilter = 'manual'">手报/声光</button>
          <button class="det-tab" :class="{ active: detFilter === 'door' }" @click="detFilter = 'door'">防火门</button>
        </div>
        <div class="stat-row">
          <div class="stat-pie">
            <div class="sp-ring" :style="ringStyle">
              <span class="sp-center"><b>{{ detSummary.normal }}</b><em>正常</em></span>
            </div>
          </div>
          <div class="stat-legend">
            <div class="sl" @click="detFilter = 'all'"><i class="sl-dot g"></i>正常<b>{{ detSummary.normal }}</b></div>
            <div class="sl" @click="detFilter = 'alarm'"><i class="sl-dot a"></i>报警<b>{{ detSummary.alarm }}</b></div>
            <div class="sl" @click="detFilter = 'fault'"><i class="sl-dot w"></i>故障<b>{{ detSummary.fault }}</b></div>
            <div class="sl" @click="detFilter = 'offline'"><i class="sl-dot o"></i>离线<b>{{ detSummary.offline }}</b></div>
          </div>
        </div>
        <div class="det-list">
          <div v-for="d in filteredDets" :key="d.type" class="det-row" :class="d.fault ? 'warn' : 'ok'">
            <span class="d-name">{{ d.type }}</span>
            <span class="d-bar"><span class="d-fill" :class="d.fault ? 'warn' : 'ok'" :style="{ width: detPct(d) + '%' }"></span></span>
      ...
      <Panel>
        ...
        <div class="qie-fei">
          <div class="qf-row"><span class="k">切非联动</span><span class="v">{{ s.qieFei.state }}</span></div>
...
const alarmPoint = computed(() => points.value.find((p) => p.state === 'alarm') || null)
...
function buildPoints(detectors: FireDetectorView[], events: FireEventView[]): Point[] {
  ...
  layout.forEach((l) => {
    l.xs.forEach((x, i) => {
      out.push({
        id: `${l.kind}-${idx + 1}`, label: `${kindText(l.kind)} ${idx + 1}`,
        short: `${lk(l.kind)}${idx + 1}`, kind: l.kind, state: 'normal', x, y: l.ys[i] ?? l.ys[0], zone: l.zone,
      })
      idx++
    })
  })
  ...
  if (alarmEvent) {
    ...
    if (p) { p.state = 'alarm'; p.desc = alarmEvent.desc }
  }
  const faultEvent = events.find((e) => /故障|误报/.test(e.desc))
  if (faultEvent) {
    const p = out.find((o) => o.kind === 'temp')
    if (p) { p.state = 'fault'; p.desc = faultEvent.desc }
  }
  return out
}
function kindText(k: string) { return { smoke: '感烟探测器', temp: '感温探测器', sprinkler: '喷淋头', manual: '手动报警', hydrant: '消火栓' }[k] || k }
function lk(k: string) { return { smoke: 'S', temp: 'T', sprinkler: 'P', manual: 'M', hydrant: 'H' }[k] || 'X' }
function selectPoint(p: Point) { selected.value = p }
function stateText(st: string) { return { normal: '正常', alarm: '报警', fault: '故障', offline: '离线' }[st] || st }

// 探测器统计
const detFilter = ref<'all' | 'smoke' | 'heat' | 'manual' | 'door' | 'alarm' | 'fault' | 'offline'>('all')
...
const filteredDets = computed(() => {
  ...
  const map: Record<string, string[]> = { smoke: ['感烟探测器', '极早期(VESDA)'], heat: ['感温探测器'], manual: ['手报/声光'], door: ['防火门监控'] }
  return s.value.detectors.filter((d) => map[detFilter.value]?.includes(d.type))
})
function detPct(d: FireDetectorView): number { return d.n ? +(((d.n - d.fault) / d.n) * 100).toFixed(0) : 0 }
...
function buildLinkage(events: FireEventView[]): LinkDev[] {
  ...
  return [
    { name: '排烟风机', icon: Fan, state: hasAlarm ? 'act' : 'ok', stateText: hasAlarm ? '联动启动' : '待命' },
    { name: '消防广播', icon: Volume2, state: hasAlarm ? 'act' : 'ok', stateText: hasAlarm ? '疏散广播' : '待命' },
    { name: '门禁释放', icon: DoorOpen, state: hasAlarm ? 'act' : 'ok', stateText: hasAlarm ? '已释放' : '正常' },
    { name: '电梯迫降', icon: ArrowDownToLine, state: hasAlarm ? 'act' : 'ok', stateText: hasAlarm ? '迫降首层' : '正常' },
  ]
}
...
async function load() {
  try {
    ...
    if (data && data.points) { s.value = data; usingMock.value = false }
...
function mockSummary(): FireSummary {
  return {
    ...
    vesda: Array.from({ length: 6 }, (_, i) => ({ id: `VESDA R${String(i * 2 + 1).padStart(2, '0')}`, level: i === 1 ? '轻微' : '正常', val: +(i === 1 ? 0.012 : 0.002).toFixed(3) })),
    ...
    knowledge: {
      ...
      arch: {
        components: ['感烟/感温探测器', '手动报警按钮', '声光报警器', '消防广播', '防火门监控', '气体灭火控制盘', '消防联动控制器'],
      ...
      faults: [
        { no: 1, fault: '探测器故障', lock: '该点位失监+故障告警', action: '查线路/底座', manualReset: false },
        { no: 2, fault: '回路短路', lock: '整回路失监', action: '查短路点', manualReset: false },
        { no: 3, fault: '气灭钢瓶压力低', lock: '该区气灭失效', action: '补气/换瓶', manualReset: true },
        { no: 4, fault: '联动控制器离线', lock: '联动失效', action: '查供电/网络', manualReset: true },
      ],
...
.host-badge { font-size: 12px; padding: 4px 10px; border-radius: 20px; }
.host-badge.g { background: rgba(47,174,107,.12); color: #2fae6b; }
.host-badge.a { background: rgba(208,106,58,.12); color: #d06a3a; }
...
.lg-dot.sm { background: #3f9fcf; } .lg-dot.tm { background: #d06a3a; } .lg-dot.sp { background: #5b8def; }
.lg-dot.mb { background: #c065d6; } .lg-dot.hy { background: #2fae6b; } .lg-dot.a { background: #d23b3b; }
...
.corridor { fill: #14202c; stroke: #2a3d4d; }
.zone-block { fill: #101b26; stroke: #243240; stroke-dasharray: 3 3; }
.s-label { fill: #7d93a8; font-size: 12px; font-weight: 600; }
.z-label { fill: #8aa2b5; font-size: 11px; }
.fp { stroke: #0a0e14; stroke-width: 1; }
.fp.smoke { fill: #3f9fcf; } .fp.temp { fill: #d06a3a; } .fp.sprinkler { fill: #5b8def; }
.fp.manual { fill: #c065d6; } .fp.hydrant { fill: #2fae6b; }
.fp.alarm { fill: #d23b3b; } .fp.fault { fill: #d23b3b; stroke: #d23b3b; }
.fp-pulse { fill: none; stroke: #d23b3b; stroke-width: 1.5; animation: fppulse 1.4s ease-out infinite; }
@keyframes fppulse { 0% { r: 7; opacity: .9; } 100% { r: 16; opacity: 0; } }
.fp-name { fill: #9fb3c5; font-size: 8px; }
.pop-bg { fill: #14202c; stroke: #d23b3b; stroke-width: 1.5; }
...
.host-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 2px 14px; }
.host-kv { display: flex; flex-direction: column; gap: 2px; padding: 6px 0; border-bottom: 1px dashed var(--border); }
.host-kv .k { font-size: 11px; color: var(--text-3); } .host-kv .v { font-size: 13px; font-weight: 600; }
.g { color: #2fae6b; } .a { color: #d06a3a; }
.host-vesda { margin-top: 10px; }
.hv-head { font-size: 12px; font-weight: 600; margin-bottom: 6px; color: var(--text-2); }
.hv-list { display: flex; flex-direction: column; gap: 3px; }
.hv-row { display: grid; grid-template-columns: 1fr auto 44px; align-items: center; gap: 6px; font-size: 12px; padding: 4px 6px; border-radius: 5px; }
.hv-row.ok { background: rgba(47,174,107,.08); } .hv-row.warn { background: rgba(208,106,58,.12); }
.hv-id { font-weight: 500; } .hv-val { color: var(--text-2); } .hv-lv { font-size: 11px; }
.hv-lv.ok { color: #2fae6b; } .hv-lv.warn { color: #d06a3a; }

/* 探测器统计 */
.det-tabs { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 10px; }
...
.stat-row { display: flex; align-items: center; gap: 18px; margin-bottom: 12px; }
.sp-ring { width: 88px; height: 88px; border-radius: 50%; display: flex; align-items: center; justify-content: center; position: relative; }
.sp-center { background: var(--bg-1); width: 64px; height: 64px; border-radius: 50%; display: flex; flex-direction: column; align-items: center; justify-content: center; }
.sp-center b { font-size: 18px; } .sp-center em { font-size: 10px; color: var(--text-2); font-style: normal; }
.stat-legend { display: flex; flex-direction: column; gap: 4px; font-size: 12px; }
.sl { display: flex; align-items: center; gap: 6px; cursor: pointer; }
.sl-dot { width: 9px; height: 9px; border-radius: 50%; }
.sl-dot.g { background: #2fae6b; } .sl-dot.a { background: #d23b3b; } .sl-dot.w { background: #d06a3a; } .sl-dot.o { background: #8a93a0; }
.sl b { margin-left: 4px; }
.det-list { display: flex; flex-direction: column; gap: 4px; max-height: 180px; overflow-y: auto; }
.det-row { display: grid; grid-template-columns: 1fr 70px 50px; align-items: center; gap: 8px; padding: 4px 6px; border-radius: 5px; font-size: 12px; }
.det-row:hover { background: var(--bg-2); }
.det-row.warn { background: rgba(208,106,58,.1); }
.d-name { font-weight: 500; } .d-st { color: var(--text-2); text-align: right; }
.d-bar { height: 8px; border-radius: 4px; background: var(--track, #2a3645); overflow: hidden; }
.d-fill { display: block; height: 100%; border-radius: 4px; }
.d-fill.ok { background: linear-gradient(90deg, rgba(47,174,107,.5), rgba(47,174,107,.85)); }
.d-fill.warn { background: linear-gradient(90deg, rgba(208,106,58,.5), rgba(208,106,58,.85)); }

/* 联动设备 */
.link-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
.link-item { display: flex; align-items: center; gap: 10px; padding: 10px; border-radius: 8px; background: var(--bg-2); border: 1px solid var(--border); }
.link-item.ok { border-color: rgba(47,174,107,.3); }
.link-item.act { border-color: rgba(210,59,59,.4); background: rgba(210,59,59,.08); }
.link-item.fault { border-color: rgba(208,106,58,.4); }
.li-ico { width: 34px; height: 34px; border-radius: 8px; display: flex; align-items: center; justify-content: center; background: var(--bg-1); }
.link-item.ok .li-ico { color: #2fae6b; } .link-item.act .li-ico { color: #d23b3b; } .link-item.fault .li-ico { color: #d06a3a; }
.li-name { font-size: 13px; font-weight: 500; } .li-st { font-size: 11px; }
.li-st.ok { color: #2fae6b; } .li-st.act { color: #d23b3b; } .li-st.fault { color: #d06a3a; }
.qie-fei { margin-top: 10px; border-top: 1px solid var(--border); padding-top: 8px; }
.qf-row { display: flex; gap: 8px; font-size: 12px; padding: 3px 0; }
.qf-row .k { color: var(--text-3); min-width: 60px; } .qf-row .v { color: var(--text-1); }
