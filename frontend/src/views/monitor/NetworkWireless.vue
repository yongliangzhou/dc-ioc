<template>
  <div>
    <div class="view-head">
      ...
      <span class="sub">{{ tl('AP管理') }} · {{ tl('信号覆盖') }}</span>
    </div>

    <div class="grid cols-4" v-if="s">
      <MetricCard metric-name="ap-total" :label="tl('设备总数')" :value="s.total" unit="台" quality="good" :online="true" />
      <MetricCard metric-name="ap-online" :label="tl('在线率')" :value="onlinePercent" unit="%" quality="good" :online="true" />
      <MetricCard metric-name="ap-ping" :label="tl('平均延迟')" :value="s.avgPingMs" unit="ms" quality="good" :online="true" />
      <MetricCard metric-name="ap-bw" :label="tl('平均带宽利用率')" :value="s.avgBwUtilization" unit="%" :quality="s.avgBwUtilization && s.avgBwUtilization > 70 ? 'uncertain' : 'good'" :online="true" :severity="s.avgBwUtilization && s.avgBwUtilization > 85 ? 'warn' : 'normal'" />
    </div>
