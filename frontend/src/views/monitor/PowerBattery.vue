<template>
  <div>
    <div class="view-head">
      ...
      <span class="sub">{{ tl('电池监控') }} {{ tl('·') }} {{ tl('电压与内阻') }}</span>
    </div>

    <div class="grid cols-4" v-if="s">
      <MetricCard metric-name="battery-total" :label="tl('设备总数')" :value="s.total" unit="台" quality="good" :online="true" />
      <MetricCard metric-name="battery-online" :label="tl('在线率')" :value="onlinePercent" unit="%" quality="good" :online="true" />
      <MetricCard metric-name="battery-load" :label="tl('平均负载率')" :value="s.avgLoadPercent" unit="%" quality="good" :online="true" />
      <MetricCard metric-name="battery-voltage" :label="tl('平均电压')" :value="s.avgVoltage" unit="V" quality="good" :online="true" />
    </div>
...
import { getPowerBattery, type PowerSystemSummary } from '@/api/power'
const { t: tl } = useI18n()
