<template>
  <div>
    <div class="view-head">
      ...
      <span class="sub">{{ tl('nav.networkSwitch') }} / {{ tl('nav.networkRouter') }} / {{ tl('nav.networkFirewall') }} / {{ tl('nav.networkWireless') }} {{ tl('·') }} {{ tl('总览 · 点击进入子系统') }}</span>
    </div>
    ...
    <div class="grid cols-2" v-if="overview">
      <router-link to="/monitor/net/switches" class="entry-card">
        <div class="card-head"><span class="ct">{{ tl('nav.networkSwitch') }}</span><span class="pill" :class="pillCls(overview.switchs)">{{ overview.switchs.online }}/{{ overview.switchs.total }} {{ tl('在线') }}</span></div>
        <div class="sub-label">{{ tl('核心层 · 汇聚层 · 接入层') }}</div>
        <div class="kvs"><span class="k">{{ tl('平均延迟') }}</span><span class="v">{{ ms(overview.switchs.avgPingMs) }}</span><span class="k">{{ tl('带宽利用率') }}</span><span class="v">{{ pct(overview.switchs.avgBwUtilization) }}</span></div>
      </router-link>
      <router-link to="/monitor/net/routers" class="entry-card">
        <div class="card-head"><span class="ct">{{ tl('nav.networkRouter') }}</span><span class="pill" :class="pillCls(overview.routers)">{{ overview.routers.online }}/{{ overview.routers.total }} {{ tl('在线') }}</span></div>
        <div class="sub-label">{{ tl('路由转发 · 协议状态') }}</div>
        <div class="kvs"><span class="k">{{ tl('平均延迟') }}</span><span class="v">{{ ms(overview.routers.avgPingMs) }}</span><span class="k">{{ tl('带宽利用率') }}</span><span class="v">{{ pct(overview.routers.avgBwUtilization) }}</span></div>
      </router-link>
    </div>
    <div class="grid cols-2" v-if="overview" style="margin-top:12px">
      <router-link to="/monitor/net/firewalls" class="entry-card">
        <div class="card-head"><span class="ct">{{ tl('nav.networkFirewall') }}</span><span class="pill" :class="pillCls(overview.firewalls)">{{ overview.firewalls.online }}/{{ overview.firewalls.total }} {{ tl('在线') }}</span></div>
        <div class="sub-label">{{ tl('安全策略 · 流量过滤') }}</div>
        <div class="kvs"><span class="k">{{ tl('平均延迟') }}</span><span class="v">{{ ms(overview.firewalls.avgPingMs) }}</span><span class="k">{{ tl('带宽利用率') }}</span><span class="v">{{ pct(overview.firewalls.avgBwUtilization) }}</span></div>
      </router-link>
      <router-link to="/monitor/net/wireless" class="entry-card">
        <div class="card-head"><span class="ct">{{ tl('nav.networkWireless') }}</span><span class="pill" :class="pillCls(overview.wireless)">{{ overview.wireless.online }}/{{ overview.wireless.total }} {{ tl('在线') }}</span></div>
        <div class="sub-label">{{ tl('AP管理 · 信号覆盖') }}</div>
        <div class="kvs"><span class="k">{{ tl('平均延迟') }}</span><span class="v">{{ ms(overview.wireless.avgPingMs) }}</span><span class="k">{{ tl('带宽利用率') }}</span><span class="v">{{ pct(overview.wireless.avgBwUtilization) }}</span></div>
      </router-link>
    </div>
...
.sub-label { font-size: 11px; color: var(--muted); margin-bottom: 8px; }
