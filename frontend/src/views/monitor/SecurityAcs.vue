<template>
  <div>
    <div class="view-head">
      ...
      <span class="sub">{{ tl('门禁管理') }} {{ tl('·') }} {{ tl('出入控制与授权') }}</span>
    </div>

    <div class="grid cols-4" v-if="s">
      <MetricCard metric-name="acs-total" :label="tl('设备总数')" :value="s.total" unit="台" quality="good" :online="true" />
      <MetricCard metric-name="acs-online" :label="tl('在线率')" :value="onlinePercent" unit="%" quality="good" :online="true" />
      <MetricCard metric-name="acs-events" :label="tl('今日事件')" :value="s.eventsToday" unit="起" quality="good" :online="true" />
      <MetricCard metric-name="acs-alerts" :label="tl('今日告警')" :value="s.alertsToday" unit="起" :quality="s.alertsToday ? 'uncertain' : 'good'" :online="true" :severity="s.alertsToday ? 'warn' : 'normal'" />
    </div>
...
import { getSecurityAcs, type SecuritySystemSummary } from '@/api/security'
const { t: tl } = useI18n()
