  ...
  <div class="page">
    ...
    <div class="kpi-row">
      ...
      <KpiCard
        ...
        :value="onlinePercent"
        unit="%"
        :trend="s.openAbnormal ? -1 : 1"
        :sub="s.openAbnormal + ' 樘门磁异常'"
      />
      <KpiCard title="今日刷卡" :value="s.todayEvents" unit="次" :sub="s.visitors + ' 名访客'" />
      <KpiCard
        title="拒绝/异常"
        :value="s.denied + s.openAbnormal"
        unit="次"
        :trend="s.denied + s.openAbnormal ? -1 : 1"
        :sub="s.denied + ' 次拒绝'"
      />
    </div>
    ...
    <div class="grid-main">
      ...
      <Panel class="plan-card">
        ...
        <div class="plan-wrap">
          <svg viewBox="0 0 600 380" class="plan-svg">
            ...
            <!-- 区域块 -->
            <g v-for="zone in zonesView" :key="zone.id">
            ...
            <!-- 门禁点位 -->
            <g
              v-for="pt in pointsView"
              :key="pt.id"
              @click="selectPoint(pt)"
              class="pt"
              :class="{ sel: selectedPoint && selectedPoint.id === pt.id }"
            >
          ...
          <div v-if="selectedPoint" class="pt-info">
            <div class="pt-info-h">
              <span class="pt-dot" :class="selectedPoint.state"></span>{{ selectedPoint.name }}
            </div>
            <div class="pt-info-b">
      ...
      <!-- 统一图形编辑入口: 对门禁平面图点位/区域做增删改 (覆盖层, 不影响接口数据) -->
      <GraphicEditDrawer
        v-model="editOpen"
      ...
      <div class="side-col">
        <Panel>
          ...
          <div v-if="selectedPoint" class="ctrl-sel">
            ...
            <div class="qc-actions">
              <button class="qc-btn open" @click="toggleDoor(selectedPoint, 'open')">
                远程开门
              </button>
              <button class="qc-btn lock" @click="toggleDoor(selectedPoint, 'lock')">
                远程锁门
              </button>
            </div>
            <span class="ctrl-state" :class="selectedPoint.state">{{
              stateText(selectedPoint.state)
            }}</span>
          </div>
        </Panel>
        ...
        <Panel class="alarm-card">
          <div class="card-head">
            ...
            ><span class="card-sub">{{ alarms.length }} 条</span>
          </div>
    ...
    <div class="grid-sub">
      ...
      <Panel>
        <div class="card-head">
          <span class="card-title">通行统计</span><span class="card-sub">各门禁点今日通行人次</span>
        </div>
        <TrendChart
          title=""
          :series="passSeries"
      ...
      <Panel>
        <div class="card-head">
          <span class="card-title">授权区域分级</span
          ><span class="card-sub">{{ s.areas.length }} 级区 · {{ s.total }} 门</span>
        </div>
        <div class="area-grid">
          <div class="area-block" v-for="a in s.areas" :key="a.id">
            ...
            <div class="area-auth">
              <span class="muted">认证方式</span><span class="auth-val">{{ a.auth }}</span>
            </div>
          </div>
        </div>
      </Panel>
...
const onlinePercent = computed(() =>
  s.value.total ? +((s.value.online / s.value.total) * 100).toFixed(1) : 0,
)
...
function derivePoints(areas: AcsAreaView[], events: AcsEventView[]): Point[] {
  ...
  const map: { zone: string; x: number; y: number }[] = [
    { zone: '动力机房', x: 75, y: 90 },
    { zone: '柴发/油罐区', x: 245, y: 90 },
    { zone: '网络核心', x: 420, y: 90 },
    { zone: '机房包间', x: 90, y: 270 },
    { zone: 'UPS 室', x: 300, y: 270 },
    { zone: '存储库房', x: 460, y: 270 },
  ]
  const list: Point[] = []
  map.forEach((m) => {
...
/* ───────── 统一图形编辑入口 (门禁平面图) ─────────
 * 场景覆盖层: 改名/改状态/改坐标/改参数 = 覆盖; 删除 = removed; 新增 = 自建点位。
 * 接口派生点位仍是默认值并持续刷新, 编辑只在其上叠加, 不污染数据源。 */
const graphicEditor = useGraphicEditor('security-acs-plan', { title: '门禁平面图' })
const editOpen = ref(false)
const hasGraphicEdits = computed(() => graphicEditor.hasOverrides.value)

/** Point ↔ GraphicNode 双向映射 (short/通行量/认证方式走 params, 未改动字段保留接口原值) */
const pointAdapter: NodeAdapter<Point> = {
  toNode: (p) => ({
    ...
    y: p.y,
    status: p.state,
    params: { short: p.short, pass: String(p.pass), auth: p.auth },
  }),
  fromNode: (g, base) => {
    const src: Point = base ?? {
      id: g.id,
      name: g.label || g.id,
      short: g.params?.short || g.id.slice(0, 3),
      zone: g.type || '未分区',
      x: g.x ?? 0,
      y: g.y ?? 0,
      state: (g.status as Point['state']) || 'normal',
      pass: Number(g.params?.pass ?? 0) || 0,
      auth: g.params?.auth || '刷卡',
    }
    const passRaw = g.params?.pass
    return {
      ...src,
      id: g.id,
      name: g.label || src.name,
      short: g.params?.short || src.short,
      zone: g.type || src.zone,
      x: g.x ?? src.x,
      y: g.y ?? src.y,
      state: (g.status || src.state) as Point['state'],
      pass:
        passRaw != null && passRaw !== '' && Number.isFinite(Number(passRaw))
          ? Number(passRaw)
          : src.pass,
      auth: g.params?.auth || src.auth,
    }
  },
}

const pointsView = computed(() => graphicEditor.apply(points.value, pointAdapter))
/** 硬编码区域块: 支持隐藏 (isHidden) 与改名 (labelOf) */
const zonesView = computed(() =>
  zones
    .filter((z) => !graphicEditor.isHidden(z.id))
    .map((z) => ({ ...z, short: graphicEditor.labelOf(z.id, z.short) })),
)
...
const alarms = computed(() => {
  ...
  if (s.value.openAbnormal)
    out.unshift({
      lv: 'crit',
      door: '门磁异常',
      desc: `${s.value.openAbnormal} 樘门磁异常开启 > 60s`,
    })
  if (s.value.denied)
    out.push({ lv: 'warn', door: '授权校验', desc: `${s.value.denied} 次刷卡被拒绝` })
  return out
})
...
function bulkOpen() {
  points.value.forEach((p) => {
    if (p.state !== 'intrude') p.state = 'open'
  })
}
function bulkLock() {
  points.value.forEach((p) => {
    if (p.state !== 'intrude' && p.state !== 'abnormal') p.state = 'normal'
  })
}
...
function lvCls(lv: string) {
  return lv === 'crit' || lv === 'r' ? 'r' : lv === 'warn' || lv === 'a' ? 'a' : 'g'
}
function lvText(lv: string) {
  return lv === 'crit' || lv === 'r' ? '严重' : lv === 'warn' || lv === 'a' ? '告警' : '信息'
}
...
async function load() {
  ...
  try {
    ...
    if (data && data.total) {
      ...
      markPartial('门禁点位通行量/授权方式由本地生成，后端接口未提供')
