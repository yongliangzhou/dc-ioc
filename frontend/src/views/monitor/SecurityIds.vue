<template>
  <div>
    <div class="view-head">
      ...
      <span class="sub">{{ tl('防入侵系统') }} {{ tl('·') }} {{ tl('周界与内部探测') }}</span>
    </div>

    <div class="grid cols-4" v-if="s">
      <MetricCard metric-name="ids-total" :label="tl('设备总数')" :value="s.total" unit="台" quality="good" :online="true" />
      <MetricCard metric-name="ids-online" :label="tl('在线率')" :value="onlinePercent" unit="%" quality="good" :online="true" />
      <MetricCard metric-name="ids-events" :label="tl('今日事件')" :value="s.eventsToday" unit="起" quality="good" :online="true" />
      <MetricCard metric-name="ids-alerts" :label="tl('今日告警')" :value="s.alertsToday" unit="起" :quality="s.alertsToday ? 'uncertain' : 'good'" :online="true" :severity="s.alertsToday ? 'warn' : 'normal'" />
    </div>
...
import { getSecurityIds, type SecuritySystemSummary } from '@/api/security'
const { t: tl } = useI18n()
