<template>
  <div>
    <div class="view-head">
      ...
      <span class="sub">{{ tl('记录所有写操作') }} (CRUD) {{ tl('与关键读操作') }}, {{ tl('由后端') }} core/audit {{ tl('中间件自动落库') }}</span>
    </div>
    ...
    <div class="card">
      <div class="flex gap8 wrap" style="align-items:center">
        ...
        <input v-model.trim="filters.username" class="ipt" :placeholder="tl('操作人')" style="width:140px" @keyup.enter="reload" />
        <input v-model.trim="filters.keyword" class="ipt" placeholder="关键字 (路径/详情)" style="width:200px" @keyup.enter="reload" />
        <button class="btn-sm primary" @click="reload">{{ tl('查询') }}</button>
        <button class="btn-sm" @click="resetFilters">{{ tl('重置') }}</button>
        <span class="muted" style="margin-left:auto;font-size:11px">{{ tl('共') }} {{ total }} {{ tl('条') }}</span>
      </div>
    </div>
      ...
      <table>
        ...
        <tbody>
          <tr v-for="row in rows" :key="row.id">
            ...
            <td><span class="tag b">{{ row.method }}</span></td>
            <td><span class="tag" :class="actionTag(row.action)">{{ row.action }}</span></td>
            <td>{{ row.resource }}</td>
            <td class="mono" style="font-size:11px;max-width:260px;overflow:hidden;text-overflow:ellipsis" :title="row.path">{{ row.path }}</td>
            <td>{{ row.username || '匿名' }}</td>
            <td class="mono" style="font-size:11px">{{ row.ip }}</td>
            <td><span class="tag" :class="statusTag(row.status_code)">{{ row.status_code }}</span></td>
            <td class="mono" style="font-size:10.5px;max-width:320px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" :title="row.detail">{{ row.detail }}</td>
          </tr>
          <tr v-if="!rows.length"><td colspan="9" class="muted" style="text-align:center;padding:20px">{{ loading ? '加载中…' : '暂无审计记录' }}</td></tr>
        </tbody>
      </table>
    </div>
    ...
    <div class="flex gap8" style="align-items:center;margin-top:12px">
      ...
      <span class="muted" style="font-size:12px">{{ tl('第') }} {{ page }} {{ tl('页') }} / {{ tl('共') }} {{ totalPages }} {{ tl('页') }}</span>
      <button class="btn-sm" :disabled="page >= totalPages" @click="page++; reload()">{{ tl('下一页') }}</button>
      <select v-model="pageSize" class="ipt" style="width:120px;margin-left:auto" @change="page = 1; reload()">
...
const filters = reactive<{ resource: string; action: string; username: string; keyword: string }>({
  resource: "", action: "", username: "", keyword: "",
});
