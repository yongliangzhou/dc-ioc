  ...
  <div class="page-wrap">
    ...
    <div class="grid cols-4">
      <div class="card" style="padding: 14px">
        ...
        <div class="text-2xl font-semibold" style="color: var(--txt-strong)">
          {{ rules.length }}
        </div>
      </div>
      <div class="card" style="padding: 14px">
        <div class="text-xs" style="color: var(--txt2)">{{ t.enabledCount }}</div>
        <div class="text-2xl font-semibold" style="color: var(--green)">
          {{ rules.filter((r) => r.enabled).length }}
        </div>
      </div>
      <div class="card" style="padding: 14px">
        <div class="text-xs" style="color: var(--txt2)">{{ t.highRisk }}</div>
        <div class="text-2xl font-semibold" style="color: var(--red)">
    ...
    <div class="flex gap-2 flex-wrap">
      ...
      <button class="btn-primary" @click="aiScan" :disabled="aiLoading" :aria-busy="aiLoading">
        <span
          v-if="aiLoading"
          class="spin-ic"
          aria-hidden="true"
          ...
          >◐</span
        >{{ aiLoading ? t.aiScanning : t.aiScanBtn }}
      </button>
    </div>
    ...
    <div class="grid cols-3">
      <div v-for="r in filtered" :key="r.id" class="card flex flex-col">
        ...
        <div class="mt-2 text-xs space-y-1" style="color: var(--txt2)">
          <div>
            {{ t.signal }}: <span class="mono">{{ r.signal }}</span>
          </div>
        ...
        <div class="mt-3 flex gap-2">
          <button class="btn-primary flex-1" @click="aiJudge(r)" :disabled="aiLoading">
            {{ t.aiJudge }}
          </button>
      ...
      <div class="modal">
        ...
        <div class="space-y">
          <div class="grid cols-2" style="gap: 12px">
            <div class="field">
              <span>{{ t.name }}</span
              ><input v-model="form.name" class="inp" :placeholder="t.namePlaceholder" />
            </div>
            <div class="field">
              <span>{{ t.category }}</span
              ><input v-model="form.category" class="inp" :placeholder="t.catPlaceholder" />
            </div>
          </div>
          <div class="grid cols-2" style="gap: 12px">
            <div class="field">
            ...
            <div class="field">
              <span>{{ t.signal }}</span
              ><input v-model="form.signal" class="inp mono" :placeholder="t.signalPlaceholder" />
            </div>
          </div>
          <div class="field">
            <span>{{ t.threshold }}</span
            ><input v-model="form.threshold" class="inp" :placeholder="t.thresholdPlaceholder" />
          </div>
          <div class="field">
            <span>{{ t.desc }}</span
            ><textarea
          ...
          <div class="field">
            <span>{{ t.action }}</span
            ><input v-model="form.action" class="inp" :placeholder="t.actionPlaceholder" />
          </div>
        </div>
...
const t = new Proxy({} as any, {
  get(_t, key) {
    const ns = (tm('powerAiHazards') || {}) as any
  ...
  return [
    {
      ...
      level: 'high',
      signal: 'breaker.temp',
      threshold: '> 70℃ 或 温升 > 40K',
      desc: '断路器触头接触电阻增大导致温升超标，存在熔焊/起火风险。',
      action: '立即安排红外复测，必要时减载并安排检修',
      enabled: true,
    },
    {
      id: 'hz2',
      ...
      category: 'power',
      level: 'high',
      signal: 'ups.battery.ir',
      threshold: '内阻 > 标称 1.5 倍',
      desc: '单体电池内阻持续上升，容量衰减，断电时后备时间不足。',
      action: '更换异常电池组并做容量核试验证',
      enabled: true,
    },
    {
      id: 'hz3',
      ...
      signal: 'busbar.thd',
      threshold: 'THD > 8%',
      desc: '非线性负载引起谐波放大，导致中性线过载与设备误动。',
      action: '加装滤波装置，核查非线性负载分布',
      enabled: true,
    },
    {
      id: 'hz4',
      ...
      category: 'power',
      level: 'mid',
      signal: 'feeder.imbalance',
      threshold: '不平衡度 > 15%',
      desc: '三相负荷分配不均，中性点偏移、变压器损耗增加。',
      action: '重新平衡各相负荷',
      enabled: false,
    },
    {
      ...
      category: 'power',
      level: 'low',
      signal: 'ground.resistance',
      threshold: '> 4Ω',
      desc: '接地网腐蚀或断裂，雷击/故障残压抬升威胁人身安全。',
      action: '雨后复测，必要时增补接地极',
      enabled: true,
    },
    {
      id: 'hz6',
      ...
      category: 'power',
      level: 'high',
      signal: 'transformer.oil',
      threshold: '油位低或顶层油温 > 85℃',
      desc: '冷却失效或内部故障前兆，存在爆燃风险。',
      action: '核查冷却系统，红外+油色谱分析',
      enabled: true,
    },
    {
      id: 'hz7',
    ...
    },
    {
      ...
      level: 'mid',
      signal: 'genset.load',
      threshold: '带载率 < 额定 80% 且 启动超时',
      desc: '长期空载导致启动困难，市电中断时无法按期接管。',
      action: '按月带载试机并核查燃油/液位',
      enabled: false,
    },
    {
      id: 'hz9',
      ...
      category: 'power',
      level: 'low',
      signal: 'pdu.outlet.temp',
      threshold: '> 55℃',
      desc: '插接头松动或过载，局部过热引燃风险。',
      action: '紧固端子、核查单回路负荷',
      enabled: true,
    },
    {
      ...
      category: 'power',
      level: 'high',
      signal: 'paralleling.circ',
      threshold: '环流 > 额定 5%',
      desc: '多机并联均流异常，单机过载老化加速。',
      action: '调整调差系数，核查同步信号',
      enabled: true,
    },
    {
      id: 'hz11',
    ...
    },
    {
      ...
      category: 'power',
      level: 'low',
      signal: 'rpp.voltage.dip',
      threshold: '瞬时跌落 > 10%',
      desc: '上游切换或启动冲击导致 IT 设备重启。',
      action: '核查切换时序，必要时加装 AVR',
      enabled: false,
    },
    {
      id: 'hz13',
      ...
      category: 'power',
      level: 'high',
      signal: 'dc.screen.voltage',
      threshold: '母线电压 < 下限',
      desc: '直流操作电源失压，保护/开关拒动。',
      action: '立即充电并核容，排查充电机',
      enabled: true,
    },
    {
      id: 'hz14',
    ...
    },
    {
      ...
      category: 'power',
      level: 'mid',
      signal: 'switchgear.pd',
      threshold: 'TEV > 20dB',
      desc: '绝缘缺陷早期表征，可能发展为击穿。',
      action: '超声+地电波复测，安排解体',
      enabled: true,
    },
    {
      id: 'hz17',
