<template>
  <div class="page-wrap">
          ...
          <table class="w-full">
            ...
            <tbody>
              <tr v-for="p in plans" :key="p.id">
                <td>
                  <span class="tag b">{{ p.type }}</span>
                </td>
                <td class="cell-strong">
                  {{ p.name }}
                  <span v-if="p.source === 'real'" class="pill g" style="margin-left: 4px">{{
                    t.suggest || '建议'
                  }}</span>
                </td>
                <td>{{ p.level || '—' }}</td>
                ...
                <td>
                  <span class="tag" :class="stateClass(p.state)">{{ p.state }}</span>
                </td>
                <td>
                  <span class="tag" :class="resultClass(p.result)">{{ p.result }}</span>
                </td>
                <td>
                  <div class="flex gap-1">
                    <button class="btn-sm btn-primary" @click="openPreview(p)">
                      {{ t.preview || '演练预演' }}
                    </button>
          ...
          <table class="w-full">
            ...
            <tbody>
              <tr v-for="r in records" :key="r.id">
                ...
                <td>
                  <span class="tag" :class="resultClass(r.result)">{{ r.result }}</span>
                </td>
                <td>{{ r.score }}</td>
      ...
      <div class="modal">
        ...
        <div class="space-y">
          <div class="grid cols-2" style="gap: 12px">
            <div class="field">
              <span>{{ t.plans }}</span
              ><input v-model="form.name" class="inp" placeholder="科目名称" />
            </div>
            <div class="field">
              <span>{{ t.type }}</span>
          ...
          <div class="grid grid-cols-3" style="gap: 12px">
            ...
            <div class="field">
              <span>{{ t.scope }}</span
              ><input v-model="form.scope" class="inp" placeholder="如 全园区供电" />
            </div>
            <div class="field">
              <span>{{ t.estDuration }}</span
              ><input v-model.number="form.duration" type="number" class="inp" placeholder="分钟" />
            </div>
          </div>
          <div class="grid cols-2" style="gap: 12px">
            <div class="field">
              <span>计划日期</span
              ><input v-model="form.date" class="inp" placeholder="YYYY-MM-DD" />
            </div>
            <div class="field">
              <span>状态</span>
          ...
          <div class="field">
            ...
            <div class="space-y-2">
              ...
              <button
                ...
                @click="form.steps.push({ title: '', minutes: 0, desc: '' })"
              >
                + {{ t.addStep || '添加步骤' }}
              </button>
            </div>
      ...
      <div class="modal" style="max-width: 480px">
        ...
        <div class="space-y">
          ...
          <div class="grid cols-2" style="gap: 12px">
            <div class="field">
              <span>{{ t.colDate }}</span
              ><input v-model="recForm.date" class="inp" placeholder="YYYY-MM-DD" />
            </div>
            <div class="field">
              <span>{{ t.colOwner }}</span
              ><input
                v-model.number="recForm.participants"
                type="number"
                class="inp"
                placeholder="人数"
              />
          ...
          <div class="grid cols-2" style="gap: 12px">
            <div class="field">
              <span>开始</span><input v-model="recForm.startAt" class="inp" placeholder="HH:mm" />
            </div>
            <div class="field">
              <span>结束</span><input v-model="recForm.endAt" class="inp" placeholder="HH:mm" />
            </div>
          </div>
          <div class="grid cols-2" style="gap: 12px">
            <div class="field">
          ...
          <div class="field">
            <span>{{ t.colNote }}</span
            ><input v-model="recForm.note" class="inp" />
          </div>
        </div>
        <div class="modal-actions">
          <button class="btn-ghost" @click="showRecord = false">{{ t.cancel || '取消' }}</button>
          <button class="btn-primary" @click="saveRecord" :disabled="saving">
            {{ saving ? '...' : t.save || '保存' }}
          </button>
        </div>
      </div>
    ...
    <div v-if="showPreview" class="modal-mask" @click.self="showPreview = false">
      <div class="modal" style="max-width: 680px">
        ...
        <div v-if="previewLoading" class="loading-box">
          ...
          <span>{{ t.analyzing || '分析中…' }}</span>
        </div>
        <div v-else-if="previewError" class="result bad">
          <span class="r-ic">!</span>{{ previewError }}
        </div>
        <div v-else-if="previewRes" class="space-y">
          <div class="grid cols-3">
            <div
              class="card"
              style="padding: 12px; text-align: center; border-color: rgba(255, 77, 94, 0.4)"
            >
              <div class="text-xl font-bold" style="color: var(--red)">
                {{ sevText(previewRes.summary?.severity) }}
              </div>
            ...
            <div
              class="card"
              style="padding: 12px; text-align: center; border-color: rgba(255, 176, 32, 0.4)"
            >
              <div class="text-xl font-bold" style="color: var(--amber)">
                {{ previewRes.summary?.affectedCount ?? '—' }}
              </div>
              <div class="text-xs mt-1" style="color: var(--txt2)">
                {{ t.affectedNodes || '受影响节点' }}
              </div>
            </div>
            <div
              class="card"
              style="padding: 12px; text-align: center; border-color: rgba(139, 92, 246, 0.4)"
            >
              <div class="text-xl font-bold" style="color: var(--blue)">
                {{ previewRes.summary?.criticalPaths ?? '—' }}
              </div>
              <div class="text-xs mt-1" style="color: var(--txt2)">
                {{ t.criticalPaths || '关键链路' }}
              </div>
            </div>
          </div>

          <div v-if="(previewRes.businesses || []).length" class="card" style="padding: 12px">
            <div class="text-xs mb-2" style="color: var(--txt2)">
              {{ t.affectedBiz || '受影响业务域' }}
            </div>
            <div class="space-y-1">
              <div
                v-for="b in previewRes.businesses"
                :key="b.business"
                class="flex items-center justify-between text-sm"
              >
                <span style="color: var(--txt)"
                  >{{ b.business }} <span style="color: var(--txt3)">(SLA {{ b.sla }})</span></span
                >
                <span
                  class="tag"
                  :class="b.severity === 'critical' ? 'r' : 'a'"
                  :title="sevText(b.severity)"
                  >{{ sevText(b.severity) }}</span
                >
              </div>
            </div>
          </div>
          ...
          <div v-if="(previewRes.mitigations || []).length" class="card" style="padding: 12px">
            <div class="text-xs mb-2" style="color: var(--txt2)">
              {{ t.suggestActions || '建议处置措施' }}
            </div>
            <div class="space-y-1">
              <div
                v-for="m in previewRes.mitigations.slice(0, 5)"
                :key="m.seq"
        ...
        <div class="modal-actions">
          ...
          <button class="btn-primary" @click="runPreview" :disabled="previewLoading">
            {{ t.previewRun }}
          </button>
        </div>
      </div>
    ...
    <div v-if="offline" class="result bad" style="margin-top: 8px">
      <span class="r-ic">!</span
      >{{ t.offlineMsg || '后端不可达, 已使用离线模拟数据 (刷新后端后重试)。' }}
    </div>
  </div>
</template>
...
function mapDrillToFaults(drill: DrillPlan, sources: FaultSourceNode[]): number[] {
  ...
  const picked = sources.filter((n) => {
    ...
    if (
      scope &&
      scope !== (n.roomCode || '') &&
      !(n.label || '').includes(scope) &&
      !(n.domain || '').toLowerCase().includes(scope.toLowerCase())
    )
      return false
    return true
  })
...
/** 严重级别本地化：后端返回 critical/high/medium/low 英文原值，直接渲染会漏出英文 */
function sevText(v?: unknown) {
  if (v === null || v === undefined || v === '') return '—'
  const raw = String(v)
  const map: Record<string, string> = {
    critical: t.sevCritical || '严重',
    high: t.sevHigh || '高',
    medium: t.sevMedium || '中',
    low: t.sevLow || '低',
  }
