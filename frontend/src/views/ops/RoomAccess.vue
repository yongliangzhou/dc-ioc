<template>
  <div class="page">
    ...
    <section class="kpi-row">
      <div class="kpi">
        <span class="kpi-v">{{ insideCount }}</span
        ><span class="kpi-l">{{ t.stillIn }}</span>
      </div>
      <div class="kpi">
        <span class="kpi-v">{{ items.length }}</span
        ><span class="kpi-l">{{ t.history }}</span>
      </div>
      <div class="kpi">
        <span class="kpi-v">{{ personCount }}</span
        ><span class="kpi-l">{{ t.person }}</span>
      </div>
      <div class="kpi">
        <span class="kpi-v">{{ itemCount }}</span
        ><span class="kpi-l">{{ t.item }}</span>
      </div>
    </section>
    ...
    <div class="list">
      <article v-for="r in filtered" :key="r.id" class="row-card" :class="{ inside: !r.outAt }">
        <div class="row-main">
          <div class="row-title">
            {{ r.name }}
            <span class="badge" :class="r.type === 'person' ? 'blue' : 'amber'">{{
              r.type === 'person' ? t.person : t.item
            }}</span>
            <span v-if="!r.outAt" class="badge green">{{ t.stillIn }}</span>
          </div>
          <div class="row-sub">
            <span class="muted">{{ r.org || '—' }}</span> ·
            <span class="muted">{{ t.area }}：{{ r.area || '—' }}</span> ·
        ...
        <div class="drawer-body">
          ...
          <label class="fld"
            ...
            ><input
              ...
              @blur="validate('name', form)"
          /></label>
          <label class="fld"
            ><span>{{ t.org }}</span
            ><input v-model="form.org" class="inp" :placeholder="t.orgPlaceholder"
          /></label>
          <label class="fld"
            ><span>{{ t.purpose }}</span
            ><input v-model="form.purpose" class="inp" :placeholder="t.purposePlaceholder"
          /></label>
          <label class="fld"
            ><span>{{ t.area }}</span
            ><input v-model="form.area" class="inp" :placeholder="t.areaPlaceholder"
          /></label>
          <label class="fld"
            ><span>{{ t.carriedBy }}</span
            ><input v-model="form.carriedBy" class="inp" :placeholder="t.carriedByPlaceholder"
          /></label>
          <label class="fld"
            ><span>{{ t.carryIn }}</span
            ><input v-model="form.carryIn" class="inp" :placeholder="t.carryPlaceholder"
          /></label>
          <label class="fld"
            ><span>{{ t.carryOut }}</span
            ><input v-model="form.carryOut" class="inp" :placeholder="t.carryPlaceholder"
          /></label>
          <label class="fld"
            ><span>{{ t.inAt }}</span
            ><input v-model="form.inAt" class="inp" type="datetime-local"
          /></label>
          <div v-if="touched.name && errors.name" class="err">{{ errors.name }}</div>
          <div class="drawer-actions">
            <button class="btn green" :disabled="saving" @click="save">{{ t.save }}</button
            ><button class="btn ghost" @click="editor = false">{{ t.cancel }}</button>
          </div>
        </div>
      </aside>
    </div>
...
function persist() {
  localStorage.setItem(LS, JSON.stringify(items.value))
}
...
const form = ref<Partial<AccessRecord>>({
  type: 'person',
  name: '',
  org: '',
  purpose: '',
...
function genId() {
  return 'RA-' + Date.now().toString(36)
}
function save() {
  if (!validateAll(form.value as unknown as Record<string, unknown>)) return
  const rec: AccessRecord = {
    id: genId(),
    type: (form.value.type as AType) || 'person',
    name: form.value.name!,
    org: form.value.org || '',
    purpose: form.value.purpose || '',
    area: form.value.area || '',
    carriedBy: form.value.carriedBy || '',
    carryIn: form.value.carryIn || '',
    carryOut: form.value.carryOut || '',
    inAt: (form.value.inAt || '').replace('T', ' '),
    outAt: '',
  }
...
function signOut(r: AccessRecord) {
  r.outAt = new Date().toISOString().replace('T', ' ').slice(0, 16)
  persist()
}
const showDel = ref(false)
const delTarget = ref<AccessRecord | null>(null)
function askRemove(r: AccessRecord) {
  delTarget.value = r
  showDel.value = true
}
function confirmRemove() {
  if (delTarget.value) {
    items.value = items.value.filter((x) => x.id !== delTarget.value!.id)
    persist()
  }
  showDel.value = false
