<template>
  <div class="page">
    ...
    <section class="kpi-row">
      <div class="kpi"><span class="kpi-v">{{ shares.length }}</span><span class="kpi-l">{{ t.sharedCount }}</span></div>
      <div class="kpi"><span class="kpi-v">{{ favs.length }}</span><span class="kpi-l">{{ t.favTotal }}</span></div>
      <div class="kpi"><span class="kpi-v">{{ cites.length }}</span><span class="kpi-l">{{ t.citeTotal }}</span></div>
      <div class="kpi"><span class="kpi-v">{{ items.length }}</span><span class="kpi-l">{{ t.mine }}</span></div>
    </section>
    ...
    <div v-else class="grid cards">
      <article v-for="it in filtered" :key="it.id" class="card" :class="{ shared: isShared(it.id) }">
        <div class="card-top">
          ...
          <button class="x" :title="t.delete" @click="remove(it)">×</button>
        </div>
        ...
        <div class="card-actions">
          <button class="btn sm" :class="isFav(it.id) ? 'ghost' : 'soft'" @click="toggleFav(it.id)">{{ isFav(it.id) ? '★ ' + t.unfavorite : '☆ ' + t.favorite }}</button>
          <button class="btn sm" :class="isShared(it.id) ? 'ghost' : 'soft'" @click="toggleShare(it)">{{ isShared(it.id) ? t.cancelShare : t.publish }}</button>
          <button v-if="it.reviewStatus === 'pending'" class="btn sm green" @click="openReview(it)">{{ t.review }}</button>
          <button class="btn sm" @click="openDetail(it)">{{ t.cite }} / {{ t.comment }}</button>
        </div>
        <div class="card-foot">
          <span>⭐ {{ ratingOf(it.id).toFixed(1) }}</span>
          <span>💬 {{ commentsOf(it.id).length }}</span>
          <span>🔗 {{ citeCountOf(it.id) }}</span>
        </div>
      </article>
    ...
    <div v-if="detail" class="mask" @click.self="detail = null">
      <aside class="drawer">
        ...
        <div class="drawer-body">
          <div class="row"><span>{{ t.status }}：</span><b :class="statusCls(detail.reviewStatus)">{{ reviewLabel(detail.reviewStatus) }}</b></div>
          <div class="row"><span>{{ t.author }}：</span><b>{{ detail.owner || '—' }}</b></div>
          <div class="row"><span>{{ t.createdAt }}：</span><b>{{ fmt(detail.createdAt) }}</b></div>
          <div v-if="detail.reviewer" class="row"><span>{{ t.reviewedBy }}：</span><b>{{ detail.reviewer }}</b></div>
          <p class="content">{{ detail.content }}</p>

          <div class="block">
            <div class="block-h">{{ t.rating }}</div>
            <div class="stars">
              <button v-for="n in 5" :key="n" class="star" :class="{ on: n <= Math.round(ratingOf(detail.id)) }" @click="rate(detail.id, n)">★</button>
              <span class="muted">{{ ratingOf(detail.id).toFixed(1) }}</span>
            </div>
          </div>
          ...
          <div class="block">
            ...
            <div v-if="citeCountOf(detail.id)" class="muted small">{{ t.citeCount }}：{{ citeCountOf(detail.id) }}</div>
          </div>
          ...
          <div class="block">
            ...
            <div v-if="commentsOf(detail.id).length" class="comment-list">
              <div v-for="(c, i) in commentsOf(detail.id)" :key="i" class="comment">
                <div class="comment-meta"><b>{{ c.user }}</b><span class="muted">{{ fmt(c.at) }}</span></div>
                <div class="comment-text">{{ c.text }}</div>
              </div>
            </div>
            <div v-else class="muted small">{{ t.noComment }}</div>
            <div class="cite-row">
              <input v-model="commentText" class="inp" :placeholder="t.commentPlaceholder" @keyup.enter="sendComment(detail.id)" />
    ...
    <div v-if="reviewing" class="mask" @click.self="reviewing = null">
      <aside class="drawer">
        <header class="drawer-head">
          ...
          <button class="x" @click="reviewing = null">×</button>
        </header>
        <div class="drawer-body">
          <p class="content">{{ reviewing.content }}</p>
          <div class="block">
            <div class="block-h">{{ t.reviewNote }}</div>
            <textarea v-model="reviewNote" class="area" :placeholder="t.reviewNotePlaceholder"></textarea>
          </div>
          <div class="drawer-actions">
            <button class="btn green" :disabled="reviewingLoading" @click="doReview('approved')">{{ t.approve }}</button>
            <button class="btn red" :disabled="reviewingLoading" @click="doReview('rejected')">{{ t.reject }}</button>
          </div>
        </div>
      </aside>
    </div>
...
// ---- localStorage 协作层 ----
const LS = { fav: 'kc_fav', share: 'kc_share', cite: 'kc_cite', rate: 'kc_rate', comment: 'kc_comment' }
function read<T>(k: string, def: T): T { try { return JSON.parse(localStorage.getItem(k) || JSON.stringify(def)) } catch { return def } }
...
const comments = ref<Record<number, { user: string; text: string; at: string }[]>>(read(LS.comment, {}))
...
const filtered = computed(() => {
  ...
  if (q) list = list.filter(it => (it.title + ' ' + (it.tags || []).join(' ') + ' ' + (it.owner || '')).toLowerCase().includes(q))
  switch (activeTab.value) {
    case 'pending': return list.filter(i => i.reviewStatus === 'pending')
    case 'approved': return list.filter(i => i.reviewStatus === 'approved')
    case 'rejected': return list.filter(i => i.reviewStatus === 'rejected')
    case 'shared': return list.filter(i => isShared(i.id))
    case 'fav': return list.filter(i => isFav(i.id))
    case 'review': return list.filter(i => i.reviewer)
  }
  return list
})

function isFav(id: number) { return favs.value.includes(id) }
function isShared(id: number) { return shares.value.some(s => s.id === id) }
function citeCountOf(id: number) { return cites.value.filter(c => c.id === id).length }
function ratingOf(id: number) { const a = rates.value[id]; return a && a.length ? a.reduce((s, n) => s + n, 0) / a.length : 0 }
function commentsOf(id: number) { return comments.value[id] || [] }
...
function rate(id: number, n: number) {
  ...
  a.push(n); rates.value = { ...rates.value, [id]: a }
...
async function doReview(status: 'approved' | 'rejected') {
  ...
  } finally { reviewingLoading.value = false }
}

function openDetail(it: KnowledgeItem) { detail.value = it }

function statusCls(s?: string) { return { pending: 'amber', approved: 'green', rejected: 'red' }[s || 'pending'] || 'gray' }
...
.inp { background: var(--panel-2); border: 1px solid var(--line); color: var(--text); border-radius: 8px; padding: 7px 10px; min-width: 220px; }
...
.kpi-v { font-size: 26px; font-weight: 800; color: var(--cyan); }
.kpi-l { font-size: 12px; color: var(--muted); margin-top: 4px; }
.tabs { display: flex; gap: 6px; flex-wrap: wrap; margin-bottom: 14px; }
.tab { background: var(--panel-2); border: 1px solid var(--line); color: var(--muted); padding: 6px 14px; border-radius: 999px; cursor: pointer; font-size: 13px; }
.tab.active { background: var(--cyan); color: #04121a; border-color: var(--cyan); font-weight: 600; }
...
.card.shared { border-color: var(--green); }
...
.chip { font-size: 11px; background: var(--panel-2); color: var(--muted); border-radius: 6px; padding: 2px 6px; }
.card-actions { display: flex; flex-wrap: wrap; gap: 6px; margin-top: 4px; }
.card-foot { display: flex; gap: 12px; font-size: 12px; color: var(--muted); border-top: 1px dashed var(--line); padding-top: 8px; }
.x { margin-left: auto; background: transparent; border: none; color: var(--muted); font-size: 16px; cursor: pointer; }
...
.drawer { width: 460px; max-width: 92vw; background: var(--panel); height: 100%; overflow-y: auto; padding: 18px 20px; }
...
.content { font-size: 13px; line-height: 1.7; color: var(--text); white-space: pre-wrap; background: var(--panel-2); border-radius: 10px; padding: 12px; margin: 10px 0; }
...
.star { background: none; border: none; font-size: 22px; color: var(--line); cursor: pointer; }
.star.on { color: #fbbf24; }
.cite-row { display: flex; gap: 8px; margin: 8px 0; }
.area { width: 100%; min-height: 90px; background: var(--panel-2); border: 1px solid var(--line); color: var(--text); border-radius: 8px; padding: 10px; resize: vertical; }
.comment-list { display: flex; flex-direction: column; gap: 8px; margin-bottom: 10px; }
.comment { background: var(--panel-2); border-radius: 8px; padding: 8px 10px; }
.comment-meta { display: flex; justify-content: space-between; font-size: 12px; }
