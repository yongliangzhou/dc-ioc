    ...
    <div class="view-head">
      <div class="vh-icon">
        <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.8">
          <path d="M9 7H6a3 3 0 0 0-3 3v7a3 3 0 0 0 3 3h12a3 3 0 0 0 3-3v-7a3 3 0 0 0-3-3h-3" stroke-linecap="round"/>
          <path d="M12 3v7M9 7l3-3 3 3" stroke-linecap="round" stroke-linejoin="round"/>
          <circle cx="9" cy="14" r="1.2"/><circle cx="15" cy="14" r="1.2"/>
        </svg>
      </div>
      <div>
        <h1>{{ t.title }}</h1>
      ...
      <div class="pill" :class="overallCls"><span class="dot" :class="overallDot"></span>{{ overallText }}</div>
    </div>

    <!-- Tabs -->
    <div class="tabs">
      <button v-for="tb in tabs" :key="tb.k"
        class="tab" :class="{ active: activeTab === tb.k }" @click="activeTab = tb.k">
        <span class="tab-ic">{{ tb.ic }}</span>{{ tb.label }}
      </button>
    </div>
      ...
      <div class="card progress-card">
        <div class="card-head">
          ...
          <button class="btn-primary" @click="verifyAll" :disabled="verifying">
            <span v-if="verifying" class="spin-ic">◐</span>{{ verifying ? t.verifying : t.verifyAll }}
          </button>
        </div>
        <div class="pv">
      ...
      <div class="space-y">
        <div v-for="grp in subsystemGroups" :key="grp.domain" class="card sub-card">
          <div class="sub-head">
            ...
            <div class="sub-rate">
              <span class="rate-num" :class="grp.online > 0 ? 'ok' : ''">{{ grp.online }}/{{ grp.total }}</span>
              ...
              <tbody>
                <tr v-for="d in grp.items" :key="d.device_id">
                  ...
                  <td>
                    ...
                    <span v-else-if="rtState[d.device_id] === 'fail'" class="tag r">✕ {{ t.notLanded }}</span>
                    <span v-else class="tag">—</span>
                  </td>
                  <td>
                    <span class="tag" :class="verifyTagCls(d.device_id)">{{ verifyLabel(d.device_id) }}</span>
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <div class="model-line">
            ...
            <span v-if="modelMap[grp.domain]" class="tag g">{{ modelMap[grp.domain].category_label }} ({{ modelMap[grp.domain].metrics.length }} {{ t.points }})</span>
        ...
        <div class="card">
          ...
          <div class="form-grid">
            ...
            <label class="fld">
              ...
              <input v-model.trim="cfg.token" type="password" class="inp" :placeholder="t.tokenPlaceholder" />
            </label>
            <label class="fld">
          ...
          <div class="act-row">
            ...
            <button class="btn-ghost" @click="testConn" :disabled="testing">{{ testing ? t.testing : t.testConn }}</button>
            <button class="btn-ghost" @click="pushSample" :disabled="testing">{{ t.pushSample }}</button>
          </div>

          <div v-if="connResult" class="result" :class="connResult.ok ? 'ok' : 'bad'">
            <span class="r-ic">{{ connResult.ok ? '✓' : '✕' }}</span>{{ connResult.msg }}
          </div>
        </div>
            ...
            <table v-if="syncLogs.length">
              ...
              <tbody>
                <tr v-for="l in syncLogs" :key="l.id">
                  ...
                  <td><span class="tag" :class="l.ok ? 'g' : 'r'">{{ l.ok ? t.sent : t.failed }}</span></td>
                </tr>
              </tbody>
            </table>
...
const t = new Proxy({} as any, {
  get(_t, key) {
    const ns = (raw('integrationHub') || {}) as any
...
const DOMAIN_LABELS: Record<string, string> = {
  video: '视频监控', access: '门禁', fire: '消防', power: '供配电',
  hvac: '制冷空调', network: '网络', security: '安防',
}
const subsystemGroups = computed(() => {
  if (!devList.value) return []
  const groups: Record<string, { domain: string; label: string; total: number; online: number; items: ExternalDeviceView[] }> = {}
  devList.value.items.forEach(d => {
    const domain = d.domain || 'other'
    if (!groups[domain]) groups[domain] = { domain, label: DOMAIN_LABELS[domain] || domain, total: 0, online: 0, items: [] }
    groups[domain].total++
...
const stats = computed(() => {
  ...
  return [
    { k: 'reg', label: t.registered, value: total, color: 'var(--txt-strong)', dot: 'b', bar: '100%' },
    { k: 'on', label: t.online, value: online, color: 'var(--green)', dot: 'g', bar: total ? (online / total * 100) + '%' : '0%' },
    { k: 'md', label: t.models, value: models.value.length, color: 'var(--blue)', dot: 'b', bar: '100%' },
    { k: 'mt', label: t.totalMetrics, value: metrics.toLocaleString(), color: 'var(--cyan)', dot: 'c', bar: '100%' },
  ]
})

// ---- 验证总览 ----
const passCount = computed(() => Object.values(verifyState.value).filter(s => s === 'pass').length)
const failCount = computed(() => Object.values(verifyState.value).filter(s => s === 'fail').length)
const pendingCount = computed(() => Object.values(verifyState.value).filter(s => !s || s === 'pending').length)
...
function testConn() {
  ...
  setTimeout(() => {
    ...
    connResult.value = ok
      ? { ok: true, msg: t.connOk }
      : { ok: false, msg: t.connBad }
...
<style scoped>
.ihub{display:flex; flex-direction:column; gap:14px}
.vh-icon{width:42px; height:42px; border-radius:11px; display:flex; align-items:center; justify-content:center;
  color:var(--cyan); background:linear-gradient(180deg,rgba(34,227,255,.16),rgba(34,227,255,.02)); border:1px solid rgba(34,227,255,.35); box-shadow:var(--glow)}
.sub{margin-top:2px}
/* tabs */
.tabs{display:flex; gap:6px; border-bottom:1px solid var(--line); padding-bottom:0}
.tab{display:inline-flex; align-items:center; gap:7px; padding:9px 16px; font-size:13px; font-weight:600;
  color:var(--txt2); background:transparent; border:none; border-bottom:2px solid transparent; cursor:pointer;
  border-radius:8px 8px 0 0; transition:all .15s}
.tab:hover{color:var(--txt)}
.tab.active{color:var(--cyan); border-bottom-color:var(--cyan); background:linear-gradient(180deg,rgba(34,227,255,.08),transparent)}
.tab-ic{font-size:12px; opacity:.85}
/* empty / loading */
.empty-state{display:flex; flex-direction:column; align-items:center; gap:12px; padding:60px 0; color:var(--txt2)}
.spinner{width:30px; height:30px; border-radius:50%; border:3px solid var(--line); border-top-color:var(--cyan); animation:spin 1s linear infinite}
@keyframes spin{to{transform:rotate(360deg)}}
/* stat cards */
.stat .cv{font-size:26px}
/* progress */
.progress-card .pv{margin-top:4px}
.pv-bar{height:10px; border-radius:6px; background:var(--track); overflow:hidden}
.pv-bar>i{display:block; height:100%; border-radius:6px; background:linear-gradient(90deg,var(--cyan),var(--blue)); transition:width .5s ease}
.pv-meta{display:flex; align-items:center; gap:8px; margin-top:10px; flex-wrap:wrap}
.pv-pct{margin-left:auto; font-size:15px; font-weight:800; color:var(--cyan); font-variant-numeric:tabular-nums}
/* subsystem */
...
.sub-id{display:flex; align-items:center; gap:9px}
.sub-name{font-size:14px; font-weight:700; color:var(--txt-strong)}
.sub-domain{font-size:11px; color:var(--txt3); font-family:"SF Mono",Consolas,monospace; padding:1px 7px; border:1px solid var(--line); border-radius:12px}
.sub-rate{display:flex; align-items:baseline; gap:6px}
.rate-num{font-size:16px; font-weight:800; font-variant-numeric:tabular-nums}
.rate-num.ok{color:var(--green)}
.rate-lab{font-size:11px; color:var(--txt2)}
.cell-strong{font-weight:600; color:var(--txt-strong)}
.dim{color:var(--txt2)}
.clip{max-width:200px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap}
.model-line{margin-top:12px; padding-top:10px; border-top:1px dashed var(--line); font-size:12px; color:var(--txt2); display:flex; align-items:center; gap:8px; flex-wrap:wrap}
.ml-k{color:var(--txt2)}
/* buttons */
.btn-primary{background:linear-gradient(90deg,var(--cyan),var(--blue)); color:#04121f; font-weight:700; font-size:12.5px;
  border:none; border-radius:8px; padding:8px 16px; cursor:pointer; transition:.15s; display:inline-flex; align-items:center; gap:6px}
...
.btn-ghost{background:var(--panel); color:var(--txt); font-size:12.5px; font-weight:600; border:1px solid var(--line); border-radius:8px; padding:8px 16px; cursor:pointer; transition:.15s}
.btn-ghost:hover{border-color:var(--cyan); color:var(--txt-strong)}
.btn-ghost:disabled{opacity:.5; cursor:not-allowed}
.spin-ic{display:inline-block; animation:spin 1s linear infinite}
/* north form */
.form-grid{display:grid; grid-template-columns:1fr 1fr; gap:12px}
.fld{display:flex; flex-direction:column; gap:6px}
.fl{font-size:11.5px; color:var(--txt2)}
.inp{background:var(--bg2); border:1px solid var(--line); border-radius:8px; padding:9px 11px; color:var(--txt); font-size:13px; outline:none; transition:.15s}
.inp:focus{border-color:var(--cyan); box-shadow:var(--glow)}
.act-row{display:flex; gap:9px; margin-top:16px; flex-wrap:wrap}
.result{margin-top:14px; padding:10px 12px; border-radius:9px; font-size:12.5px; display:flex; align-items:center; gap:8px}
.result.ok{background:rgba(43,212,122,.1); color:var(--green); border:1px solid rgba(43,212,122,.35)}
.result.bad{background:rgba(255,77,94,.1); color:var(--red); border:1px solid rgba(255,77,94,.35)}
.r-ic{font-weight:800}
/* log */
.log-scroll{max-height:340px; overflow-y:auto; margin-top:4px}
.log-empty{padding:40px 0; text-align:center; color:var(--txt3); font-size:13px}
