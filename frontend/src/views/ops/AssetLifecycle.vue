              ...
              <tr v-for="e in filtered" :key="e.id">
                ...
                <td>
                  <span class="tag" :class="warrantyTag(e)">
                    {{
                      e.warrantyMonths > 0
                        ? t.monthsLeft.replace('{n}', String(e.warrantyMonths))
                        : t.expired
                    }}
                  </span>
                </td>
          ...
          <div v-for="m in modelLib" :key="m.id" class="card flex flex-col">
            <div class="flex items-start justify-between">
              <div>
                ...
                <p class="text-xs" style="margin-top: 3px; color: var(--txt2)">
            ...
            <div
              class="grid grid-cols-1 gap-1 flex-1"
              style="margin-top: 10px; color: var(--txt2); font-size: 12px"
            >
          ...
          <div class="modal">
            ...
            <div class="space-y">
              <div class="grid cols-2" style="gap: 12px">
                <div class="field">
                  <span>{{ t.vendor }}</span
                  ><input v-model="form.vendor" class="inp" />
                </div>
                <div class="field">
                  <span>{{ t.model }}</span
                  ><input v-model="form.model" class="inp" />
                </div>
              </div>
              <div class="field">
              ...
              <div class="grid cols-2" style="gap: 12px">
                <div class="field">
                  <span>{{ t.spec }}</span
                  ><input v-model="form.spec" class="inp" :placeholder="t.specPlaceholder" />
                </div>
                <div class="field">
                  <span>{{ t.power }}</span
                  ><input v-model="form.power" class="inp" :placeholder="t.powerPlaceholder" />
                </div>
              </div>
              <div class="field">
                <span>{{ t.notes }}</span
...
const tabs = [
  { k: 'life', label: t.tabLife },
  { k: 'lib', label: t.tabLib },
]
...
const stages = [
  { key: 'procure', label: t.stProcure, cls: 'text-purple-600' },
  { key: 'stock', label: t.stStock, cls: 'text-gray-500' },
  { key: 'active', label: t.stActive, cls: 'text-green-600' },
  { key: 'expired', label: t.stExpired, cls: 'text-orange-600' },
  { key: 'retire', label: t.stRetire, cls: 'text-red-600' },
]
...
function stageCount(k: string) {
  return rows.value.filter((r) => r.stage === k).length
}
function stageLabel(k: string) {
  return (
    {
      procure: t.stProcure,
      stock: t.stStock,
  ...
  rows.value.filter(
    (r) =>
      ...
      (r.name.includes(kw.value) ||
        r.code.includes(kw.value) ||
        (r.model || '').includes(kw.value)),
  ),
)
