    ...
    <div class="cols">
        ...
        <div class="tpl-list">
          <div v-for="r in routes" :key="r.id" class="tpl-item">
            <div class="tpl-main">
              ...
              <span class="pill" :class="r.status === 'active' ? 'g' : 'a'">{{ r.frequency || '-' }}</span>
            </div>
            <div class="tpl-desc">{{ r.description || '-' }}</div>
            <div class="tpl-ops">
              <button class="link" v-bind="authState('write')" @click="openTpl(r)">{{ tl('编辑') }}</button>
              <button class="link" v-bind="authState('write')" @click="dispatchTask(r)">{{ tl('下发任务') }}</button>
      ...
      <section class="panel">
        <div class="panel-head">
          <h3>{{ tl('APP 巡检任务') }}</h3>
          <div class="seg">
            <button :class="{ on: taskFilter === 'all' }" @click="taskFilter = 'all'">{{ tl('全部') }}</button>
            <button :class="{ on: taskFilter === 'todo' }" @click="taskFilter = 'todo'">{{ tl('待接单') }}</button>
            <button :class="{ on: taskFilter === 'doing' }" @click="taskFilter = 'doing'">{{ tl('执行中') }}</button>
            <button :class="{ on: taskFilter === 'done' }" @click="taskFilter = 'done'">{{ tl('已完成') }}</button>
          </div>
        </div>
        <div class="task-list">
          <div v-for="t in filteredTasks" :key="t.id" class="task-item" :class="t.status">
            ...
            <div class="task-meta">{{ tl('巡检员') }}: {{ t.inspector || '-' }} · {{ tl('计划') }}: {{ t.planDate }}</div>
            <div class="task-ops" v-if="t.status === 'todo'">
              <button class="btn-sm" v-bind="authState('write')" @click="accept(t)">{{ tl('接单') }}</button>
            </div>
            <div class="task-ops" v-else-if="t.status === 'doing'">
              <button class="btn-sm primary" v-bind="authState('write')" @click="finish(t)">{{ tl('完成') }}</button>
            </div>
          </div>
    ...
    <transition name="slide">
      <div v-if="editing" class="drawer" @click.self="editing = null">
        <div class="drawer-card">
          ...
          <label class="fld"><span>{{ tl('模板名称') }}</span><input v-model="editing.name" /></label>
          <label class="fld"><span>{{ tl('编码') }}</span><input v-model="editing.code" placeholder="PATROL-xxx" /></label>
          <label class="fld"><span>{{ tl('频次') }}</span>
          ...
          <label class="fld"><span>{{ tl('描述') }}</span><textarea v-model="editing.description" rows="3" /></label>
          <label class="fld"><span>{{ tl('状态') }}</span>
          ...
          <div class="drawer-actions">
            <button class="btn-sm danger" v-if="editing.id" v-bind="authState('write')" @click="delTpl(editing)">{{ tl('删除') }}</button>
            <button class="btn-sm primary" v-bind="authState('write')" @click="saveTpl">{{ tl('保存') }}</button>
          </div>
        </div>
      </div>
    </transition>
...
function openTpl(r?: RouteView) {
  editing.value = r ? { ...r } : { id: 0, code: '', name: '', description: '', frequency: '每日', status: 'active' }
}
async function saveTpl() {
  if (!editing.value) return
  const payload = {
    code: editing.value.code || editing.value.name,
    freq: editing.value.frequency,
...
const filteredTasks = computed(() =>
  taskFilter.value === 'all' ? tasks.value : tasks.value.filter((t) => t.status === taskFilter.value),
)
...
<style scoped>
.insp-tpl { padding: 16px 20px 32px; }
...
.panel-head h3 { margin: 0; color: #cbd5e1; font-size: 15px; }
...
.seg button { color: #94a3b8; background: transparent; border: none; padding: 5px 12px; font-size: 12px; cursor: pointer; }
.seg button.on { background: var(--cyan); color: #06121f; font-weight: 700; }
.tpl-list, .task-list { display: flex; flex-direction: column; gap: 10px; }
.tpl-item { background: #1e293b; border: 1px solid var(--line); border-radius: 10px; padding: 12px; }
.tpl-main { display: flex; align-items: center; gap: 8px; }
.tpl-name { color: #e2e8f0; font-weight: 600; flex: 1; }
.pill { font-size: 11px; padding: 2px 8px; border-radius: 20px; }
.pill.g { background: rgba(34,197,94,.15); color: #4ade80; }
.pill.a { background: rgba(148,163,184,.15); color: #94a3b8; }
.tpl-desc { color: #64748b; font-size: 12px; margin: 6px 0; }
.tpl-ops { display: flex; gap: 12px; }
.link { background: none; border: none; color: var(--cyan); cursor: pointer; font-size: 13px; padding: 0; }
.link.danger { color: #f87171; }
.task-item { background: #1e293b; border: 1px solid var(--line); border-radius: 10px; padding: 12px; border-left: 3px solid #64748b; }
.task-item.todo { border-left-color: #f59e0b; }
.task-item.doing { border-left-color: #38bdf8; }
...
.task-status.todo { color: #f59e0b; } .task-status.doing { color: #38bdf8; } .task-status.done { color: #22c55e; }
.task-meta { color: #64748b; font-size: 12px; margin: 6px 0; }
.task-ops { margin-top: 8px; }
...
.drawer-card { width: 400px; max-width: 90vw; height: 100%; background: #0f172a; border-left: 1px solid #1e293b; padding: 22px; overflow-y: auto; }
.drawer-head { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 14px; }
.drawer-head h3 { margin: 0; color: #e2e8f0; font-size: 15px; }
.fld { display: flex; flex-direction: column; gap: 6px; margin: 12px 0; font-size: 13px; color: #94a3b8; }
...
.drawer-actions { display: flex; gap: 10px; justify-content: flex-end; margin-top: 16px; }
