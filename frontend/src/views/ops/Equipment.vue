<template>
  <div>
    <div class="view-head">
      ...
      <span class="sub">{{ tl('按域') }} / {{ tl('类别') }} / {{ tl('状态检索设备') }} {{ tl('·') }} {{ tl('遥测跳转') }}</span>
    ...
    <div class="card toolbar">
      ...
      <input v-model.trim="kw" class="ipt" :placeholder="tl('搜索编码 / 名称 / 厂商')" style="width:220px" @keyup.enter="reload()" />
    ...
    <div class="card scroll-x">
      <table>
        <thead>
          <tr>
            <th>{{ tl('设备编码') }}</th><th>{{ tl('名称') }}</th><th>{{ tl('业务域') }}</th><th>{{ tl('类别') }}</th>
            <th>{{ tl('厂商') }} / {{ tl('型号') }}</th><th>{{ tl('状态') }}</th><th>{{ tl('负载率') }}</th><th>{{ tl('运行小时') }}</th><th>{{ tl('操作') }}</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="e in items" :key="e.id" @click="openEq(e)" style="cursor:pointer">
            ...
            <td>{{ e.name }}</td>
            <td>{{ e.domain }}</td>
            <td><span class="tag b">{{ e.category }}</span></td>
            <td class="mono" style="font-size:11px">{{ e.vendor }} / {{ e.model }}</td>
            <td><span class="tag" :class="e.status === '运行' ? 'g' : e.status === '故障' ? 'r' : e.status === '维保' ? 'a' : 'o'">{{ e.status }}</span></td>
            <td class="mono">{{ e.load_pct }}%</td>
            <td class="mono muted">{{ e.run_hours.toLocaleString() }}</td>
            <td><button class="btn-sm" @click.stop="openEq(e)">{{ tl('遥测') }}</button></td>
          </tr>
          <tr v-if="!items.length"><td colspan="9" class="muted" style="text-align:center;padding:18px">{{ tl('无匹配设备') }}</td></tr>
        </tbody>
      </table>
    </div>
      ...
      <div class="tf-mask" v-if="sel" @click.self="sel = null">
        <div class="tf-modal" style="width:min(760px,96vw)">
          <div class="tf-head">
            <div>
              ...
              <div class="muted" style="font-size:11px;margin-top:3px">
                {{ sel.code }} {{ tl('·') }} {{ sel.domain }} / {{ sel.category }} {{ tl('·') }} {{ tl('厂商') }} {{ sel.vendor }} {{ tl('·') }} {{ tl('冗余') }} {{ sel.redundancy }}
              </div>
              <div
                v-if="sel.attrs?.location || sel.attrs?.ip || sel.attrs?.protocol || sel.attrs?.online === false"
                class="muted"
          ...
          <div class="tf-body">
            ...
            <div v-else class="muted" style="text-align:center;padding:22px">{{ eqLoading ? "加载中…" : "暂无数据" }}</div>
          </div>
          <div class="tf-foot">
            <span class="muted" style="font-size:11px">{{ tl('数据源') }} /api/equipment/{{ sel?.id }}/metrics {{ tl('·') }} {{ tl('近') }} 60min</span>
    ...
    <div class="footer-note">{{ tl('资产管理·统一设备台账') }} {{ tl('—') }} {{ tl('单一事实源') }} external_devices {{ tl('·') }} {{ tl('接入后端') }} /api/equipment</div>

    <!-- 分页 -->
    <div class="pager">
      <span class="muted">{{ tl('共') }} {{ total }} {{ tl('台') }} {{ tl('·') }} {{ tl('第') }} {{ page }} / {{ totalPages }} {{ tl('页') }}</span>
      ...
      <button class="btn-sm" :disabled="page <= 1" @click="goPage(page - 1)">{{ tl('上一页') }}</button>
      <button class="btn-sm" :disabled="page >= totalPages" @click="goPage(page + 1)">{{ tl('下一页') }}</button>
      <button class="btn-sm" :disabled="page >= totalPages" @click="goPage(totalPages)">{{ tl('末页') }} »</button>
    </div>
  </div>
</template>
...
function onPageSizeChange() { reload(true); }
...
const eqSeries = computed<Record<string, MetricHistoryPoint[]>>(() => {
  ...
  for (const k of m.metrics) {
    out[k] = (m.series[k] ?? []).map((p) => ({ ts: p.ts, value: p.value, quality: "good" as const }));
  }
  return out;
});
...
function goTelemetry() { router.push("/ops/telemetry"); }

let timer = 0;
onMounted(() => {
  loadMeta();
  reload();
  timer = window.setInterval(() => reload(false), Number(import.meta.env.VITE_REFRESH_INTERVAL ?? 15000));
});
