      ...
      <div class="modal" style="width: min(760px, 96vw)">
        <div class="modal-head">
          <div>
            <div class="modal-title">
              {{ mode === 'edit' ? `编辑设备 · ${editForm.device_id}` : '添加设备 · 基于物模型' }}
            </div>
            <div class="muted" style="font-size: 11px; margin-top: 3px">
              {{
                mode === 'edit'
                  ? `修改设备信息 · PUT /api/external/devices/${editForm.device_id}`
                  : '经契约端点 POST /api/external/device/register 注册 (先注册, 采集器方可上报)'
              }}
            </div>
          </div>
        ...
        <div class="modal-body">
          ...
          <div class="tabs" v-if="mode === 'add'">
            <button :class="['tab', addMode === 'single' && 'on']" @click="addMode = 'single'">
              {{ tl('单个添加') }}
            </button>
            <button :class="['tab', addMode === 'batch' && 'on']" @click="addMode = 'batch'">
              {{ tl('批量添加') }}
            </button>
          </div>
          ...
          <template v-if="mode === 'add' && addMode === 'single'">
            <div class="grid2">
              ...
              <div class="form-row">
                <label>{{ tl('厂商') }}</label
                ><input v-model.trim="form.vendor" class="ipt" /><span class="hint"
                  >{{ tl('最长') }} 64 {{ tl('字符') }}</span
                >
              </div>
              <div class="form-row">
                <label>{{ tl('业务域') }}</label
                ><input v-model.trim="form.domain" class="ipt" placeholder="hvac_source" />
              </div>
              <div class="form-row">
                <label>{{ tl('类别') }}</label
                ><input v-model.trim="form.category" class="ipt" placeholder="chiller" />
              </div>
              <div class="form-row">
                <label>{{ tl('位置') }}</label
                ><input v-model.trim="form.location" class="ipt" placeholder="R01" />
              </div>
              <div class="form-row">
                <label>{{ tl('协议') }}</label
                ><input v-model.trim="form.protocol" class="ipt" placeholder="modbus/snmp" />
              </div>
            </div>
            <div class="form-row">
              <label>{{ tl('标签') }} ({{ tl('逗号分隔') }})</label
              ><input v-model.trim="form.tags" class="ipt" placeholder="cooling,hvac" />
            </div>
          </template>
          ...
          <template v-else-if="mode === 'add' && addMode === 'batch'">
            <div class="grid2">
              ...
              <div class="form-row">
                <label>起始序号</label
                ><input v-model.number="batch.start" class="ipt" type="number" min="1" />
              </div>
              <div class="form-row">
                <label>数量 *</label
                ><input
              ...
              <div class="form-row">
                <label>IP 起始</label
                ><input v-model.number="batch.ipStart" class="ipt" type="number" min="1" />
              </div>
              <div class="form-row">
                <label>型号</label><input v-model.trim="form.model" class="ipt" />
              </div>
              <div class="form-row">
                <label>厂商</label><input v-model.trim="form.vendor" class="ipt" />
              </div>
              <div class="form-row">
                <label>业务域</label
                ><input v-model.trim="form.domain" class="ipt" placeholder="hvac_source" />
            ...
            <div v-if="batchPreview.length" class="preview">
              <div class="muted" style="margin: 10px 0 6px">
                将生成 {{ batchPreview.length }} 台 (预览前 50 台)：
              </div>
              <table>
                <thead>
                  <tr>
          ...
          <template v-else-if="mode === 'edit'">
            <div class="grid2">
              <div class="form-row">
                <label>设备 ID</label><input disabled :value="editForm.device_id" class="ipt" />
              </div>
              <div class="form-row">
              ...
              <div class="form-row">
                <label>业务域</label><input v-model.trim="editForm.domain" class="ipt" />
              </div>
              <div class="form-row">
                <label>类别</label><input v-model.trim="editForm.category" class="ipt" />
              </div>
              <div class="form-row">
                <label>位置</label><input v-model.trim="editForm.location" class="ipt" />
              </div>
              <div class="form-row">
                <label>协议</label><input v-model.trim="editForm.protocol" class="ipt" />
              </div>
            </div>
            <div class="form-row">
              <label>标签 (逗号分隔)</label><input v-model.trim="editForm.tags" class="ipt" />
            </div>
            <div v-if="editThingModel" class="thing-model-info">
              <div class="section-title" style="margin-top: 14px">
                传感器 / 测点模板 ({{ editThingModel.category_label }})
              </div>
              <table>
                <thead>
                  <tr>
                ...
                <tbody>
                  <tr v-for="m in editThingModel.metrics" :key="m.metric_name">
              ...
              <span class="muted" style="font-size: 10px"
                >以上为
                {{ editThingModel.category }} 类别的标准测点模板，实际上报的测点由采集器决定</span
              >
            </div>
          </template>
...
function resetForms() {
  ...
  form.protocol = ''
  form.tags = ''
  batch.prefix = 'MOCK-CHILLER-'
  batch.start = 1
  batch.count = 5
  batch.ipPrefix = '10.30.0.'
  batch.ipStart = 1
  applyModel()
  if (props.device) {
    editForm.device_id = props.device.device_id
    editForm.name = props.device.name || ''
    editForm.ip = props.device.ip
    editForm.sn = props.device.sn
    editForm.model = props.device.model
    editForm.vendor = props.device.vendor || ''
    editForm.domain = props.device.domain || ''
    editForm.category = props.device.category || ''
    editForm.location = props.device.location || ''
    editForm.protocol = props.device.protocol || ''
    editForm.tags = (props.device.tags || []).join(',')
  }
}

watch(
  () => [props.open, props.mode, props.device],
  () => {
    if (props.open) resetForms()
  },
  { immediate: true },
)
  ...
  if (addMode.value === 'single') {
    ...
    const payloads: ExternalDevice[] = batchPreview.value.map((p, i) => {
      ...
      return {
        ...
        domain: form.domain || undefined,
        category: form.category || undefined,
        location: form.location || `R${(i % 3) + 1}`,
        protocol: form.protocol || undefined,
        tags: parseTags(),
      }
    })
