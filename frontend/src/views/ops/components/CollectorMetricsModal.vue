      ...
      <div class="modal" role="dialog" aria-modal="true">
        <div class="modal-head">
          <div>
            ...
            <div class="muted" style="font-size:11px;margin-top:3px">
              {{ tl('最近') }} {{ metrics.length }} {{ tl('条上报') }} {{ tl('·') }} {{ tl('数据源') }} /api/external/devices/{{ device.device_id }}/metrics
            </div>
          </div>
        ...
        <div class="modal-body scroll-x">
          <table v-if="metrics.length">
            <thead>
              <tr><th>{{ tl('测点') }} ({{ tl('语义') }})</th><th>{{ tl('数值') }}</th><th>{{ tl('单位') }}</th><th>{{ tl('质量') }}</th><th>{{ tl('采样时间') }}</th><th>{{ tl('接收时间') }}</th></tr>
            </thead>
            <tbody>
              <tr v-for="(m, i) in metrics" :key="i">
                ...
                <td>{{ m.unit || "—" }}</td>
                <td><span class="tag" :class="qTag(m.quality)">{{ m.quality }}</span></td>
                <td class="mono">{{ fmt(m.ts) }}</td>
                <td class="mono">{{ fmt(m.received_at) }}</td>
              </tr>
            </tbody>
          </table>
        ...
        <div class="modal-foot">
          <span class="muted" style="font-size:11px">{{ tl('由采集器每') }} 5s {{ tl('经契约端点推送') }} {{ tl('·') }} {{ tl('质量码') }} good / uncertain / bad</span>
...
function mLab(name: string): string {
  const map: Record<string, string> = {
    supply_temp: "送风温度", return_temp: "回风温度", inlet_temp: "进风温度", outlet_temp: "出风温度",
    humidity: "湿度", power_kw: "功耗", cpu_usage: "CPU 使用率", temp: "温度",
  };
...
.modal { width: min(900px, 96vw); max-height: 86vh; display: flex; flex-direction: column; background: linear-gradient(180deg, var(--panel), var(--bg2)); border: 1px solid var(--line); border-radius: 14px; box-shadow: 0 20px 60px rgba(0, 0, 0, .5); overflow: hidden; }
