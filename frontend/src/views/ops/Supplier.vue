  ...
  <div class="page-wrap">
    ...
    <div class="grid cols-4">
      ...
      <div class="card" style="padding: 14px">
        ...
        <div class="text-2xl font-semibold" style="color: var(--blue)">
          {{ avgScore
          }}<span style="font-size: 10px; color: var(--txt3); margin-left: 2px">{{
            t.scoreScale
          }}</span>
        </div>
      </div>
      <div class="card" style="padding: 14px">
        ...
        <div class="text-2xl font-semibold" style="color: var(--green)">
          {{ suppliers.filter((s) => s.score >= 85).length }}
        </div>
        <div style="color: var(--txt3); font-size: 10px; margin-top: 4px">
          {{ t.excellentHint }}
        </div>
      </div>
      <div class="card" style="padding: 14px">
        ...
        <div style="color: var(--txt3); font-size: 10px; margin-top: 4px">{{ t.improveHint }}</div>
      </div>
    </div>
      ...
      <div v-for="s in suppliers" :key="s.id" class="card flex flex-col">
        <div class="flex items-start justify-between">
          <div>
            ...
            <p class="text-xs mt-1" style="color: var(--txt2)">
              {{ t.category }}: {{ s.category }}
            </p>
          </div>
          <div class="text-right">
            <div class="text-2xl font-bold" :style="{ color: scoreColor(s.score) }">
              {{ s.score }}
            </div>
        ...
        <div class="mt-3 space-y-1">
          ...
          >
            ...
            <div
              class="flex-1 pbar"
              role="progressbar"
              :aria-valuenow="s.detail[dim.k] || 0"
              aria-valuemin="0"
              aria-valuemax="100"
              :aria-label="t['dim_' + dim.k] || dim.k"
            >
              <i :style="{ width: (s.detail[dim.k] || 0) + '%' }"></i>
            </div>
        ...
        <div class="space-y">
          <div class="grid cols-2" style="gap: 12px">
            ...
            <div class="field">
              ...
              ><input v-model="form.category" class="inp" :placeholder="t.categoryPlaceholder" />
            </div>
          </div>
          <div class="grid cols-2" style="gap: 12px">
            <div class="field">
              <span>{{ t.contact }}</span
              ><input v-model="form.contact" class="inp" />
            </div>
            <div class="field">
              <span>{{ t.phone }}</span
          ...
          <div class="grid cols-2" style="gap: 12px">
            <div class="field">
              ...
              ><input v-model="form.contractNo" class="inp" />
            </div>
            <div class="field">
              <span>{{ t.validTo }}</span
              ><input v-model="form.validTo" type="date" class="inp" />
            </div>
          </div>
          <div class="field">
            <span>{{ t.sla }}</span
            ><input v-model="form.sla" class="inp" :placeholder="t.slaPlaceholder" />
          </div>
          <div v-if="editing">
            <label class="block text-xs" style="margin-bottom: 8px; color: var(--txt2)">{{
              t.rateDims
            }}</label>
            <div class="space-y-2">
              <div
              ...
              >
                ...
                <input
                  ...
                  min="0"
                  max="100"
                  v-model.number="form.detail[dim.k]"
                  class="flex-1 inp"
                  style="padding: 0"
                />
                <span style="width: 32px; text-align: right; font-size: 12px">{{
                  form.detail[dim.k]
                }}</span>
              </div>
            </div>
        ...
        <div class="space-y-2">
          ...
          >
            ...
            <input
              ...
              min="0"
              max="100"
              v-model.number="rateDetail[dim.k]"
              class="flex-1 inp"
              style="padding: 0"
            />
            <span style="width: 32px; text-align: right; font-size: 12px">{{
...
function blank(): Supplier {
  return {
    ...
    sla: '',
    score: 0,
    detail: { quality: 80, response: 80, price: 80, sla: 80, cooperation: 80 },
  }
}
  ...
  return [
    {
      ...
      contact: '王工',
      phone: '13800000001',
      contractNo: 'HT-2025-PW-01',
      validTo: '2026-12-31',
      sla: '故障 4h 到场',
      score: 92,
      detail: { quality: 95, response: 90, price: 88, sla: 94, cooperation: 91 },
    },
    {
      id: 'sp2',
      name: '云网通信',
      ...
      contact: '李经理',
      phone: '13800000002',
      contractNo: 'HT-2025-NW-03',
      validTo: '2026-06-30',
      sla: '故障 2h 响应',
      score: 81,
      detail: { quality: 82, response: 85, price: 78, sla: 83, cooperation: 77 },
    },
    {
      id: 'sp3',
      name: '安盾消防',
      ...
      contact: '赵工',
      phone: '13800000003',
      contractNo: 'HT-2025-FR-02',
      validTo: '2025-12-31',
      sla: '月度巡检',
      score: 68,
      detail: { quality: 70, response: 65, price: 72, sla: 66, cooperation: 67 },
    },
    {
      id: 'sp4',
      name: '数擎存储',
      ...
      contact: '陈工',
      phone: '13800000004',
      contractNo: 'HT-2025-ST-05',
      validTo: '2027-03-31',
      sla: '7x24 支持',
      score: 88,
      detail: { quality: 90, response: 87, price: 84, sla: 89, cooperation: 90 },
    },
  ]
}
