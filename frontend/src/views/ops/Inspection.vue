<template>
  <div>
    ...
    <div class="grid cols-5" v-if="stats">
      <MetricCard metric-name="inspect-routes" :label="tl('巡检路线')" :value="stats.totalRoutes" unit="" quality="good" :online="true" />
      <MetricCard metric-name="inspect-today" :label="tl('今日记录')" :value="stats.todayRecords" unit="条" quality="good" :online="true" />
      <MetricCard metric-name="inspect-complete" :label="tl('完成率')" :value="Number(stats.completionRate.toFixed(1))" unit="%" :quality="stats.completionRate > 80 ? 'good' : 'uncertain'" :online="true" :severity="stats.completionRate < 60 ? 'warn' : 'normal'" />
      <MetricCard metric-name="inspect-pass" :label="tl('合格率')" :value="Number(stats.passRate.toFixed(1))" unit="%" :quality="stats.passRate > 80 ? 'good' : 'uncertain'" :online="true" :severity="stats.passRate < 60 ? 'warn' : 'normal'" />
      <MetricCard metric-name="inspect-fail" :label="tl('不合格')" :value="stats.failRecords" unit="条" :quality="stats.failRecords ? 'bad' : 'good'" :online="true" :severity="stats.failRecords ? 'crit' : 'normal'" />
    </div>
      ...
      <div class="card">
        ...
        <div class="route-list" v-if="routes.length">
          <div v-for="r in routes" :key="r.id" class="route-row"
            ...
            <span class="pill-tag" :class="r.status === 'active' ? 'g' : 'm'">{{ r.status === 'active' ? tl('已启用') : tl('已停用') }}</span>
          </div>
        </div>
        ...
        <div class="record-table" v-if="records.length">
          ...
          <div v-for="rec in records" :key="rec.id" class="tbl-row" @click="openRecord(rec)">
            ...
            <span class="col w-status">
              <span class="pill-tag" :class="rec.status === 'completed' ? 'g' : rec.status === 'in_progress' ? 'a' : 'm'">
                {{ rec.status === 'completed' ? tl('已完成') : rec.status === 'in_progress' ? tl('进行中') : tl('待开始') }}
              </span>
            </span>
            <span class="col w-result">
              <span class="pill-tag" :class="rec.result === 'pass' ? 'g' : rec.result === 'fail' ? 'r' : rec.result ? 'a' : 'm'">
                {{ rec.result === 'pass' ? tl('合格') : rec.result === 'fail' ? tl('不合格') : rec.result === 'partial' ? tl('部分合格') : '-' }}
              </span>
            </span>
            <span class="col w-items muted">{{ rec.itemPassed }}/{{ rec.itemTotal }} {{ tl('通过') }}</span>
          </div>
        </div>
    ...
    <div class="modal-overlay" v-if="detailRecord" @click.self="detailRecord = null">
      <div class="modal-card">
        ...
        <div class="modal-meta">
          ...
          <span class="pill-tag" :class="detailRecord.result === 'pass' ? 'g' : 'r'">{{ detailRecord.result || '-' }}</span>
        </div>
        <div class="item-table" v-if="items.length">
          ...
          <div v-for="it in items" :key="it.id" class="tbl-row">
            ...
            <span class="col w-item-result">
              <span class="pill-tag" :class="it.result === 'pass' ? 'g' : it.result === 'fail' ? 'r' : 'a'">{{ it.result || '-' }}</span>
            </span>
    ...
    <!-- 加载 / 错误 -->
    <div class="card" v-if="!routes.length && !error">
...
import {
  getInspectionRoutes, getInspectionRecords, getInspectionStats, getInspectionItems,
  type RouteView, type RecordView, type ItemView, type InspectionStats
} from '@/api/inspection'
const { t: tl } = useI18n()
...
.route-list, .record-table, .item-table { max-height: 360px; overflow-y: auto; }
.route-row {
  display: flex; align-items: center; gap: 8px; padding: 10px 8px;
  cursor: pointer; border-radius: 6px; border-bottom: 1px solid var(--border-light, rgba(255,255,255,0.04));
  font-size: 13px;
}
.route-row:hover { background: var(--bg2); }
.route-row.active { background: rgba(64,150,255,0.08); border-left: 3px solid var(--blue); }
.r-name { font-weight: 500; flex: 1; }
.r-code { color: var(--muted); font-size: 11px; }
...
.pill-tag.m { background: rgba(255,255,255,0.06); color: var(--muted); }

.tbl-head, .tbl-row { display: flex; align-items: center; padding: 9px 8px; font-size: 12.5px; }
.tbl-head { font-weight: 600; color: var(--muted); border-bottom: 2px solid var(--border); font-size: 12px; }
.tbl-row { border-bottom: 1px solid var(--border-light, rgba(255,255,255,0.04)); cursor: pointer; }
.tbl-row:hover { background: var(--bg2); }
.col { flex-shrink: 0; }
.w-inspector { width: 80px; }
.w-time { width: 160px; }
.w-status { width: 80px; }
.w-result { width: 80px; }
.w-items { width: 70px; text-align: right; }
.w-device { width: 90px; }
.w-item { width: 140px; flex: 1; }
.w-chk { width: 50px; }
.w-item-result { width: 70px; }
.w-remark { width: 120px; }
...
.modal-card {
  background: var(--card-bg); border-radius: 12px; padding: 24px; width: 700px; max-height: 80vh;
  overflow-y: auto; box-shadow: 0 8px 32px rgba(0,0,0,0.4);
}
.modal-meta { display: flex; align-items: center; gap: 8px; margin: 8px 0 16px; font-size: 12px; color: var(--muted); }
.btn-close { margin-top: 16px; padding: 6px 20px; border-radius: 6px; border: 1px solid var(--border); background: var(--bg2); color: var(--text); cursor: pointer; }
