  ...
  <div class="dc-manage">
    <div class="view-head">
      ...
      <div class="head-actions">
        <button class="btn-sm" @click="page.reload" :disabled="busy">
          {{ tl('datacenter.refresh') }}
        </button>
        <button class="btn-sm" @click="openOpLog" :disabled="busy">
          {{ tl('datacenter.opLog') }}
        </button>
        <button
          class="btn-sm danger"
        ...
        >
          {{ tl('datacenter.batchDelete') }} ({{ selectedIds.length }})
        </button>
    ...
    <div class="kpis">
      <div class="kpi">
        <span class="kv">{{ kpis.total }}</span
        ><span class="kl">{{ tl('datacenter.kpiTotal') }}</span>
      </div>
      <div class="kpi">
        <span class="kv">{{ kpis.enabled }}</span
        ><span class="kl">{{ tl('datacenter.enabled') }}</span>
      </div>
      <div class="kpi">
        <span class="kv">{{ kpis.power.toFixed(1) }}</span
        ><span class="kl">{{ tl('datacenter.kpiPower') }} (MW)</span>
      </div>
      <div class="kpi">
        <span class="kv">{{ kpis.rack }}</span
        ><span class="kl">{{ tl('datacenter.kpiRack') }}</span>
      </div>
    </div>
    ...
    <div class="toolbar" v-if="canAdmin">
      <label class="chk"
        ...
        {{ tl('datacenter.selectAll') }}</label
      >
      <span class="sel-info">{{ tl('datacenter.selected', { n: selectedIds.length }) }}</span>
      <button class="btn-xs" v-if="selectedIds.length" @click="clearSel">
        {{ tl('datacenter.clearSel') }}
      </button>
    </div>
    ...
    <AsyncSection :page="page" @retry="page.reload">
      <div class="cards">
        <div
          ...
          :key="d.id"
          class="card"
          :class="{ cur: d.isCurrent, sel: selectedIds.includes(d.id) }"
        >
          <div class="card-top">
            <label v-if="canAdmin" class="chk card-chk"
              ><input
                type="checkbox"
            ...
            <span v-else class="tag" :class="statusClass(d.status)">{{
          ...
          <div class="c-metrics">
            <div>
              <span>{{ d.powerCapacityMw.toFixed(1) }}</span
              ><label>{{ tl('datacenter.kpiPower') }}</label>
            </div>
            <div>
              <span>{{ d.coolingCapacityMw.toFixed(1) }}</span
              ><label>{{ tl('datacenter.kpiCooling') }}</label>
            </div>
            <div>
              <span>{{ d.rackCapacity }}</span
              ><label>{{ tl('datacenter.kpiRack') }}</label>
            </div>
            <div>
              <span>{{ d.capacityKw }}</span
              ><label>{{ tl('datacenter.capKw') }}</label>
            </div>
          </div>
          <div class="c-desc">{{ d.description || '—' }}</div>
          <div class="c-actions">
            <button v-if="!d.isCurrent" class="btn-sm" @click="setCurrent(d)" :disabled="!canAdmin">
              {{ tl('datacenter.setCurrent') }}
            </button>
            <button class="btn-sm" @click="toggleStatus(d)" :disabled="!canAdmin">
              {{
                d.status === 'disabled' || d.status === '下线'
                  ? tl('datacenter.enable')
                  : tl('datacenter.disable')
              }}
            </button>
            <button class="btn-sm" @click="openServices(d)" :disabled="busy">
              {{ tl('datacenter.services') }}
            </button>
            <button class="btn-sm" @click="openEdit(d)" :disabled="!canAdmin">
              {{ tl('common.edit') }}
            </button>
            <button class="btn-sm danger" @click="remove(d)" :disabled="!canAdmin || d.isCurrent">
              {{ tl('datacenter.delete') }}
            </button>
            <router-link class="btn-sm" :to="{ path: '/ops/datacenter/compare' }">{{
              tl('datacenter.compare')
            }}</router-link>
          </div>
        </div>
      </div>
    ...
    <div class="drawer-mask" v-if="drawer" @click.self="drawer = false">
      <div class="drawer">
        ...
        <div class="form">
          <label
            >{{ tl('datacenter.code')
            }}<input v-model.trim="form.code" class="ipt" :disabled="!!editId"
          /></label>
          <label>{{ tl('datacenter.name') }}<input v-model.trim="form.name" class="ipt" /></label>
          <div class="row">
            <label
              >{{ tl('datacenter.region') }}<input v-model.trim="form.region" class="ipt"
            /></label>
            <label
              >{{ tl('datacenter.status') }}>
          ...
          <label
            >{{ tl('datacenter.address') }}<input v-model.trim="form.address" class="ipt"
          /></label>
          <div class="row">
            <label
              >{{ tl('datacenter.powerMw')
              }}<input v-model.number="form.powerCapacityMw" class="ipt" type="number"
            /></label>
            <label
              >{{ tl('datacenter.coolingMw')
              }}<input v-model.number="form.coolingCapacityMw" class="ipt" type="number"
            /></label>
          </div>
          <div class="row">
            <label
              >{{ tl('datacenter.rackCap')
              }}<input v-model.number="form.rackCapacity" class="ipt" type="number"
            /></label>
            <label
              >{{ tl('datacenter.capKw')
              }}<input v-model.number="form.capacityKw" class="ipt" type="number"
            /></label>
          </div>
          <label
            >{{ tl('datacenter.rooms')
            }}<input v-model.number="form.rooms" class="ipt" type="number"
          /></label>
          <label
            >{{ tl('datacenter.desc') }}><textarea
      ...
      <div class="drawer wide">
        ...
        <div class="log-list">
          ...
          <div v-for="r in opLogs" :key="r.id" class="log-row">
            ...
            <span
              ><span class="tag sm" :class="actionClass(r.action)">{{
    ...
    <div class="drawer-mask" v-if="svcOpen" @click.self="svcOpen = false">
      <div class="drawer">
        ...
        <div class="svc-summary" v-if="services">
          <div>
            <b>{{ services.totalDevices }}</b
            ><label>{{ tl('datacenter.serviceTotalDevices') }}</label>
          </div>
          <div>
            <b class="ok">{{ services.onlineDevices }}</b
            ><label>{{ tl('datacenter.serviceOnlineDevices') }}</label>
          </div>
        </div>
        <div class="svc-list">
          <div v-for="s in services?.services || []" :key="s.key" class="svc-row">
            ...
            <div class="svc-metrics">
              <span
                >{{ tl('datacenter.serviceDevices') }}: <b>{{ s.deviceCount }}</b></span
              >
              <span class="ok"
                >{{ tl('datacenter.serviceOnline') }}: <b>{{ s.onlineCount }}</b></span
              >
              <span :class="s.alarmCount ? 'warn' : ''"
                >{{ tl('datacenter.serviceAlarm') }}: <b>{{ s.alarmCount }}</b></span
              >
            </div>
          </div>
          <div v-if="!services || !services.services.length" class="empty">
            {{ tl('datacenter.serviceEmpty') }}
          </div>
        </div>
      </div>
    </div>
...
const page = useAsyncPage<Idc[]>(
  async () => {
    ...
    list.value = r || []
    dcStore.setIdcList(
      list.value.map((i) => ({ id: i.id, name: i.name, region: i.region, status: i.status })),
    )
    return list.value
  },
...
function openEdit(d: Idc) {
  ...
  Object.assign(form, {
    ...
    region: d.region,
    address: d.address,
    powerCapacityMw: d.powerCapacityMw,
    coolingCapacityMw: d.coolingCapacityMw,
    rackCapacity: d.rackCapacity,
    rooms: d.rooms,
    status: d.status,
...
function save() {
  ...
  op.then(() => {
    ...
    .catch((e) => toast.error(toErrorMessage(e, '保存失败')))
...
async function toggleStatus(d: Idc) {
  ...
  if (
    !(await useConfirm({
      message: tl('datacenter.confirmToggle', { name: d.name, act }),
      danger: true,
    }))
  )
    return
  toggleIdcStatus(d.id)
    .then((r) => {
      toast.success(
        tl('datacenter.toggleOk', {
          txt: r.status === 'disabled' ? tl('datacenter.disabled') : tl('datacenter.enabled'),
        }),
      )
      page.reload()
    })
    .catch((e) => toast.error(toErrorMessage(e, '操作失败')))
}
...
async function remove(d: Idc) {
  ...
  deleteIdc(d.id)
    ...
    .catch((e) => toast.error(toErrorMessage(e, '删除失败')))
}
...
async function batchDelete() {
  ...
  if (
    !(await useConfirm({
      message: tl('datacenter.confirmBatchDelete', { n: selectedIds.value.length }),
      danger: true,
    }))
  )
    return
  batchDeleteIdcs(selectedIds.value)
    ...
    .catch((e) => toast.error(toErrorMessage(e, '批量删除失败')))
}

function openOpLog() {
  getIdcOpLogs(50)
    .then((r) => {
      opLogs.value = r.items || []
    })
...
.log-row {
  display: grid;
  grid-template-columns: 1.3fr 0.8fr 1.2fr 0.7fr 1fr;
  gap: 8px;
  padding: 8px;
