<template>
  <div>
    <div class="view-head">
      ...
      <span class="sub">{{ tl('外部设备注册') }} {{ tl('·') }} {{ tl('在线状态') }} {{ tl('·') }} {{ tl('实时测点落地') }} ({{ tl('契约') }}: /api/external/*)</span>
      <span class="pill">HTTP + Kafka {{ tl('双通道') }}</span>
    </div>

    <!-- 概览指标 (容器层展示) -->
    <div class="grid cols-4" v-if="list">
      <div class="card"><div class="ct">{{ tl('已注册设备') }}</div><div class="cv">{{ list.total }}<small>{{ tl('台') }}</small></div></div>
      <div class="card"><div class="ct">{{ tl('在线') }}</div><div class="cv" style="color:var(--green)">{{ list.online }}<small>{{ tl('台') }}</small></div></div>
      <div class="card"><div class="ct">{{ tl('离线') }}</div><div class="cv" style="color:var(--txt2)">{{ list.offline }}<small>{{ tl('台') }}</small></div></div>
      <div class="card"><div class="ct">{{ tl('累计测点') }}</div><div class="cv" style="color:var(--cyan)">{{ list.total_metrics.toLocaleString() }}<small>{{ tl('条') }}</small></div></div>
    </div>
    ...
    <div class="card">
      <div class="flex gap8 wrap" style="align-items:center">
        ...
        <input v-model.trim="filters.domain" class="ipt" placeholder="业务域 (如 hvac_terminal)" style="width:220px" @keyup.enter="load" />
        <input v-model.trim="filters.protocol" class="ipt" placeholder="协议 (如 modbus/snmp/kafka)" style="width:200px" @keyup.enter="load" />
        <button class="btn-sm" @click="load">{{ tl('查询') }}</button>
        <button class="btn-sm" @click="resetFilters">{{ tl('重置') }}</button>
        <span class="muted" style="margin-left:auto;font-size:11px">{{ loading ? "刷新中…" : `最近刷新 ${lastRefresh}` }}</span>
      </div>
    </div>
    ...
    <teleport to="body">
      <div class="modal-mask" v-if="deleteConfirmOpen" @click.self="deleteConfirmOpen = false">
        <div class="modal" style="width:min(420px,94vw)">
          <div class="modal-head"><div class="modal-title">{{ tl('确认删除') }}</div><button class="btn-sm" @click="deleteConfirmOpen = false">{{ tl('关闭') }} ✕</button></div>
          <div class="modal-body">
            <p>{{ tl('确定要删除设备') }} <strong>{{ deleteTarget?.device_id }}</strong> {{ tl('吗') }}？</p>
            <p class="muted" style="font-size:11px;margin-top:4px">{{ tl('该操作将同时删除此设备的所有测点数据') }}，{{ tl('且不可恢复') }}。</p>
            <div v-if="deleteError" class="result warn" style="margin-top:12px">{{ deleteError }}</div>
          </div>
          <div class="modal-foot">
            <button class="btn-sm" @click="deleteConfirmOpen = false">{{ tl('取消') }}</button>
            <button class="btn-sm danger" :disabled="deleting" @click="doDelete">{{ deleting ? '删除中…' : '确认删除' }}</button>
          </div>
        </div>
      </div>
    </teleport>

    <div class="footer-note">{{ tl('运维作业·采集器接入') }} {{ tl('—') }} {{ tl('数据每') }} {{ refreshSec }}s {{ tl('刷新') }}, {{ tl('接入后端') }} /api/external/devices</div>
  </div>
</template>

<script setup lang="ts">import { useI18n } from "vue-i18n";
const { t: tl } = useI18n();
import { onBeforeUnmount, onMounted, reactive, ref } from "vue";import { deleteDevice, getDeviceMetrics, getExternalDevices, registerDevice, updateDevice } from "@/api";
import type { DeviceListResponse, ExternalDevice, ExternalDeviceView, MetricRecordView } from "@/types";
...
const FIELD_LABELS: Record<string, string> = {
  device_id: "设备 ID", ip: "IP 地址", sn: "序列号", model: "型号",
  name: tl('名称'), vendor: "厂商", domain: "业务域", category: "类别",
  location: "位置", protocol: "协议", tags: "标签", description: "描述",
};
...
.btn-sm.danger:disabled { opacity: .6; cursor: default; }
...
.modal-mask { position: fixed; inset: 0; background: rgba(6, 11, 20, .6); backdrop-filter: blur(2px); display: flex; align-items: center; justify-content: center; padding: 6vh 16px; z-index: 1000; }
.modal { width: min(420px, 94vw); background: linear-gradient(180deg, var(--panel), var(--bg2)); border: 1px solid var(--line); border-radius: 14px; box-shadow: 0 20px 60px rgba(0, 0, 0, .5); overflow: hidden; }
...
.modal-foot { display: flex; align-items: center; justify-content: flex-end; gap: 12px; padding: 10px 16px; border-top: 1px solid var(--line); }
</style>
