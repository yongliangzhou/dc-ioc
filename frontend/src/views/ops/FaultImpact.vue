    ...
    <div class="view-head">
      <div class="vh-icon">
        <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.8">
          <path d="M12 2v6M12 22v-6M2 12h6M22 12h-6M5 5l4 4M19 5l-4 4M5 19l4-4M19 19l-4-4" stroke-linecap="round"/>
        </svg>
      </div>
      <div>
        <h1>{{ t.title }}</h1>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div>
          ...
          <div class="flex gap-2 mt-3">
            <button class="btn-primary flex-1" @click="analyze" :disabled="!selectedIds.length || loading">
    ...
    <div v-if="selectedIds.length === 1 && selSource" class="card">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div class="card" style="padding:12px;text-align:center;border-color:var(--line)">
          <div class="text-2xl font-bold" :style="{ color: healthColor(selSource.health) }">{{ selSource.health }}</div>
          <div class="text-xs mt-1" style="color:var(--txt2)">{{ t.health }}</div>
        </div>
        <div class="card" style="padding:12px;text-align:center;border-color:var(--line)">
          <div class="text-2xl font-bold" :style="{ color: loadColor(selSource.loadPct) }">{{ selSource.loadPct }}%</div>
          <div class="text-xs mt-1" style="color:var(--txt2)">{{ t.load }}</div>
        </div>
        <div class="card" style="padding:12px;text-align:center;border-color:var(--line)">
          <div class="text-2xl font-bold" style="color:var(--txt-strong)">{{ selSource.status || '—' }}</div>
          <div class="text-xs mt-1" style="color:var(--txt2)">{{ t.colStatus }}</div>
        </div>
        <div class="card" style="padding:12px;text-align:center;border-color:var(--line)">
          <div class="text-2xl font-bold" style="color:var(--txt-strong)">{{ selSource.roomCode || '—' }}</div>
          <div class="text-xs mt-1" style="color:var(--txt2)">{{ t.room }}</div>
        </div>
      </div>
      <div class="mt-3 space-y-2">
        <div>
          <div class="flex justify-between text-xs" style="color:var(--txt2)"><span>{{ t.health }}</span><span>{{ selSource.health }}/100</span></div>
          <div class="pbar"><i :style="{ width: selSource.health + '%', background: healthBar(selSource.health) }"></i></div>
        </div>
        <div>
          <div class="flex justify-between text-xs" style="color:var(--txt2)"><span>{{ t.load }}</span><span>{{ selSource.loadPct }}%</span></div>
          <div class="pbar"><i :style="{ width: Math.min(100, selSource.loadPct) + '%', background: loadBar(selSource.loadPct) }"></i></div>
        </div>
      </div>
      <div class="mt-3">
        <div class="text-xs mb-1" style="color:var(--txt2)">{{ t.telemetryRealtime }}</div>
        <div v-if="realtimeLoading" class="text-xs" style="color:var(--txt3)">{{ t.analyzing }}</div>
        <div v-else-if="realtimePoints.length" class="flex flex-wrap gap-2">
          <span v-for="p in realtimePoints" :key="p.metric_name" class="tag">{{ p.metric_name }}: {{ p.value }}{{ p.unit || '' }}</span>
        </div>
    ...
    <template v-else-if="result">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div class="lg:col-span-2 card">
          ...
          <svg :viewBox="`0 0 ${svgW} ${svgH}`" class="w-full" style="min-height:320px">
            <line v-for="(e, i) in drawEdges" :key="'e' + i"
              :x1="e.x1" :y1="e.y1" :x2="e.x2" :y2="e.y2"
              :stroke="e.type === 'it_feed' ? '#f59e0b' : (e.critical ? '#dc2626' : '#cbd5e1')"
              :stroke-width="e.type === 'it_feed' ? 1.2 : (e.critical ? 3 : 1.5)" :stroke-dasharray="e.type === 'it_feed' ? '4 3' : ''" />
            <g v-for="d in drawNodes" :key="d.id">
              <circle :cx="d.x" :cy="d.y" r="13"
                :fill="d.state === 'fault' ? '#dc2626' : (d.state === 'affected' ? '#f59e0b' : '#3b82f6')"
                :stroke="d.critical ? '#7f1d1d' : '#fff'" :stroke-width="d.critical ? 2.5 : 1" />
              <text :x="d.x" :y="d.y + 4" text-anchor="middle" fill="#fff" font-size="9">{{ d.short }}</text>
              <text :x="d.x" :y="d.y + 28" text-anchor="middle" fill="#9fb3d1" font-size="10">{{ d.label }}</text>
            </g>
          </svg>
          <div class="flex gap-4 mt-2 text-xs flex-wrap" style="color:var(--txt2)">
            <span class="flex items-center gap-1"><span class="w-3 h-3 rounded-full bg-red-600 inline-block"></span>{{ t.faultNode }}</span>
            <span class="flex items-center gap-1"><span class="w-3 h-3 rounded-full bg-amber-500 inline-block"></span>{{ t.affectedNode }}</span>
            <span class="flex items-center gap-1"><span class="w-3 h-3 rounded-full bg-blue-500 inline-block"></span>{{ t.normalNode }}</span>
            <span class="flex items-center gap-1"><span class="w-3 h-3 rounded-full border-2 border-red-900 bg-gray-100 inline-block"></span>{{ t.critical }}</span>
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div class="space-y-3">
            ...
            <div class="flex items-center justify-between">
              ...
              <span class="text-sm font-semibold" style="color:var(--txt-strong)">{{ result.summary.affectedCount }}</span>
            </div>
            <div class="flex items-center justify-between">
              <span class="text-xs" style="color:var(--txt2)">{{ t.criticalPaths }}</span>
              <span class="text-sm font-semibold" style="color:var(--txt-strong)">{{ result.summary.criticalPaths }}</span>
            </div>
            <div class="flex items-center justify-between">
              <span class="text-xs" style="color:var(--txt2)">{{ t.bizCount }}</span>
              <span class="text-sm font-semibold" style="color:var(--txt-strong)">{{ result.summary.bizCount }}</span>
            </div>
            <div class="flex items-center justify-between">
              <span class="text-xs" style="color:var(--txt2)">{{ t.slaRisk }}</span>
              <span class="text-sm font-semibold" :style="{ color: result.summary.slaRisk === 'high' ? 'var(--red)' : 'var(--green)' }">
                {{ result.summary.slaRisk === 'high' ? t.sevHigh : (result.summary.slaRisk === 'medium' ? t.sevMedium : t.sevLow) }}
              </span>
            </div>
            <div class="pt-2" style="border-top:1px solid var(--line)">
              <p class="text-xs mb-1" style="color:var(--txt2)">{{ t.suggestion }}</p>
              <p class="text-sm leading-relaxed" style="color:var(--txt)">{{ result.suggestion }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="result.businesses.length" class="card mt-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
          <div v-for="b in result.businesses" :key="b.business" class="card" style="padding:12px;border-color:var(--line)"
            :style="b.severity === 'critical' ? 'border-color:rgba(255,77,94,.5);background:rgba(255,77,94,.06)' : ''">
            <div class="flex items-center justify-between">
              <span class="text-sm font-semibold" style="color:var(--txt-strong)">{{ b.business }}</span>
              <span class="tag" :class="sevClass(b.severity)">{{ sevText(b.severity) }}</span>
            </div>
            <div class="mt-2 text-xs space-y-1" style="color:var(--txt2)">
              <div class="flex justify-between"><span>{{ t.bizSla }}</span><span style="color:var(--txt)">{{ b.sla }}</span></div>
              <div class="flex justify-between"><span>{{ t.bizAffected }}</span><span style="color:var(--txt)">{{ b.affectedDevices }}</span></div>
              <div class="flex justify-between"><span>{{ t.critical }}</span><span style="color:var(--txt)">{{ b.criticalDevices }}</span></div>
      ...
      <div v-if="result.mitigations && result.mitigations.length" class="card mt-4">
        ...
        <div class="space-y-2">
          <div v-for="m in result.mitigations" :key="m.seq" class="flex items-start gap-3 card" style="padding:12px;border-color:var(--line)"
            :style="m.priority === 'P0' ? 'border-color:rgba(255,77,94,.5);background:rgba(255,77,94,.06)' : (m.priority === 'P1' ? 'border-color:rgba(255,176,32,.5);background:rgba(255,176,32,.06)' : '')">
            <span class="prio" :class="'p-' + m.priority">{{ m.priority }}</span>
            <div class="min-w-0">
              <div class="text-sm font-semibold" style="color:var(--txt-strong)">{{ m.action }} · <span class="font-normal" style="color:var(--txt2)">{{ m.target }}</span></div>
              <div class="text-xs mt-0.5 leading-relaxed" style="color:var(--txt2)">{{ m.detail }}</div>
            </div>
          </div>
        </div>
      </div>
          ...
          <table class="w-full">
            ...
            <tbody>
              <tr v-for="n in affectedNodes" :key="n.id">
                ...
                <td>{{ n.health }}</td>
                <td>{{ n.hop }}</td>
                <td><span class="tag" :class="n.critical ? 'r' : ''">{{ n.critical ? t.critical : t.normal }}</span></td>
                <td>{{ n.business || '—' }}</td>
              </tr>
            </tbody>
          </table>
      ...
      <div class="card mt-4">
        <div class="card-head">
          ...
          <button :disabled="savingHistory || !result.faultIds.length" @click="saveReport" class="btn-primary">
            {{ t.historySave }}
          </button>
        </div>
          ...
          <div v-for="h in historyList" :key="h.id" class="card" style="padding:12px">
            <div class="flex items-center justify-between">
              <div class="text-sm font-medium" style="color:var(--txt-strong)">
                {{ h.title || ('#' + h.id) }}
                <span class="tag" style="margin-left:8px" :class="sevClass(h.severity)">{{ h.severity }}</span>
              </div>
              <div class="text-xs" style="color:var(--txt3)">{{ h.createdAt }}</div>
            </div>
            <div class="text-xs mt-1" style="color:var(--txt2)">
              {{ t.historyCreatedBy }}: {{ h.createdBy || '—' }} ·
              {{ t.historySigners }}: {{ h.signers.length ? h.signers.join('、') : '—' }}
              <span v-if="h.pushed" class="ml-2" style="color:var(--green)">· {{ t.historyPushed }}</span>
            </div>
            <div class="flex items-center gap-2 mt-2">
              <input v-model="signerName" :placeholder="t.signPlaceholder" class="inp" style="flex:1" />
              <button @click="signReport(h.id)" class="btn-ghost" style="padding:7px 12px">{{ t.historySign }}</button>
              <button @click="pushReport(h)" class="btn-ghost" style="padding:7px 12px">{{ t.historyPush }}</button>
            </div>
          </div>
        </div>
      </div>
...
const t = new Proxy({} as any, {
  get(_t, key) {
    const ns = (raw('faultImpact') || {}) as any
...
const selSource = computed(() =>
  selectedIds.value.length === 1 ? (sources.value.find((s) => s.id === selectedIds.value[0]) || null) : null
)
...
// ---- 颜色辅助 ----
const healthColor = (h: number) => (h <= 40 ? 'var(--red)' : h <= 70 ? 'var(--amber)' : 'var(--green)')
...
const drawNodes = computed(() => {
  const out: Array<{ id: number; x: number; y: number; label: string; short: string; state: string; critical: boolean }> = []
  layerNodes.value.forEach((layer, li) => {
    ...
    layer.nodes.forEach((n, ni) => {
      ...
      out.push({
        id: n.id, x, y, label: n.label.length > 10 ? n.label.slice(0, 9) + '…' : n.label,
        short: String(n.id).slice(-2), state: n.state, critical: n.critical,
      })
    })
    if (layer.hidden > 0) {
      const x = svgW / 2 + (total - (total - 1) / 2) * NODE_GAP + NODE_GAP / 2
      out.push({ id: -layer.hop - 1, x, y, label: `+${layer.hidden}`, short: '+', state: 'affected', critical: false })
    }
  })
  return out
})
...
const drawEdges = computed(() => {
  const res = result.value
  const out: Array<{ x1: number; y1: number; x2: number; y2: number; type: string; critical: boolean }> = []
  if (!res) return out
...
const affectedNodes = computed(() =>
  (result.value?.nodes || []).filter((n) => n.state !== 'normal').sort((a, b) => a.hop - b.hop || a.label.localeCompare(b.label))
)
...
function exportReport() {
  ...
  lines.push(`故障源数: ${r.summary.faultCount}  受影响节点: ${r.summary.affectedCount}  关键链路: ${r.summary.criticalPaths}  SLA风险: ${r.summary.slaRisk}`)
  lines.push('')
  lines.push('--- 故障源 ---')
  r.nodes.filter((n) => n.state === 'fault').forEach((n) => lines.push(`  [${n.category}] ${n.label} (${n.roomCode || '—'})`))
  lines.push('')
  lines.push('--- 受影响业务域 ---')
  r.businesses.forEach((b) => lines.push(`  ${b.business} (SLA ${b.sla}) 受影响设备 ${b.affectedDevices} 关键 ${b.criticalDevices} [${sevText(b.severity)}]`))
  lines.push('')
  lines.push('--- 受影响资产 ---')
  affectedNodes.value.forEach((n) => lines.push(`  ${n.label} | ${n.category} | 机房 ${n.roomCode || '—'} | 跳数 ${n.hop} | ${n.critical ? '关键' : '普通'} | ${n.business || '—'}`))
  lines.push('')
  lines.push('--- 处置建议 ---')
