...
export function useTelemetry(deviceId: Ref<string> | string, opts: TelemetryOptions = {}) {
  ...
  function _connectWs(): void {
    ...
    const base = opts.wsBase ?? import.meta.env.VITE_WS_URL ?? `ws://${window.location.hostname}:8000`;
    ...
    sock.onmessage = (ev: MessageEvent) => {
      try {
        ...
        } else if (msg.type === "device_metrics" && msg.device_id === devId && Array.isArray(msg.points)) {
