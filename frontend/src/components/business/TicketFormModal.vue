    ...
    <div v-if="open" class="tf-mask" @click.self="onClose">
      <div class="tf-modal">
        ...
        <div class="tf-body">
          ...
          <label>处理说明 / 描述</label>
          <textarea v-model.trim="form.description" class="tf-input tf-area" rows="4" placeholder="详细描述问题现象、影响范围与处理要求" />
        ...
        <div class="tf-foot">
          ...
          <button class="tf-btn primary" :disabled="!canSubmit" @click="onSubmit">确认{{ isEdit ? "保存" : "创建" }}</button>
        </div>
      </div>
    </div>
  </teleport>
...
.tf-head {
  ...
  padding: 16px 18px; border-bottom: 1px solid var(--line, rgba(255,255,255,.1));
}
.tf-head h3 { margin: 0; font-size: 15px; color: var(--cyan, #22e3ff); }
.tf-x { background: none; border: none; color: var(--txt2, #8892b0); font-size: 15px; cursor: pointer; }
.tf-body { padding: 16px 18px; display: flex; flex-direction: column; gap: 6px; }
.tf-row { display: flex; gap: 12px; }
.tf-col { flex: 1; display: flex; flex-direction: column; gap: 6px; }
.tf-body label { font-size: 11.5px; color: var(--txt2, #8892b0); margin-top: 6px; }
.req { color: var(--red, #ff4d5e); }
.tf-input {
  background: var(--bg2, #0e1426); border: 1px solid var(--line, rgba(255,255,255,.12));
  border-radius: 7px; color: var(--txt, #e6f1ff); padding: 8px 10px; font-size: 13px; outline: none;
  width: 100%; box-sizing: border-box;
}
.tf-input:focus { border-color: var(--cyan, #22e3ff); }
.tf-area { resize: vertical; font-family: inherit; }
.tf-badge { margin-top: 8px; }
.tf-foot {
  display: flex; justify-content: flex-end; gap: 10px;
  padding: 14px 18px; border-top: 1px solid var(--line, rgba(255,255,255,.1));
}
.tf-btn {
  padding: 8px 18px; border-radius: 7px; font-size: 13px; cursor: pointer; border: 1px solid transparent;
}
.tf-btn.ghost { background: transparent; border-color: var(--line, rgba(255,255,255,.15)); color: var(--txt2, #8892b0); }
.tf-btn.primary { background: linear-gradient(135deg, #1a73e8, #22e3ff); color: #fff; font-weight: 600; }
