        ...
        <tbody>
          <tr v-for="(row, ri) in rows" :key="ri" :class="getRowClass(row)">
            ...
            >
              ...
              <!-- 自定义渲染 -->
              <slot v-else-if="col.render === 'custom'" :name="'col-' + col.key" :row="row" :value="row[col.key]" />
              <!-- 默认文本 -->
...
tbody tr:hover { background: rgba(34, 227, 255, 0.03); }
tbody tr.row-danger { background: rgba(239, 68, 68, 0.06); }
tbody tr.row-warning { background: rgba(255, 176, 32, 0.04); }
</style>
