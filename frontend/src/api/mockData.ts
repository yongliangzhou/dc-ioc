...
interface DemoDeviceItem {
  device_id: string; name: string; model: string; ip: string; protocol: string;
  online: boolean; last_seen?: string | null; metric_count: number;
}
interface DemoDeviceList {
  total: number; online: number; offline: number; total_metrics: number; items: DemoDeviceItem[];
}
...
function buildGensetUnits() {
  ...
  const PROT = ["过速保护", "低油压保护", "高水温保护", "过电流保护", "过电压保护", "欠电压保护", "逆功率保护", "接地故障保护", "差动保护", "失磁保护", "启动失败保护", "紧急停机"];
  return states.map((st, i) => {
    const running = st === "运行";
    const breaker = running ? "合闸" : "分闸";
    const incomer = running ? "合闸" : "分闸";
    let ua = 0, ub = 0, uc = 0, u = 0, ia = 0, ib = 0, ic = 0, iAvg = 0, p = 0, q = 0, pf = 0, freq = 0, energy = 0, rpm = 0, waterT = 0, oilP = 0;
    let faults: any[] = [];
    let prots: any[] = [];
    if (running) {
      ...
      waterT = rnd(38, 46, 0); energy = rnd(16000, 30000, 0);
      faults = [
      ...
      waterT = rnd(38, 46, 0); energy = rnd(20000, 40000, 0);
      prots = PROT.map((n) => ({ name: n, state: "投入", level: "g" }));
    }
    return {
      id: `DG-${String(i + 1).padStart(2, "0")}`,
      state: st, breaker, incomer,
      ua, ub, uc, u, ia, ib, ic, i: iAvg, p, q, pf, freq, energy,
      rpm, waterT, oilP, battU: +rnd(25.6, 27.2, 1).toFixed(1),
      heater: "投入", startCnt: rnd(42, 88, 0), runHrs: rnd(220, 480, 0),
      faults, protections: prots,
    };
  });
}
...
function buildFuel() {
  const sw = (level: number) => {
    const thr: [string, string, number, "g" | "a" | "r" | "b"][] = [
      ["低低位 LL", "LL", 5, "r"], ["低位 L", "L", 12, "a"],
      ["高位 H", "H", 88, "a"], ["高高位 HH", "HH", 95, "r"],
    ]
    return thr.map(([name, th, t, sev]) => {
      const trig = ((th === "LL" || th === "L") && level <= t) || ((th === "H" || th === "HH") && level >= t)
      return { name, th, state: trig ? "闭合" : "断开", level: trig ? sev : "g" } as const
    })
  }
  const MAIN_PROT = ["高液位保护", "低液位保护", "低低位联锁停泵", "渗漏检测保护", "静电接地保护", "温度高保护", "高低液位联锁"]
  const DAY_PROT = ["高液位保护", "低液位保护", "渗漏检测保护", "温度高保护", "进油阀联锁"]
  const PUMP_PROT = ["过载保护", "短路保护", "干转保护", "轴承温度高保护", "密封泄漏保护", "出口过压保护"]
  const mainProts = (level: number) => MAIN_PROT.map((n) =>
    n === "高液位保护" && level >= 88
  ...
  const mainTanks = [
    { id: "地埋主油罐 #1", cap: 50000, level: 86, t: 21.3, water: "无", leak: "正常",
      valves: [{ name: "进油阀", state: "闭合", level: "g" }, { name: "出油阀", state: "开启", level: "g" }],
      switches: sw(86), protections: mainProts(86) },
    { id: "地埋主油罐 #2", cap: 50000, level: 90, t: 21.1, water: "无", leak: "正常",
      valves: [{ name: "进油阀", state: "闭合", level: "g" }, { name: "出油阀", state: "开启", level: "g" }],
      switches: sw(90), protections: mainProts(90) },
  ]
  const dayTanks = Array.from({ length: 8 }, (_, i) => {
    const lv = rnd(78, 96, 0)
    return {
      id: `日用油箱 DT-${i + 1}`, cap: 1000, level: lv, leak: "正常",
      valve: { name: "进油阀", state: "开启", level: "g" },
      switches: sw(lv),
      protections: DAY_PROT.map((n) => ({ name: n, state: "投入", level: "g" })),
    }
  })
  const pumpStates: [string, string, string][] = [["输油泵 P-1", "运行", "自动"], ["输油泵 P-2", "运行", "自动"], ["输油泵 P-3(备)", "待机", "自动"]]
  const pumps = pumpStates.map(([pid, st, mode], idx) => {
    const alarms = ["轴承温度", "电机电流", "密封泄漏", "出口压力"].map((n) => ({ name: n, value: "正常", level: "g" }))
    if (idx === 1) alarms[0] = { name: "轴承温度", value: "偏高 78℃", level: "a" as const }
    const prots = PUMP_PROT.map((n) => ({ name: n, state: "投入", level: "g" }))
    if (idx === 1) { const p = prots.find((x) => x.name === "轴承温度高保护"); if (p) { p.state = "动作"; p.level = "a" as const } }
    return { id: pid, state: st, mode, alarms, protections: prots }
  })
  return {
    mainTanks, dayTanks, pumps,
...
function buildBattery() {
  ...
  const groups = specs.map(([gid, gtype, ncell, vbase, soc]) => {
    ...
    const cells = Array.from({ length: ncell }, (_, c) => {
      let u: number, ir: number, t: number, level: "g" | "a" | "r" | "b" = "g"
      if (isLi) { u = +(vbase + rnd(-0.08, 0.08, 3)).toFixed(3); ir = +rnd(0.25, 0.55, 2).toFixed(2); t = +rnd(23, 28, 1).toFixed(1) }
      else { u = +(vbase + rnd(-0.15, 0.15, 2)).toFixed(2); ir = +rnd(5.0, 8.5, 2).toFixed(2); t = +rnd(22, 27, 1).toFixed(1) }
      if ((isLi && ir > 0.5) || (!isLi && ir > 8.0)) level = "a"
      return { no: `#${String(c + 1).padStart(2, "0")}`, u, t, ir, level }
    })
    if (gid === "HVDC-03 电池组") { cells[27].ir = 0.62; cells[27].level = "a" }
    ...
    return {
      id: gid, type: gtype, soc,
      u: totalU, i: +rnd(0.1, 0.4, 2).toFixed(2), cdState: "浮充",
      maxT, worstCell: `${worst.no} ${worst.u}V`, ir: irConcl, state: "浮充",
      cells,
    }
  })
  const cellAlarms = [{ g: "HVDC-03 电池组", cell: "#28", item: "内阻偏高 (+18%)", lv: "预警", ts: "07-21 03:12" }]
  return { groups, backupMin: 15, lastDischarge: "2026-06-30 核容放电 30% · 通过", cellAlarms }
}

export const DC = {
  name: "华东-杭州 EC1 数据中心",
  buildings: 2, rooms: 12, racks: 3600, itDesign: 36,

  /* ---------- 全局 KPI ---------- */
  kpi: {
    pue: 1.247, wue: 1.62, itLoad: 24.6, totalLoad: 30.7,
    coolLoad: 4.9, availability: 99.9995, alarms: { crit: 1, warn: 6, info: 14 },
    freeCoolHours: 4382,
  },
  ...
  chillerPlant: {
    ...
    flow: 2860, coolingCap: 18.6, plr: 68,
    storageTank: { level: 92, dischargeMin: 15, mode: "保冷备用", capacity: 3000 },
    chillers: [
      { id: "CH-01", state: "运行", load: 78, cop: 6.42, evapT: 14.9, condT: 29.3, current: 76, runHrs: 12480 },
      { id: "CH-02", state: "运行", load: 72, cop: 6.31, evapT: 15.1, condT: 29.6, current: 71, runHrs: 11923 },
      { id: "CH-03", state: "待机", load: 0, cop: 0, evapT: "-", condT: "-", current: 0, runHrs: 10102 },
      { id: "CH-04", state: "检修", load: 0, cop: 0, evapT: "-", condT: "-", current: 0, runHrs: 13877 },
    ],
    towers: [
    ...
    pumps: {
      chw: [
        { id: "CHWP-01", state: "运行", hz: 42, kw: 55 }, { id: "CHWP-02", state: "运行", hz: 42, kw: 54 },
        { id: "CHWP-03", state: "待机", hz: 0, kw: 0 },
      ],
      cw: [
        { id: "CWP-01", state: "运行", hz: 44, kw: 75 }, { id: "CWP-02", state: "运行", hz: 44, kw: 74 },
    ...
    valves: [
      { id: "V-101 冷机侧电动阀", pos: 100, state: "开" }, { id: "V-201 板换侧电动阀", pos: 100, state: "开" },
      { id: "V-301 蓄冷罐放冷阀", pos: 0, state: "关" }, { id: "V-401 旁通调节阀", pos: 18, state: "调节" },
    ],
    staging: { rule: "供水温度 > 设定+1.0℃ 持续 5min 加机；PLR < 45% 持续 15min 减机", lastAction: "10:42 CH-02 自动加机(负载爬升)", next: "—" },
  ...
  /* ---------- 暖通 · 空调末端 ---------- */
  crac: {
    summary: { total: 96, running: 78, standby: 14, fault: 2, maint: 2, avgSupply: 18.3, avgReturn: 27.9 },
    rooms: Array.from({ length: 12 }, (_, i) => ({
      id: `包间 R${String(i + 1).padStart(2, "0")}`,
      coldAisle: rnd(21.5, 24.5), hotAisle: rnd(31, 36), rh: rnd(42, 58, 0),
      cracRun: pick([6, 7, 8]), cracN: 8, state: "正常",
    })),
    units: Array.from({ length: 10 }, (_, i) => ({
      id: `CRAC-${String(i + 1).padStart(2, "0")}`, room: `R${String((i % 5) + 1).padStart(2, "0")}`,
      type: i % 3 === 0 ? "列间空调" : "房间级精密空调",
      state: i === 7 ? "故障" : (i === 4 ? "待机" : "运行"),
      supplyT: i === 7 ? "-" : rnd(17.5, 19.5), returnT: i === 7 ? "-" : rnd(26, 30),
      fan: i === 7 || i === 4 ? 0 : rnd(60, 90, 0), valve: i === 7 || i === 4 ? 0 : rnd(35, 85, 0),
    })),
    fresh: [
  ...
  hv: {
    ...
    incomers: [
      { id: "10KV 1# 进线", src: "城东 220KV 变电站", state: "合闸", breaker: "合闸",
        ua: 10.41, ub: 10.43, uc: 10.42, u: 10.42, ia: 388, ib: 383, ic: 387, i: 386,
        p: 6.62, q: 1.93, pf: 0.96, freq: 50.01, energy: 142860 },
      { id: "10KV 2# 进线", src: "城西 220KV 变电站", state: "合闸", breaker: "合闸",
        ua: 10.37, ub: 10.39, uc: 10.38, u: 10.38, ia: 374, ib: 369, ic: 372, i: 371,
    ...
    feeders: [
      { id: "F-01", load: "1# 变压器", state: "合闸", breaker: "合闸",
        ua: 10.41, ub: 10.42, uc: 10.43, ia: 124, ib: 120, ic: 122, p: 2.05, pf: 0.95, energy: 42150 },
      { id: "F-02", load: "2# 变压器", state: "合闸", breaker: "合闸",
        ua: 10.40, ub: 10.41, uc: 10.42, ia: 120, ib: 117, ic: 118, p: 1.98, pf: 0.94, energy: 40780 },
      { id: "F-03", load: "3# 变压器", state: "合闸", breaker: "合闸",
        ua: 10.42, ub: 10.43, uc: 10.44, ia: 128, ib: 124, ic: 126, p: 2.12, pf: 0.96, energy: 43890 },
      { id: "F-04", load: "4# 变压器", state: "合闸", breaker: "合闸",
        ua: 10.39, ub: 10.40, uc: 10.41, ia: 116, ib: 112, ic: 114, p: 1.92, pf: 0.93, energy: 39620 },
      { id: "F-05", load: "冷机房变", state: "合闸", breaker: "合闸",
        ua: 10.38, ub: 10.39, uc: 10.40, ia: 98, ib: 95, ic: 96, p: 1.61, pf: 0.92, energy: 33140 },
      { id: "F-06", load: "备用", state: "分闸", breaker: "分闸",
        ua: 10.40, ub: 10.41, uc: 10.42, ia: 0, ib: 0, ic: 0, p: 0, pf: 0, energy: 0 },
    ],
    transformers: [
      { id: "1# 变压器 2500kVA", feeder: "F-01", state: "运行",
        ...
        signals: [
          { name: "运行状态", value: "运行", level: "g" }, { name: "高压断路器", value: "合闸", level: "g" },
          { name: "低压断路器", value: "合闸", level: "g" }, { name: "轻瓦斯", value: "无", level: "g" },
          { name: "重瓦斯", value: "无", level: "g" }, { name: "绕组超温", value: "无", level: "g" },
          { name: "压力释放", value: "无", level: "g" }, { name: "冷却风机", value: "运行", level: "g" },
      ...
      { id: "3# 变压器 2500kVA", feeder: "F-03", state: "运行",
        ...
        signals: [
          { name: "运行状态", value: "运行", level: "g" }, { name: "高压断路器", value: "合闸", level: "g" },
          { name: "低压断路器", value: "合闸", level: "g" }, { name: "轻瓦斯", value: "无", level: "g" },
          { name: "重瓦斯", value: "无", level: "g" }, { name: "绕组超温", value: "预警", level: "a" },
          { name: "压力释放", value: "无", level: "g" }, { name: "冷却风机", value: "运行", level: "g" },
  ...
  lv: {
    transformers: [
      { id: "T1 2500KVA", load: 61, t: 78, state: "运行", u: 0.398, i: 1860, p: 1455, q: 430, pf: 0.96, freq: 50.02, energy: 28940, thdu: 2.3, thdi: 4.1 },
      { id: "T2 2500KVA", load: 58, t: 76, state: "运行", u: 0.397, i: 1795, p: 1380, q: 410, pf: 0.96, freq: 50.01, energy: 27620, thdu: 2.2, thdi: 4.0 },
      { id: "T3 2500KVA", load: 63, t: 81, state: "运行", u: 0.399, i: 1910, p: 1505, q: 450, pf: 0.96, freq: 50.02, energy: 30110, thdu: 2.5, thdi: 4.3 },
      { id: "T4 2500KVA", load: 55, t: 74, state: "运行", u: 0.396, i: 1670, p: 1295, q: 380, pf: 0.96, freq: 50.00, energy: 25900, thdu: 2.1, thdi: 3.9 },
    ],
    upsGroups: [
    ...
    hvdc: [
      { id: "HVDC-01", u: 243.2, load: 52, modN: 40, modRun: 26, state: "正常", i: 530, p: 126, pf: 0.99, energy: 2980, thdi: 2.8 },
      { id: "HVDC-02", u: 242.8, load: 49, modN: 40, modRun: 24, state: "正常", i: 499, p: 119, pf: 0.99, energy: 2810, thdi: 2.6 },
      { id: "HVDC-03", u: 243.5, load: 54, modN: 40, modRun: 27, state: "正常", i: 551, p: 131, pf: 0.99, energy: 3120, thdi: 2.9 },
    ],
    ats: [
    ...
    busbars: Array.from({ length: 8 }, (_, i) => ({
      id: `母排 BB-${String(i + 1).padStart(2, "0")}`, load: rnd(38, 68, 0), i: rnd(800, 1500, 0), state: "正常",
  ...
  /* ---------- 安防 · 视频监控 ---------- */
  cctv: {
    total: 486, online: 482, offline: 4, nvr: { total: 12, ok: 12, storeDays: 92, required: 90 },
    zones: [
  ...
  /* ---------- 安防 · 门禁 ---------- */
  acs: {
    doors: 268, online: 266, openAbnormal: 1,
    todayEvents: 1642, denied: 12, visitors: 9,
    areas: [
  ...
  fire: {
    ...
    vesda: Array.from({ length: 6 }, (_, i) => ({ id: `VESDA R${String(i * 2 + 1).padStart(2, "0")}`, level: pick(["正常", "正常", "正常", "轻微"]), val: rnd(0.001, 0.018, 3) })),
    qieFei: { desc: "确认火警 → 切除非消防电源(切非) → 联动气灭 → 应急照明投入", state: "自动允许", lastDrill: "2026-06-28 消防演练通过" },
  ...
  capacity: {
    ...
    rooms: Array.from({ length: 12 }, (_, i) => ({
      id: `R${String(i + 1).padStart(2, "0")}`, racks: 300, used: rnd(240, 296, 0),
      powerPct: rnd(55, 88, 0), coolPct: rnd(50, 82, 0),
    })),
  ...
  energy: {
    ...
    loadForecast: Array.from({ length: 24 }, (_, h) => ({
      h, actual: h <= 14 ? rnd(23.2, 25.4) : null, pred: rnd(23.0, 25.8),
    })),
...
interface MockMetricSpec {
  name: string; unit: string; base: number; amp: number;
  /** 随机游走: 噪声幅度 (每步漂移标准差) */
  noise?: number;
  /** 均值回归系数 (趋向 base 的速度) */
  reversion?: number;
...
const CATEGORY_METRICS: Record<string, MockMetricSpec[]> = {
  chiller: [
    { name: "supply_temp", unit: "℃", base: 15.2, amp: 0.6, noise: 0.3, reversion: 0.08, excProb: 0.05, excTarget: 19.5, excHold: 40, decimals: 1 },
    { name: "return_temp", unit: "℃", base: 21.0, amp: 0.8, noise: 0.4, reversion: 0.08, excProb: 0.02, excTarget: 23.5, excHold: 30, decimals: 1 },
    { name: "power_kw", unit: "kW", base: 520, amp: 40, noise: 15, reversion: 0.07, excProb: 0.04, excTarget: 615, excHold: 40, decimals: 0 },
    { name: "cpu_usage", unit: "%", base: 38, amp: 8, noise: 4, reversion: 0.06, decimals: 0 },
  ],
  crac: [
    { name: "supply_temp", unit: "℃", base: 18.2, amp: 0.8, noise: 0.4, reversion: 0.08, excProb: 0.02, excTarget: 19.5, excHold: 20, decimals: 1 },
    { name: "return_temp", unit: "℃", base: 27.0, amp: 1.0, noise: 0.5, reversion: 0.08, excProb: 0.05, excTarget: 34, excHold: 40, decimals: 1 },
  ...
  ups: [
    { name: "power_kw", unit: "kW", base: 312, amp: 20, noise: 10, reversion: 0.07, excProb: 0.04, excTarget: 505, excHold: 40, decimals: 0 },
...
interface MockRegDevice {
  device_id: string; name: string; model: string; ip: string; sn: string;
  vendor: string; domain: string; category: string; protocol: string; location: string;
  online: boolean;
}
const MOCK_REGISTERED: MockRegDevice[] = [
  { device_id: "CHILLER-01", name: "1#冷水机组", model: "Carrier-19XR", ip: "10.20.1.11", sn: "CH-SN-0001", vendor: "Carrier", domain: "hvac_source", category: "chiller", protocol: "modbus", location: "R01", online: true },
  { device_id: "CHILLER-02", name: "2#冷水机组", model: "Carrier-19XR", ip: "10.20.1.12", sn: "CH-SN-0002", vendor: "Carrier", domain: "hvac_source", category: "chiller", protocol: "modbus", location: "R01", online: true },
  { device_id: "CRAC-01", name: "A区精密空调", model: "Emerson-DX", ip: "10.20.2.21", sn: "CR-SN-0021", vendor: "Emerson", domain: "hvac_terminal", category: "crac", protocol: "snmp", location: "R02", online: true },
  { device_id: "CRAC-02", name: "B区精密空调", model: "Emerson-DX", ip: "10.20.2.22", sn: "CR-SN-0022", vendor: "Emerson", domain: "hvac_terminal", category: "crac", protocol: "snmp", location: "R03", online: false },
  { device_id: "UPS-A", name: "UPS A组", model: "Vertiv-Liebert", ip: "10.20.3.31", sn: "UP-SN-0031", vendor: "Vertiv", domain: "power_lv", category: "ups", protocol: "snmp", location: "R04", online: true },
  { device_id: "UPS-B", name: "UPS B组", model: "Vertiv-Liebert", ip: "10.20.3.32", sn: "UP-SN-0032", vendor: "Vertiv", domain: "power_lv", category: "ups", protocol: "snmp", location: "R04", online: true },
];
...
function buildDeviceView(d: MockRegDevice): ExternalDeviceView {
  ...
  return {
    device_id: d.device_id, ip: d.ip, sn: d.sn, model: d.model, name: d.name,
    vendor: d.vendor, domain: d.domain, category: d.category, location: d.location,
    protocol: d.protocol, tags: [], metric_count: specs.length, online: d.online,
...
function externalDevicesMock(params?: any): DeviceListResponse {
  ...
  return { total: items.length, online, offline: items.length - online, total_metrics: totalMetrics, items };
}
...
function demoDevicesMock(params?: any): DemoDeviceList {
  ...
  return { total: base.total, online: base.online, offline: base.offline, total_metrics: base.total_metrics, items };
}

function thingModelsMock(): ThingModelDef[] {
  return THING_MODELS.map((m) => ({
    category: m.category, category_label: m.name, domain: m.domain, protocol: m.protocol,
...
function recentMetricsMock(deviceId: string, limit = 20): MetricRecordView[] {
  ...
  for (let i = 0; i < limit; i++) {
    ...
    const quality: MetricQuality = Math.random() < 0.92 ? "good" : (Math.random() < 0.5 ? "uncertain" : "bad");
    out.push({
      device_id: deviceId, ts, metric_name: spec.name, value: walkVal(spec, `${deviceId}::${spec.name}`),
      quality, unit: spec.unit, received_at: new Date(now - age + 300).toISOString(),
    });
  }
  return out;
}
...
function realtimeMock(deviceId: string): MetricRealtimeResponse {
  ...
  return {
    ...
    points: specs.map((s) => ({ metric_name: s.name, value: walkVal(s, `${deviceId}::${s.name}`), unit: s.unit, quality: "good" as MetricQuality })),
  };
}
...
const ALARM_RULES: AlarmRuleDef[] = [
  { id: 1, category: "hvac", metric: "supply_temp", ruleCode: "HVAC-SUPPLY-TEMP-HI", warnLo: null, warnHi: 18, critLo: null, critHi: 22, unit: "℃", enabled: true, status: "enabled", source: "DEFAULT" },
  { id: 2, category: "hvac", metric: "return_temp", ruleCode: "HVAC-RETURN-TEMP-HI", warnLo: null, warnHi: 32, critLo: null, critHi: 38, unit: "℃", enabled: true, status: "enabled", source: "DEFAULT" },
  { id: 3, category: "power", metric: "power_kw", ruleCode: "PWR-CHILLER-HI", warnLo: null, warnHi: 600, critLo: null, critHi: 680, unit: "kW", enabled: true, status: "enabled", source: "DEFAULT" },
  { id: 4, category: "common", metric: "cpu_usage", ruleCode: "CTRL-CPU-HI", warnLo: null, warnHi: 85, critLo: null, critHi: 95, unit: "%", enabled: true, status: "silenced", source: "DEFAULT" },
  { id: 5, category: "power", metric: "ups_load", ruleCode: "PWR-UPS-LOAD-HI", warnLo: null, warnHi: 480, critLo: null, critHi: 520, unit: "kW", enabled: true, status: "enabled", source: "DEFAULT" },
  { id: 6, category: "fire", metric: "fault_points", ruleCode: "FIRE-FAULT", warnLo: 0, warnHi: 0, critLo: null, critHi: 1, unit: "个", enabled: true, status: "enabled", source: "DEFAULT" },
  { id: 7, category: "security", metric: "door_abnormal", ruleCode: "SEC-DOOR-ABN", warnLo: 0, warnHi: 0, critLo: null, critHi: 1, unit: "次", enabled: false, status: "disabled", source: "DEFAULT" },
  { id: 8, category: "hvac", metric: "supply_temp_dev", ruleCode: "HVAC-SUPPLY-DEV", warnLo: -12, warnHi: 12, critLo: -20, critHi: 20, unit: "%", enabled: true, status: "enabled", source: "DEFAULT" },
];
...
function genAlarmEvents(): AlarmEvent[] {
  ...
  for (let i = 0; i < 42; i++) {
    ...
    evs.push({
      id: `EVT-${String(1000 + i)}`, ruleId: String(r.id), ruleName: r.ruleCode ?? r.metric, metric: r.metric, sys: r.category,
      lv, desc, value, threshold: ref, unit: r.unit, state,
      triggeredAt, acknowledgedAt, acknowledgedBy, resolvedAt, resolvedBy, note,
...
function alarmHistoryMock(params?: AlarmHistoryQuery): AlarmHistoryResponse {
  ...
  const stats = {
    ...
    active24h: ALARM_EVENTS.filter((e) => e.state === "active" && within(e.triggeredAt, 86400000)).length,
    resolved24h: ALARM_EVENTS.filter((e) => e.state === "resolved" && e.resolvedAt && within(e.resolvedAt, 86400000)).length,
...
function genCabinets(): Cabinet[] {
  ...
  for (let ri = 1; ri <= 12; ri++) {
    ...
    for (let j = 1; j <= 16; j++) {
      ...
      list.push({
        id, idc_id: 1, code: `${room}-C${String(j).padStart(2, "0")}`, room, row: j % 2 ? "B" : "A",
        u_total: uTotal, u_used: uUsed, rated_power_kw: rated, current_power_kw: cur, status,
      });
      id++;
    }
  }
...
function cabinetMetricsMock(id: number, params?: any): CabinetMetrics {
  ...
  return { cabinet_id: id, code, range_minutes: minutes, temperature: tArr, humidity: hArr, power_kw: pArr };
}

interface EqSeed { d: string; c: string; n: string; v: string; m: string }
...
function genEquipment(): Equipment[] {
  ...
  for (const s of EQ_SEEDS) {
    for (let i = 1; i <= 8; i++) {
      ...
      list.push({
        id, idc_id: 1, room_id: (i % 12) + 1,
        code: `${s.c.toUpperCase()}-${String(i).padStart(3, "0")}`,
        name: `${s.n} ${String(i).padStart(2, "0")}`,
        domain: s.d, category: s.c, vendor: s.v, model: s.m,
        status, load_pct: +rnd(20, 95).toFixed(0), run_hours: Math.floor(rnd(1000, 20000)),
...
function equipmentMetricsMock(id: number, params?: any): EquipmentMetrics {
  ...
  const baseFor = (m: string) => (m === "inlet_temp" ? 22 : m === "outlet_temp" ? 30 : m === "power_kw" ? 120 : 35);
  const noiseFor = (m: string) => (m === "inlet_temp" ? 0.6 : m === "outlet_temp" ? 0.8 : m === "power_kw" ? 8 : 4);
