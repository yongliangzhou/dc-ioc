...
export const getEquipment = (id: number) =>
  request.get<unknown, Equipment>(`/api/equipment/${id}`);
...
export const getAlarmRules = () =>
  request.get<unknown, AlarmRuleDef[]>("/api/alarm-rules");
...
export const toggleAlarmRule = (id: string, status: AlarmRuleStatus) =>
  request.patch<unknown, AlarmRuleDef>(`/api/alarm-rules/${encodeURIComponent(id)}/status`, { status });

export const getAuditLogs = (params: Record<string, any> = {}) =>
  request.get<unknown, { items: any[]; total: number; page: number; page_size: number }>("/api/audit-logs", { params });
...
export const getCampuses = () =>
  request.get<unknown, CampusesResponse>("/api/dashboard/campuses");
...
export const silenceAlarmRule = (id: string, durationMin = 30) =>
  request.patch<unknown, AlarmRuleDef>(`/api/alarm-rules/${encodeURIComponent(id)}/silence`, { duration: durationMin });
