...
export interface ChillerPlant {
  ...
  coolingCap: number;
  plr: number;
  storageTank: { level: number; dischargeMin: number; mode: string; capacity: number; topTemp: number; botTemp: number; flow: number; power: number };
  chillers: { id: string; state: string; load: number; cop: number; evapT: number | string; condT: number | string; current: number; runHrs: number }[];
  ...
  ambient: { outdoorTemp: number; outdoorRH: number; wetBulb: number; indoorTemp: number; indoorRH: number; freeCooling: string };
  hex: { id: string; state: string; eff: number; priIn: number; priOut: number; secIn: number; secOut: number }[];
...
export interface CracRoomLeak { status: string; level: string; position: number | null; zone: number }
export interface CracRoom {
  id: string; name: string
  avgTemp: number; avgRh: number
  hotAisle: number; hotRh: number
  coldAisle: number; coldRh: number
  inOutDiff: number; dewPoint: number
  cracRun: number; cracN: number; state: string
  leak: CracRoomLeak
}
export interface CracUnitControl {
  fanEnable: boolean; fanSpeedSet: number; waterValveSet: number
  coolingMode: string; humidOn: boolean
}
export interface CracUnitSetpoints {
  supplyTSet: number; rhSet: number; roomTSet: number
  highTempAlarm: number; lowTempAlarm: number; highRhAlarm: number
}
export interface CracUnit {
  id: string; room: string; type: string; state: string
  supplyT: number | string; returnT: number | string
  supplyRh: number | string; returnRh: number | string
  chilledWaterT: number | string; returnWaterT: number | string
  fan: number; valve: number; waterValve: number
  power: number | string; dp: number | string; filter: string
  control: CracUnitControl
  setpoints: CracUnitSetpoints
}
export interface CracLeakDevice {
  id: string; location: string; zone: number
  status: string; position: number | null
  cableLength: number; cableStatus: string
}
export interface CracLeak {
  total: number; alarm: number; devices: CracLeakDevice[]
}
export interface CracSummary {
  total: number; running: number; standby: number; fault: number; maint: number
  avgSupply: number | string; avgReturn: number | string
  avgSupplyWater: number | string; avgReturnWater: number | string
  outdoorRef: number; avgInOutDiff: number
  leakAlarm: number; leakTotal: number
}
export interface Crac {
  ...
  units: CracUnit[]
  leak: CracLeak
  fresh: { id: string; state: string; supplyT: number | string; rh: number | string; co2: number | string; filterDp: number | string }[];
  ...
  ctrl?: {
    ...
    secPump: { diffTarget: number; diffUnit: string; addHz: number; addDelayMin: number; reduceHz: number; reduceDelayMin: number; minRun: number; desc: string };
  };
...
export interface LiquidCoolingPrimaryCDU {
  ...
  heatExEff: number; priInTemp: number; priOutTemp: number;
  secInTemp: number; secOutTemp: number;
  flowPri: number; flowSec: number; dpPri: number; dpSec: number;
  pumpSpeed: number; pumpKw: number; valve: number;
  leakStatus: string; runHrs: number;
}
export interface LiquidCoolingSecondaryCDU {
  id: string; name: string; rackGroup: string;
  state: string; supplyTemp: number | string; returnTemp: number | string;
  flow: number; dp: number; pumpSpeed: number; pumpKw: number;
  leakStatus: string; coldPlateCount: number; coldPlateOnline: number;
}
export interface LiquidCoolingColdPlate {
  rackId: string; nodeType: string; inletTemp: number; outletTemp: number;
  flow: number; dp: number; gpuTemp: number[]; state: string;
}
export interface LiquidCoolingManifoldNode {
  id: string; zone: string; temp: number; pressure: number; flow: number;
  valvesOpen?: number; branchCount?: number;
}
export interface LiquidCoolingLeakRope {
  id: string; location: string; status: string; length: number; coverage: number;
}
export interface LiquidCoolingLeakPoint {
  id: string; zone: string; count: number; alarmCount: number;
}
export interface LiquidCoolingCoolantQuality {
  type: string; conductivity: number; ph: number;
  corrosionInhibitor: number; glycolConcentration: number;
  particleCount: number; lastTested: string; nextTest: string; status: string;
}
export interface LiquidCoolingTowerFan {
  id: string; state: string; fanHz: number; outletTemp: number | string; approach: number | string;
}
export interface LiquidCoolingDryCooler {
  id: string; state: string; fanHz: number; ambientT: number;
}
export interface LiquidCoolingRejectionPump {
  id: string; state: string; hz: number; kw: number;
}
export interface LiquidCoolingHeatRecovery {
  enabled: boolean; recoveryRate: number; recoveryTemp: number;
  returnTemp: number; flow: number; usageType: string;
  co2Reduction: number; annualSaving: number;
}
export interface LiquidCoolingControl {
  primarySupplySetpoint: number; secondarySupplySetpoint: number;
  approachTarget: number; glycolMin: number; conductivityMax: number;
  leakResponseTime: number; pumpRedundancy: string; cdurRedundancy: string;
  description: string;
}
export interface LiquidCooling {
  systemMode: string; outdoorT: number; outdoorRH: number;
  totalCoolingCap: number; coolingCapUsed: number; capRate: number;
  supplyTemp: number; returnTemp: number; primaryFlow: number; primaryPressure: number;
  secSupplyTemp: number; secReturnTemp: number; secFlow: number; secPressure: number;
  deltaT: number; pueContribution: number; freeCoolingHours: number; heatRecovery: number;
  ...
  leakDetection: {
    totalSensors: number; alarmCount: number; warningCount: number;
    ropeLeak: LiquidCoolingLeakRope[]; pointLeak: LiquidCoolingLeakPoint[];
  };
  coolantQuality: LiquidCoolingCoolantQuality;
  heatRejection: {
    type: string; towerFans: LiquidCoolingTowerFan[]; dryCoolers: LiquidCoolingDryCooler[];
    rejectionPumps: LiquidCoolingRejectionPump[]; totalHeatRejected: number;
    approachTemp: number; freeCoolingAvailable: boolean;
  };
  heatRecoveryDetail: LiquidCoolingHeatRecovery;
  controlStrategy: LiquidCoolingControl;
  supplyTempTrend: number[]; returnTempTrend: number[];
  flowTrend: number[]; deltaTTrend: number[];
  knowledge?: PowerKnowledge;
}
...
export interface HvPhase {
  ua: number; ub: number; uc: number;   // 三相线电压 kV
  ia: number; ib: number; ic: number;   // 三相电流 A
}
/** 10KV 进线柜 (双路市电) */
export interface HvIncomer {
  id: string; src: string; state: string; breaker: string;  // state=断路器分合
  ua: number; ub: number; uc: number; u: number;            // 三相+平均线电压 kV
  ia: number; ib: number; ic: number; i: number;            // 三相+平均电流 A
  p: number; q: number; pf: number; freq: number;           // 有功MW/无功Mvar/功率因数/频率
  energy: number;                                            // 当日有功电度 kWh
}
/** 10KV 出线馈线柜 */
export interface HvFeeder {
  id: string; load: string; state: string; breaker: string; // load=所带负荷名称
  ua: number; ub: number; uc: number;                       // 三相线电压 kV
  ia: number; ib: number; ic: number;                       // 三相电流 A
  p: number; pf: number; energy: number;                    // 功率MW/功率因数/累计电度 kWh
}
/** 遥信量 (开关量) */
export interface HvSignal { name: string; value: string; level: "g" | "a" | "r" | "b"; }
/** 10KV/0.4KV 配电变压器 (由出线供电) */
export interface HvTransformer {
  id: string; feeder: string; state: string;
  load: number;            // 负载率 %
  uHigh: number; iHigh: number;   // 高压侧 线电压kV/电流A
  uLow: number; iLow: number;     // 低压侧 线电压kV/电流A
  windingT: number; oilT: number; // 绕组温度/油温 ℃
  ambT: number; humidity: number; // 环境温度℃/湿度 %
  tap: number; fan: string;       // 有载分接档位/冷却风机
  signals: HvSignal[];            // 遥信量 (运行状态/保护告警/断路器位置等)
}
export interface HvBusTie { id: string; state: string; autoSwitch: string; mode: string; }
export interface HvAts { logic: string; lastTest: string; switchTime: string; }
export interface HvQuality { thdU: number; thdI: number; unbalance: number; }
...
export interface LvBranch {
  ...
  rated: number;         // 额定电流 A
  ua: number; ub: number; uc: number; u: number;   // 三相相电压 V + 平均
  ia: number; ib: number; ic: number; i: number;     // 三相电流 A + 平均
  freq: number;          // 频率 Hz
  p: number;             // 有功功率 kW
  q: number;             // 无功功率 kvar
  pf: number;            // 功率因数
...
export interface LvTransformer {
  id: string; load: number; t: number; state: string;
  u: number; i: number; p: number; q: number; pf: number; freq: number;   // 电压kV/电流A/有功kW/无功kvar/功率因数/频率
  energy: number; thdu: number; thdi: number;                             // 电度/谐波
}
export interface LvUpsGroup {
  id: string; n: string; load: number; uIn: number; uOut: number; mode: string; bypass: string; state: string;
  iIn: number; iOut: number; p: number; pf: number; freq: number; energyIn: number; thdu: number; thdi: number;
}
export interface LvHvdc {
  id: string; u: number; load: number; modN: number; modRun: number; state: string;
  i: number; p: number; pf: number; energy: number; thdi: number;
}
export interface LvAts {
  id: string; state: string; mode: string; lastSw: string;
  uIn: number; uOut: number; pf: number; p: number;
}
export interface LvBusbar {
  id: string; load: number; i: number; state: string;
  u: number; pf: number; energy: number; thdu: number;
}
...
export interface GensetFault { name: string; value: string; level: "g" | "a" | "r" | "b"; }
export interface GensetProtection { name: string; state: string; level: "g" | "a" | "r" | "b"; }
...
export interface GensetUnit {
  ...
  // 三相电参量 (高压 10kV 级)
  ua: number; ub: number; uc: number; u: number;     // 三相线电压 kV / 平均
  ia: number; ib: number; ic: number; i: number;     // 三相电流 A / 平均
  p: number; q: number; pf: number; freq: number;    // 有功kW / 无功kvar / 功率因数 / 频率Hz
  energy: number;           // 累计有功电度 kWh
  // 运行参数
  rpm: number; waterT: number; oilP: number; battU: number; heater: string; startCnt: number; runHrs: number;
...
export interface FuelSwitch { name: string; th: string; state: "断开" | "闭合"; level: "g" | "a" | "r" | "b"; }
/** 阀门分合状态 */
export interface FuelValve { name: string; state: string; level: "g" | "a" | "r" | "b"; }
/** 保护装置投退/动作状态 */
export interface FuelProtection { name: string; state: string; level: "g" | "a" | "r" | "b"; }
/** 油泵告警 (遥信) */
export interface FuelAlarm { name: string; value: string; level: "g" | "a" | "r" | "b"; }
/** 燃油监控 — 主油罐 / 日用油箱 / 输油泵 */
export interface MainTank {
  id: string; cap: number; level: number; t: number; water: string; leak: string;
  valves: FuelValve[];         // 进油阀 / 出油阀
  switches: FuelSwitch[];      // 低低位/低位/高位/高高位
  protections: FuelProtection[];
}
export interface DayTank {
  id: string; cap: number; level: number; leak: string;
  valve: FuelValve;            // 进油阀
  switches: FuelSwitch[];
  protections: FuelProtection[];
}
...
export interface BattGroup {
  id: string; type: string; soc: number;
  u: number;           // 电池组总电压 V
...
/** 设计/告警阈值 */
export interface KnowledgeThreshold { k: string; v: string; note?: string; }
/** 控制/切换逻辑步骤 */
export interface KnowledgeLogicStep { step: number; text: string; ok?: boolean; }
/** 一组控制/切换逻辑 */
export interface KnowledgeLogicGroup { title: string; steps: KnowledgeLogicStep[]; }
/** 故障锁定知识库 */
export interface KnowledgeFault { no: number; fault: string; lock: string; action: string; manualReset: boolean; }
/** 系统架构与组成 */
export interface KnowledgeArch { components: string[]; design: string; redundancy?: string; }
...
export interface Acs {
  doors: number; online: number; openAbnormal: number; todayEvents: number; denied: number; visitors: number;
...
export interface Fire {
  hostState: string; loops: number; points: number; faultPoints: number;
...
export interface CapacityForecastPoint { month: number; pct: number; lo: number; hi: number }
export interface CapacityForecastDim {
  id: string; unit: string; currentPct: number; slopePerMonth: number;
  projected: CapacityForecastPoint[]; warnMonth: number | null; fullMonth: number | null;
  status: string;
}
export interface CapacityForecastTrendRow { monthOffset: number; type: string; [dim: string]: number | string }
export interface CapacityForecast {
  method: string; generatedAt: string; historyMonths: number; horizonMonths: number;
  warnThreshold: number; fullThreshold: number;
  byDim: CapacityForecastDim[]; trend: CapacityForecastTrendRow[];
  headline: string; advice: string;
}
export interface Energy {
  todayKwh: number; monthKwh: number; yearKwh: number;
  pueTrend: number[];
...
export interface EnergySuggestion {
  id: string; title: string; priority: string; savingKw: number; savingPct: number;
  detail: string; basis: string;
}
export interface EnergyEfficiency {
  chillerCop: number; upsEff: number; upsAvgLoad: number;
  cracSupplyTemp: number | null; chillerSupplyTemp: number;
}
export interface EnergyAdvice {
  ...
  totalSavingKw: number; totalSavingPct: number;
  realData: { itLoadKw: number; coolingKw: number; chillerPowerKw: number; cracPowerKw: number;
    pumpPowerKw: number; facilityKw: number; distributionKw: number; dataSource: string };
  generatedAt: string;
}
/* ===== 工单模型 (CRUD 生命周期) ===== */
...
export const TICKET_STATUS_LABEL: Record<TicketStatus, string> = {
  open: "待处理", doing: "处理中", pending: "待归档", done: "已完成",
};
...
export interface Maintain {
  ...
  plans: { id: string; equip: string; cycle: string; last: string; next: string; vendor: string; state: string; deviceId?: string; category?: string; source?: "real" | "generated" }[];
