...
DOMAIN_MAP = [
    ...
    ("bms", "楼控-BA", ["ba", "楼控", "通讯", "总线", "modbus", "bacnet",
                      "网关", "离线", "链路", "BAS系统", "RS-485", "IO子站",
                      "群控", "UC", "GC", "PLC", "触摸屏"]),
    ("water", "给排水", ["漏水", "给水", "排水", "水泵", "水位", "补水",
                       "定压补水", "旁滤", "加药"]),
    ("dcim", "DCIM", ["dcim", "容量管理", "资产管理", "三维", "可视化"]),
    ("power_fuel", "电力-燃油", ["燃油", "油罐", "油箱", "供油泵", "回油泵",
                               "紧急切断阀", "液位"]),
]
...
METRIC_MAP = [
    ...
    ("return_temp", ["回水温度", "冷冻水回水", "回水"]),
    ("evap_temp", ["蒸发温度", "蒸发压力"]),
    ("cond_temp", ["冷凝温度", "冷凝压力"]),
    ("humidity", ["湿度", "相对湿度", "含湿量"]),
    ("voltage", ["电压", "母线电压", "进线电压"]),
    ("current", ["电流", "负载电流""FLA"]),
    ("temperature", ["温度", "温升", "进水温度"]),
    ("power", ["功率", "负荷", "制冷量"]),
    ("soc", ["电量", "soc", "容量"]),
    ("flow", ["流量", "水流量", "冷冻水流量"]),
    ("pressure", ["压力", "压差", "供回水压差"]),
    ("wet_bulb", ["湿球温度", "室外湿球"]),
    ("load_pct", ["负载率", "负荷率", "FLA"]),
    ("cooling_mode", ["运行模式", "制冷模式", "预冷模式", "自然冷却"]),
    ("fan_hz", ["风机频率", "冷却塔频率"]),
    ("pump_hz", ["水泵频率", "泵频率", "一次泵", "二次泵"]),
    ("oil_temp", ["油温", "油位", "机油温度"]),
    ("fuel_level", ["油位", "液位", "油箱"]),
    ("leak", ["渗漏", "漏水", "漏油", "泄漏"]),
    ("valve_pos", ["阀门开度", "阀门", "电动阀", "水阀"]),
]
